...
static inline bool ts_query__pattern_map_search(
    ...
    while (size > 1) {
        ...
        TSSymbol mid_symbol = self->steps.contents[self->pattern_map.contents[mid_index].step_index].symbol;
    ...
    TSSymbol symbol = self->steps.contents[self->pattern_map.contents[base_index].step_index].symbol;
    ...
    while (stream.input < stream.end) {
        ...
        *error_type = ts_query__parse_pattern(self, &stream, 0, false, &capture_quantifiers);
        ...
        for (;;) {
            ...
            ts_query__pattern_map_insert(self, step->symbol, (PatternEntry){.step_index = start_step_index, .pattern_index = pattern_index, .is_rooted = is_rooted});
            if (step->symbol == WILDCARD_SYMBOL) {
...
static inline bool ts_query_cursor__advance(
    ...
    for (;;) {
        ...
        // Enter a new node.
            ...
            bool parent_precedes_range = !ts_node_is_null(parent_node) && (ts_node_end_byte(parent_node) <= self->start_byte ||
                                                                           point_lte(ts_node_end_point(parent_node), self->start_point)
                                                                          );
            bool parent_follows_range = !ts_node_is_null(parent_node) && (ts_node_start_byte(parent_node) >= self->end_byte ||
                                                                          point_gte(ts_node_start_point(parent_node), self->end_point)
                                                                         );
            bool node_precedes_range = parent_precedes_range || (ts_node_end_byte(node) <= self->start_byte ||
                                                                 point_lte(ts_node_end_point(node), self->start_point)
                                                                );
            bool node_follows_range = parent_follows_range || (ts_node_start_byte(node) >= self->end_byte ||
