#include <tree_sitter/parser.h>

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#define LANGUAGE_VERSION 14
#define STATE_COUNT 479
#define LARGE_STATE_COUNT 7
#define SYMBOL_COUNT 137
#define ALIAS_COUNT 0
#define TOKEN_COUNT 72
#define EXTERNAL_TOKEN_COUNT 6
#define FIELD_COUNT 21
#define MAX_ALIAS_SEQUENCE_LENGTH 7
#define PRODUCTION_ID_COUNT 59

enum {
    sym_identifier = 1,
    sym_hash_bang_line = 2,
    anon_sym_return = 3,
    anon_sym_SEMI = 4,
    anon_sym_EQ = 5,
    anon_sym_COMMA = 6,
    anon_sym_COLON_COLON = 7,
    sym_break_statement = 8,
    anon_sym_goto = 9,
    anon_sym_do = 10,
    anon_sym_end = 11,
    anon_sym_while = 12,
    anon_sym_repeat = 13,
    anon_sym_until = 14,
    anon_sym_if = 15,
    anon_sym_then = 16,
    anon_sym_elseif = 17,
    anon_sym_else = 18,
    anon_sym_for = 19,
    anon_sym_in = 20,
    anon_sym_function = 21,
    anon_sym_local = 22,
    anon_sym_DOT = 23,
    anon_sym_COLON = 24,
    anon_sym_LT = 25,
    anon_sym_GT = 26,
    sym_nil = 27,
    sym_false = 28,
    sym_true = 29,
    sym_number = 30,
    anon_sym_DQUOTE = 31,
    anon_sym_SQUOTE = 32,
    aux_sym__doublequote_string_content_token1 = 33,
    aux_sym__singlequote_string_content_token1 = 34,
    sym_escape_sequence = 35,
    sym_vararg_expression = 36,
    anon_sym_LPAREN = 37,
    anon_sym_RPAREN = 38,
    anon_sym_LBRACK = 39,
    anon_sym_RBRACK = 40,
    anon_sym_LBRACE = 41,
    anon_sym_RBRACE = 42,
    anon_sym_or = 43,
    anon_sym_and = 44,
    anon_sym_LT_EQ = 45,
    anon_sym_EQ_EQ = 46,
    anon_sym_TILDE_EQ = 47,
    anon_sym_GT_EQ = 48,
    anon_sym_PIPE = 49,
    anon_sym_TILDE = 50,
    anon_sym_AMP = 51,
    anon_sym_LT_LT = 52,
    anon_sym_GT_GT = 53,
    anon_sym_PLUS = 54,
    anon_sym_DASH = 55,
    anon_sym_STAR = 56,
    anon_sym_SLASH = 57,
    anon_sym_SLASH_SLASH = 58,
    anon_sym_PERCENT = 59,
    anon_sym_DOT_DOT = 60,
    anon_sym_CARET = 61,
    anon_sym_not = 62,
    anon_sym_POUND = 63,
    anon_sym_DASH_DASH = 64,
    aux_sym_comment_token1 = 65,
    sym__block_comment_start = 66,
    sym__block_comment_content = 67,
    sym__block_comment_end = 68,
    sym__block_string_start = 69,
    sym__block_string_content = 70,
    sym__block_string_end = 71,
    sym_chunk = 72,
    sym__block = 73,
    sym_statement = 74,
    sym_return_statement = 75,
    sym_empty_statement = 76,
    sym_assignment_statement = 77,
    sym__variable_assignment_varlist = 78,
    sym__variable_assignment_explist = 79,
    sym_label_statement = 80,
    sym_goto_statement = 81,
    sym_do_statement = 82,
    sym_while_statement = 83,
    sym_repeat_statement = 84,
    sym_if_statement = 85,
    sym_elseif_statement = 86,
    sym_else_statement = 87,
    sym_for_statement = 88,
    sym_for_generic_clause = 89,
    sym_for_numeric_clause = 90,
    sym_declaration = 91,
    sym_function_declaration = 92,
    sym__local_function_declaration = 93,
    sym__function_name = 94,
    sym__function_name_prefix_expression = 95,
    sym__function_name_dot_index_expression = 96,
    sym__function_name_method_index_expression = 97,
    sym_variable_declaration = 98,
    sym__local_variable_assignment = 99,
    sym__name_list = 100,
    sym__att_name_list = 101,
    sym__attrib = 102,
    sym__expression_list = 103,
    sym_expression = 104,
    sym_string = 105,
    sym__quote_string = 106,
    aux_sym__doublequote_string_content = 107,
    aux_sym__singlequote_string_content = 108,
    sym__block_string = 109,
    sym_function_definition = 110,
    sym__function_body = 111,
    sym_parameters = 112,
    sym__parameter_list = 113,
    sym__prefix_expression = 114,
    sym_variable = 115,
    sym_bracket_index_expression = 116,
    sym_dot_index_expression = 117,
    sym_function_call = 118,
    sym_method_index_expression = 119,
    sym_arguments = 120,
    sym_parenthesized_expression = 121,
    sym_table_constructor = 122,
    sym__field_list = 123,
    sym__field_sep = 124,
    sym_field = 125,
    sym_binary_expression = 126,
    sym_unary_expression = 127,
    sym_comment = 128,
    aux_sym_chunk_repeat1 = 129,
    aux_sym__variable_assignment_varlist_repeat1 = 130,
    aux_sym__variable_assignment_explist_repeat1 = 131,
    aux_sym_if_statement_repeat1 = 132,
    aux_sym__name_list_repeat1 = 133,
    aux_sym__att_name_list_repeat1 = 134,
    aux_sym__expression_list_repeat1 = 135,
    aux_sym__field_list_repeat1 = 136,
};

static const char *const ts_symbol_names[] = {
    [ts_builtin_sym_end] = "end",
    [sym_identifier] = "identifier",
    [sym_hash_bang_line] = "hash_bang_line",
    [anon_sym_return] = "return",
    [anon_sym_SEMI] = ";",
    [anon_sym_EQ] = "=",
    [anon_sym_COMMA] = ",",
    [anon_sym_COLON_COLON] = "::",
    [sym_break_statement] = "break_statement",
    [anon_sym_goto] = "goto",
    [anon_sym_do] = "do",
    [anon_sym_end] = "end",
    [anon_sym_while] = "while",
    [anon_sym_repeat] = "repeat",
    [anon_sym_until] = "until",
    [anon_sym_if] = "if",
    [anon_sym_then] = "then",
    [anon_sym_elseif] = "elseif",
    [anon_sym_else] = "else",
    [anon_sym_for] = "for",
    [anon_sym_in] = "in",
    [anon_sym_function] = "function",
    [anon_sym_local] = "local",
    [anon_sym_DOT] = ".",
    [anon_sym_COLON] = ":",
    [anon_sym_LT] = "<",
    [anon_sym_GT] = ">",
    [sym_nil] = "nil",
    [sym_false] = "false",
    [sym_true] = "true",
    [sym_number] = "number",
    [anon_sym_DQUOTE] = "\"",
    [anon_sym_SQUOTE] = "'",
    [aux_sym__doublequote_string_content_token1] = "_doublequote_string_content_token1",
    [aux_sym__singlequote_string_content_token1] = "_singlequote_string_content_token1",
    [sym_escape_sequence] = "escape_sequence",
    [sym_vararg_expression] = "vararg_expression",
    [anon_sym_LPAREN] = "(",
    [anon_sym_RPAREN] = ")",
    [anon_sym_LBRACK] = "[",
    [anon_sym_RBRACK] = "]",
    [anon_sym_LBRACE] = "{",
    [anon_sym_RBRACE] = "}",
    [anon_sym_or] = "or",
    [anon_sym_and] = "and",
    [anon_sym_LT_EQ] = "<=",
    [anon_sym_EQ_EQ] = "==",
    [anon_sym_TILDE_EQ] = "~=",
    [anon_sym_GT_EQ] = ">=",
    [anon_sym_PIPE] = "|",
    [anon_sym_TILDE] = "~",
    [anon_sym_AMP] = "&",
    [anon_sym_LT_LT] = "<<",
    [anon_sym_GT_GT] = ">>",
    [anon_sym_PLUS] = "+",
    [anon_sym_DASH] = "-",
    [anon_sym_STAR] = "*",
    [anon_sym_SLASH] = "/",
    [anon_sym_SLASH_SLASH] = "//",
    [anon_sym_PERCENT] = "%",
    [anon_sym_DOT_DOT] = "..",
    [anon_sym_CARET] = "^",
    [anon_sym_not] = "not",
    [anon_sym_POUND] = "#",
    [anon_sym_DASH_DASH] = "--",
    [aux_sym_comment_token1] = "comment_content",
    [sym__block_comment_start] = "[[",
    [sym__block_comment_content] = "comment_content",
    [sym__block_comment_end] = "]]",
    [sym__block_string_start] = "[[",
    [sym__block_string_content] = "string_content",
    [sym__block_string_end] = "]]",
    [sym_chunk] = "chunk",
    [sym__block] = "block",
    [sym_statement] = "statement",
    [sym_return_statement] = "return_statement",
    [sym_empty_statement] = "empty_statement",
    [sym_assignment_statement] = "assignment_statement",
    [sym__variable_assignment_varlist] = "variable_list",
    [sym__variable_assignment_explist] = "expression_list",
    [sym_label_statement] = "label_statement",
    [sym_goto_statement] = "goto_statement",
    [sym_do_statement] = "do_statement",
    [sym_while_statement] = "while_statement",
    [sym_repeat_statement] = "repeat_statement",
    [sym_if_statement] = "if_statement",
    [sym_elseif_statement] = "elseif_statement",
    [sym_else_statement] = "else_statement",
    [sym_for_statement] = "for_statement",
    [sym_for_generic_clause] = "for_generic_clause",
    [sym_for_numeric_clause] = "for_numeric_clause",
    [sym_declaration] = "declaration",
    [sym_function_declaration] = "function_declaration",
    [sym__local_function_declaration] = "function_declaration",
    [sym__function_name] = "_function_name",
    [sym__function_name_prefix_expression] = "_function_name_prefix_expression",
    [sym__function_name_dot_index_expression] = "dot_index_expression",
    [sym__function_name_method_index_expression] = "method_index_expression",
    [sym_variable_declaration] = "variable_declaration",
    [sym__local_variable_assignment] = "assignment_statement",
    [sym__name_list] = "variable_list",
    [sym__att_name_list] = "variable_list",
    [sym__attrib] = "attribute",
    [sym__expression_list] = "expression_list",
    [sym_expression] = "expression",
    [sym_string] = "string",
    [sym__quote_string] = "_quote_string",
    [aux_sym__doublequote_string_content] = "_doublequote_string_content",
    [aux_sym__singlequote_string_content] = "_singlequote_string_content",
    [sym__block_string] = "_block_string",
    [sym_function_definition] = "function_definition",
    [sym__function_body] = "_function_body",
    [sym_parameters] = "parameters",
    [sym__parameter_list] = "_parameter_list",
    [sym__prefix_expression] = "_prefix_expression",
    [sym_variable] = "variable",
    [sym_bracket_index_expression] = "bracket_index_expression",
    [sym_dot_index_expression] = "dot_index_expression",
    [sym_function_call] = "function_call",
    [sym_method_index_expression] = "method_index_expression",
    [sym_arguments] = "arguments",
    [sym_parenthesized_expression] = "parenthesized_expression",
    [sym_table_constructor] = "table_constructor",
    [sym__field_list] = "_field_list",
    [sym__field_sep] = "_field_sep",
    [sym_field] = "field",
    [sym_binary_expression] = "binary_expression",
    [sym_unary_expression] = "unary_expression",
    [sym_comment] = "comment",
    [aux_sym_chunk_repeat1] = "chunk_repeat1",
    [aux_sym__variable_assignment_varlist_repeat1] = "_variable_assignment_varlist_repeat1",
    [aux_sym__variable_assignment_explist_repeat1] = "_variable_assignment_explist_repeat1",
    [aux_sym_if_statement_repeat1] = "if_statement_repeat1",
    [aux_sym__name_list_repeat1] = "_name_list_repeat1",
    [aux_sym__att_name_list_repeat1] = "_att_name_list_repeat1",
    [aux_sym__expression_list_repeat1] = "_expression_list_repeat1",
    [aux_sym__field_list_repeat1] = "_field_list_repeat1",
};

static const TSSymbol ts_symbol_map[] = {
    [ts_builtin_sym_end] = ts_builtin_sym_end,
    [sym_identifier] = sym_identifier,
    [sym_hash_bang_line] = sym_hash_bang_line,
    [anon_sym_return] = anon_sym_return,
    [anon_sym_SEMI] = anon_sym_SEMI,
    [anon_sym_EQ] = anon_sym_EQ,
    [anon_sym_COMMA] = anon_sym_COMMA,
    [anon_sym_COLON_COLON] = anon_sym_COLON_COLON,
    [sym_break_statement] = sym_break_statement,
    [anon_sym_goto] = anon_sym_goto,
    [anon_sym_do] = anon_sym_do,
    [anon_sym_end] = anon_sym_end,
    [anon_sym_while] = anon_sym_while,
    [anon_sym_repeat] = anon_sym_repeat,
    [anon_sym_until] = anon_sym_until,
    [anon_sym_if] = anon_sym_if,
    [anon_sym_then] = anon_sym_then,
    [anon_sym_elseif] = anon_sym_elseif,
    [anon_sym_else] = anon_sym_else,
    [anon_sym_for] = anon_sym_for,
    [anon_sym_in] = anon_sym_in,
    [anon_sym_function] = anon_sym_function,
    [anon_sym_local] = anon_sym_local,
    [anon_sym_DOT] = anon_sym_DOT,
    [anon_sym_COLON] = anon_sym_COLON,
    [anon_sym_LT] = anon_sym_LT,
    [anon_sym_GT] = anon_sym_GT,
    [sym_nil] = sym_nil,
    [sym_false] = sym_false,
    [sym_true] = sym_true,
    [sym_number] = sym_number,
    [anon_sym_DQUOTE] = anon_sym_DQUOTE,
    [anon_sym_SQUOTE] = anon_sym_SQUOTE,
    [aux_sym__doublequote_string_content_token1] = aux_sym__doublequote_string_content_token1,
    [aux_sym__singlequote_string_content_token1] = aux_sym__singlequote_string_content_token1,
    [sym_escape_sequence] = sym_escape_sequence,
    [sym_vararg_expression] = sym_vararg_expression,
    [anon_sym_LPAREN] = anon_sym_LPAREN,
    [anon_sym_RPAREN] = anon_sym_RPAREN,
    [anon_sym_LBRACK] = anon_sym_LBRACK,
    [anon_sym_RBRACK] = anon_sym_RBRACK,
    [anon_sym_LBRACE] = anon_sym_LBRACE,
    [anon_sym_RBRACE] = anon_sym_RBRACE,
    [anon_sym_or] = anon_sym_or,
    [anon_sym_and] = anon_sym_and,
    [anon_sym_LT_EQ] = anon_sym_LT_EQ,
    [anon_sym_EQ_EQ] = anon_sym_EQ_EQ,
    [anon_sym_TILDE_EQ] = anon_sym_TILDE_EQ,
    [anon_sym_GT_EQ] = anon_sym_GT_EQ,
    [anon_sym_PIPE] = anon_sym_PIPE,
    [anon_sym_TILDE] = anon_sym_TILDE,
    [anon_sym_AMP] = anon_sym_AMP,
    [anon_sym_LT_LT] = anon_sym_LT_LT,
    [anon_sym_GT_GT] = anon_sym_GT_GT,
    [anon_sym_PLUS] = anon_sym_PLUS,
    [anon_sym_DASH] = anon_sym_DASH,
    [anon_sym_STAR] = anon_sym_STAR,
    [anon_sym_SLASH] = anon_sym_SLASH,
    [anon_sym_SLASH_SLASH] = anon_sym_SLASH_SLASH,
    [anon_sym_PERCENT] = anon_sym_PERCENT,
    [anon_sym_DOT_DOT] = anon_sym_DOT_DOT,
    [anon_sym_CARET] = anon_sym_CARET,
    [anon_sym_not] = anon_sym_not,
    [anon_sym_POUND] = anon_sym_POUND,
    [anon_sym_DASH_DASH] = anon_sym_DASH_DASH,
    [aux_sym_comment_token1] = sym__block_comment_content,
    [sym__block_comment_start] = sym__block_comment_start,
    [sym__block_comment_content] = sym__block_comment_content,
    [sym__block_comment_end] = sym__block_comment_end,
    [sym__block_string_start] = sym__block_comment_start,
    [sym__block_string_content] = sym__block_string_content,
    [sym__block_string_end] = sym__block_comment_end,
    [sym_chunk] = sym_chunk,
    [sym__block] = sym__block,
    [sym_statement] = sym_statement,
    [sym_return_statement] = sym_return_statement,
    [sym_empty_statement] = sym_empty_statement,
    [sym_assignment_statement] = sym_assignment_statement,
    [sym__variable_assignment_varlist] = sym__variable_assignment_varlist,
    [sym__variable_assignment_explist] = sym__variable_assignment_explist,
    [sym_label_statement] = sym_label_statement,
    [sym_goto_statement] = sym_goto_statement,
    [sym_do_statement] = sym_do_statement,
    [sym_while_statement] = sym_while_statement,
    [sym_repeat_statement] = sym_repeat_statement,
    [sym_if_statement] = sym_if_statement,
    [sym_elseif_statement] = sym_elseif_statement,
    [sym_else_statement] = sym_else_statement,
    [sym_for_statement] = sym_for_statement,
    [sym_for_generic_clause] = sym_for_generic_clause,
    [sym_for_numeric_clause] = sym_for_numeric_clause,
    [sym_declaration] = sym_declaration,
    [sym_function_declaration] = sym_function_declaration,
    [sym__local_function_declaration] = sym_function_declaration,
    [sym__function_name] = sym__function_name,
    [sym__function_name_prefix_expression] = sym__function_name_prefix_expression,
    [sym__function_name_dot_index_expression] = sym_dot_index_expression,
    [sym__function_name_method_index_expression] = sym_method_index_expression,
    [sym_variable_declaration] = sym_variable_declaration,
    [sym__local_variable_assignment] = sym_assignment_statement,
    [sym__name_list] = sym__variable_assignment_varlist,
    [sym__att_name_list] = sym__variable_assignment_varlist,
    [sym__attrib] = sym__attrib,
    [sym__expression_list] = sym__variable_assignment_explist,
    [sym_expression] = sym_expression,
    [sym_string] = sym_string,
    [sym__quote_string] = sym__quote_string,
    [aux_sym__doublequote_string_content] = aux_sym__doublequote_string_content,
    [aux_sym__singlequote_string_content] = aux_sym__singlequote_string_content,
    [sym__block_string] = sym__block_string,
    [sym_function_definition] = sym_function_definition,
    [sym__function_body] = sym__function_body,
    [sym_parameters] = sym_parameters,
    [sym__parameter_list] = sym__parameter_list,
    [sym__prefix_expression] = sym__prefix_expression,
    [sym_variable] = sym_variable,
    [sym_bracket_index_expression] = sym_bracket_index_expression,
    [sym_dot_index_expression] = sym_dot_index_expression,
    [sym_function_call] = sym_function_call,
    [sym_method_index_expression] = sym_method_index_expression,
    [sym_arguments] = sym_arguments,
    [sym_parenthesized_expression] = sym_parenthesized_expression,
    [sym_table_constructor] = sym_table_constructor,
    [sym__field_list] = sym__field_list,
    [sym__field_sep] = sym__field_sep,
    [sym_field] = sym_field,
    [sym_binary_expression] = sym_binary_expression,
    [sym_unary_expression] = sym_unary_expression,
    [sym_comment] = sym_comment,
    [aux_sym_chunk_repeat1] = aux_sym_chunk_repeat1,
    [aux_sym__variable_assignment_varlist_repeat1] = aux_sym__variable_assignment_varlist_repeat1,
    [aux_sym__variable_assignment_explist_repeat1] = aux_sym__variable_assignment_explist_repeat1,
    [aux_sym_if_statement_repeat1] = aux_sym_if_statement_repeat1,
    [aux_sym__name_list_repeat1] = aux_sym__name_list_repeat1,
    [aux_sym__att_name_list_repeat1] = aux_sym__att_name_list_repeat1,
    [aux_sym__expression_list_repeat1] = aux_sym__expression_list_repeat1,
    [aux_sym__field_list_repeat1] = aux_sym__field_list_repeat1,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
    [ts_builtin_sym_end] = {
        .visible = false,
        .named = true,
    },
    [sym_identifier] = {
        .visible = true,
        .named = true,
    },
    [sym_hash_bang_line] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_return] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_SEMI] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_EQ] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_COMMA] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_COLON_COLON] = {
        .visible = true,
        .named = false,
    },
    [sym_break_statement] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_goto] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_do] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_end] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_while] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_repeat] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_until] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_if] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_then] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_elseif] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_else] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_for] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_in] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_function] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_local] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_DOT] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_COLON] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_LT] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_GT] = {
        .visible = true,
        .named = false,
    },
    [sym_nil] = {
        .visible = true,
        .named = true,
    },
    [sym_false] = {
        .visible = true,
        .named = true,
    },
    [sym_true] = {
        .visible = true,
        .named = true,
    },
    [sym_number] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_DQUOTE] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_SQUOTE] = {
        .visible = true,
        .named = false,
    },
    [aux_sym__doublequote_string_content_token1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__singlequote_string_content_token1] = {
        .visible = false,
        .named = false,
    },
    [sym_escape_sequence] = {
        .visible = true,
        .named = true,
    },
    [sym_vararg_expression] = {
        .visible = true,
        .named = true,
    },
    [anon_sym_LPAREN] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_RPAREN] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_LBRACK] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_RBRACK] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_LBRACE] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_RBRACE] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_or] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_and] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_LT_EQ] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_EQ_EQ] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_TILDE_EQ] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_GT_EQ] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_PIPE] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_TILDE] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_AMP] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_LT_LT] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_GT_GT] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_PLUS] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_DASH] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_STAR] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_SLASH] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_SLASH_SLASH] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_PERCENT] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_DOT_DOT] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_CARET] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_not] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_POUND] = {
        .visible = true,
        .named = false,
    },
    [anon_sym_DASH_DASH] = {
        .visible = true,
        .named = false,
    },
    [aux_sym_comment_token1] = {
        .visible = true,
        .named = true,
    },
    [sym__block_comment_start] = {
        .visible = true,
        .named = false,
    },
    [sym__block_comment_content] = {
        .visible = true,
        .named = true,
    },
    [sym__block_comment_end] = {
        .visible = true,
        .named = false,
    },
    [sym__block_string_start] = {
        .visible = true,
        .named = false,
    },
    [sym__block_string_content] = {
        .visible = true,
        .named = true,
    },
    [sym__block_string_end] = {
        .visible = true,
        .named = false,
    },
    [sym_chunk] = {
        .visible = true,
        .named = true,
    },
    [sym__block] = {
        .visible = true,
        .named = true,
    },
    [sym_statement] = {
        .visible = false,
        .named = true,
        .supertype = true,
    },
    [sym_return_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_empty_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_assignment_statement] = {
        .visible = true,
        .named = true,
    },
    [sym__variable_assignment_varlist] = {
        .visible = true,
        .named = true,
    },
    [sym__variable_assignment_explist] = {
        .visible = true,
        .named = true,
    },
    [sym_label_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_goto_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_do_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_while_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_repeat_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_if_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_elseif_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_else_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_for_statement] = {
        .visible = true,
        .named = true,
    },
    [sym_for_generic_clause] = {
        .visible = true,
        .named = true,
    },
    [sym_for_numeric_clause] = {
        .visible = true,
        .named = true,
    },
    [sym_declaration] = {
        .visible = false,
        .named = true,
        .supertype = true,
    },
    [sym_function_declaration] = {
        .visible = true,
        .named = true,
    },
    [sym__local_function_declaration] = {
        .visible = true,
        .named = true,
    },
    [sym__function_name] = {
        .visible = false,
        .named = true,
    },
    [sym__function_name_prefix_expression] = {
        .visible = false,
        .named = true,
    },
    [sym__function_name_dot_index_expression] = {
        .visible = true,
        .named = true,
    },
    [sym__function_name_method_index_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_variable_declaration] = {
        .visible = true,
        .named = true,
    },
    [sym__local_variable_assignment] = {
        .visible = true,
        .named = true,
    },
    [sym__name_list] = {
        .visible = true,
        .named = true,
    },
    [sym__att_name_list] = {
        .visible = true,
        .named = true,
    },
    [sym__attrib] = {
        .visible = true,
        .named = true,
    },
    [sym__expression_list] = {
        .visible = true,
        .named = true,
    },
    [sym_expression] = {
        .visible = false,
        .named = true,
        .supertype = true,
    },
    [sym_string] = {
        .visible = true,
        .named = true,
    },
    [sym__quote_string] = {
        .visible = false,
        .named = true,
    },
    [aux_sym__doublequote_string_content] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__singlequote_string_content] = {
        .visible = false,
        .named = false,
    },
    [sym__block_string] = {
        .visible = false,
        .named = true,
    },
    [sym_function_definition] = {
        .visible = true,
        .named = true,
    },
    [sym__function_body] = {
        .visible = false,
        .named = true,
    },
    [sym_parameters] = {
        .visible = true,
        .named = true,
    },
    [sym__parameter_list] = {
        .visible = false,
        .named = true,
    },
    [sym__prefix_expression] = {
        .visible = false,
        .named = true,
    },
    [sym_variable] = {
        .visible = false,
        .named = true,
        .supertype = true,
    },
    [sym_bracket_index_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_dot_index_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_function_call] = {
        .visible = true,
        .named = true,
    },
    [sym_method_index_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_arguments] = {
        .visible = true,
        .named = true,
    },
    [sym_parenthesized_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_table_constructor] = {
        .visible = true,
        .named = true,
    },
    [sym__field_list] = {
        .visible = false,
        .named = true,
    },
    [sym__field_sep] = {
        .visible = false,
        .named = true,
    },
    [sym_field] = {
        .visible = true,
        .named = true,
    },
    [sym_binary_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_unary_expression] = {
        .visible = true,
        .named = true,
    },
    [sym_comment] = {
        .visible = true,
        .named = true,
    },
    [aux_sym_chunk_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__variable_assignment_varlist_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__variable_assignment_explist_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym_if_statement_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__name_list_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__att_name_list_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__expression_list_repeat1] = {
        .visible = false,
        .named = false,
    },
    [aux_sym__field_list_repeat1] = {
        .visible = false,
        .named = false,
    },
};

enum {
    field_alternative = 1,
    field_arguments = 2,
    field_attribute = 3,
    field_body = 4,
    field_clause = 5,
    field_condition = 6,
    field_consequence = 7,
    field_content = 8,
    field_end = 9,
    field_field = 10,
    field_left = 11,
    field_local_declaration = 12,
    field_method = 13,
    field_name = 14,
    field_operand = 15,
    field_parameters = 16,
    field_right = 17,
    field_start = 18,
    field_step = 19,
    field_table = 20,
    field_value = 21,
};

static const char *const ts_field_names[] = {
    [0] = NULL,
    [field_alternative] = "alternative",
    [field_arguments] = "arguments",
    [field_attribute] = "attribute",
    [field_body] = "body",
    [field_clause] = "clause",
    [field_condition] = "condition",
    [field_consequence] = "consequence",
    [field_content] = "content",
    [field_end] = "end",
    [field_field] = "field",
    [field_left] = "left",
    [field_local_declaration] = "local_declaration",
    [field_method] = "method",
    [field_name] = "name",
    [field_operand] = "operand",
    [field_parameters] = "parameters",
    [field_right] = "right",
    [field_start] = "start",
    [field_step] = "step",
    [field_table] = "table",
    [field_value] = "value",
};

static const TSFieldMapSlice ts_field_map_slices[PRODUCTION_ID_COUNT] = {
    [1] = {.index = 0, .length = 1},
    [2] = {.index = 1, .length = 4},
    [3] = {.index = 5, .length = 1},
    [4] = {.index = 6, .length = 3},
    [5] = {.index = 9, .length = 1},
    [6] = {.index = 10, .length = 2},
    [7] = {.index = 12, .length = 2},
    [8] = {.index = 14, .length = 2},
    [9] = {.index = 16, .length = 2},
    [10] = {.index = 18, .length = 2},
    [11] = {.index = 20, .length = 2},
    [12] = {.index = 22, .length = 1},
    [13] = {.index = 23, .length = 1},
    [14] = {.index = 24, .length = 2},
    [15] = {.index = 26, .length = 2},
    [16] = {.index = 28, .length = 3},
    [17] = {.index = 31, .length = 1},
    [18] = {.index = 32, .length = 1},
    [19] = {.index = 33, .length = 3},
    [20] = {.index = 36, .length = 1},
    [21] = {.index = 37, .length = 2},
    [22] = {.index = 39, .length = 3},
    [23] = {.index = 28, .length = 3},
    [24] = {.index = 42, .length = 2},
    [25] = {.index = 44, .length = 2},
    [26] = {.index = 46, .length = 2},
    [27] = {.index = 48, .length = 2},
    [28] = {.index = 50, .length = 1},
    [29] = {.index = 51, .length = 2},
    [30] = {.index = 53, .length = 1},
    [31] = {.index = 54, .length = 2},
    [32] = {.index = 56, .length = 1},
    [33] = {.index = 57, .length = 1},
    [34] = {.index = 58, .length = 1},
    [35] = {.index = 59, .length = 1},
    [36] = {.index = 60, .length = 2},
    [37] = {.index = 62, .length = 3},
    [38] = {.index = 65, .length = 4},
    [39] = {.index = 69, .length = 4},
    [40] = {.index = 73, .length = 3},
    [41] = {.index = 76, .length = 2},
    [42] = {.index = 78, .length = 2},
    [43] = {.index = 80, .length = 2},
    [44] = {.index = 82, .length = 2},
    [45] = {.index = 84, .length = 2},
    [46] = {.index = 86, .length = 2},
    [47] = {.index = 88, .length = 2},
    [48] = {.index = 90, .length = 2},
    [49] = {.index = 92, .length = 2},
    [50] = {.index = 94, .length = 1},
    [51] = {.index = 95, .length = 2},
    [52] = {.index = 97, .length = 3},
    [53] = {.index = 100, .length = 3},
    [54] = {.index = 103, .length = 3},
    [55] = {.index = 106, .length = 3},
    [56] = {.index = 109, .length = 2},
    [57] = {.index = 111, .length = 4},
    [58] = {.index = 115, .length = 4},
};

static const TSFieldMapEntry ts_field_map_entries[] = {
    [0] =
        {field_local_declaration, 0, .inherited = true},
    [1] =
        {field_body, 0, .inherited = true},
    {field_local_declaration, 0},
    {field_name, 0, .inherited = true},
    {field_parameters, 0, .inherited = true},
    [5] =
        {field_local_declaration, 0},
    [6] =
        {field_content, 0, .inherited = true},
    {field_end, 0, .inherited = true},
    {field_start, 0, .inherited = true},
    [9] =
        {field_name, 0},
    [10] =
        {field_content, 1},
    {field_start, 0},
    [12] =
        {field_field, 0, .inherited = true},
    {field_table, 0, .inherited = true},
    [14] =
        {field_method, 0, .inherited = true},
    {field_table, 0, .inherited = true},
    [16] =
        {field_body, 1, .inherited = true},
    {field_parameters, 1, .inherited = true},
    [18] =
        {field_attribute, 1, .inherited = true},
    {field_name, 1, .inherited = true},
    [20] =
        {field_end, 1},
    {field_start, 0},
    [22] =
        {field_value, 0},
    [23] =
        {field_operand, 1},
    [24] =
        {field_arguments, 1},
    {field_name, 0},
    [26] =
        {field_name, 0},
    {field_name, 1, .inherited = true},
    [28] =
        {field_content, 1},
    {field_end, 2},
    {field_start, 0},
    [31] =
        {field_body, 1},
    [32] =
        {field_condition, 2},
    [33] =
        {field_body, 2, .inherited = true},
    {field_name, 1},
    {field_parameters, 2, .inherited = true},
    [36] =
        {field_parameters, 0},
    [37] =
        {field_attribute, 1},
    {field_name, 0},
    [39] =
        {field_attribute, 1, .inherited = true},
    {field_name, 0},
    {field_name, 1, .inherited = true},
    [42] =
        {field_name, 0, .inherited = true},
    {field_value, 2, .inherited = true},
    [44] =
        {field_left, 0},
    {field_right, 2},
    [46] =
        {field_field, 2},
    {field_table, 0},
    [48] =
        {field_method, 2},
    {field_table, 0},
    [50] =
        {field_name, 1},
    [51] =
        {field_name, 0, .inherited = true},
    {field_name, 1, .inherited = true},
    [53] =
        {field_condition, 1},
    [54] =
        {field_body, 1},
    {field_condition, 3},
    [56] =
        {field_alternative, 0},
    [57] =
        {field_clause, 1},
    [58] =
        {field_name, 0, .inherited = true},
    [59] =
        {field_name, 1, .inherited = true},
    [60] =
        {field_body, 1},
    {field_parameters, 0},
    [62] =
        {field_body, 3, .inherited = true},
    {field_name, 2},
    {field_parameters, 3, .inherited = true},
    [65] =
        {field_attribute, 1},
    {field_attribute, 2, .inherited = true},
    {field_name, 0},
    {field_name, 2, .inherited = true},
    [69] =
        {field_attribute, 0, .inherited = true},
    {field_attribute, 1, .inherited = true},
    {field_name, 0, .inherited = true},
    {field_name, 1, .inherited = true},
    [73] =
        {field_attribute, 0, .inherited = true},
    {field_name, 0, .inherited = true},
    {field_value, 2, .inherited = true},
    [76] =
        {field_name, 0},
    {field_value, 2},
    [78] =
        {field_value, 0},
    {field_value, 1, .inherited = true},
    [80] =
        {field_body, 3},
    {field_condition, 1},
    [82] =
        {field_condition, 1},
    {field_consequence, 3},
    [84] =
        {field_alternative, 3},
    {field_condition, 1},
    [86] =
        {field_alternative, 3, .inherited = true},
    {field_condition, 1},
    [88] =
        {field_alternative, 0, .inherited = true},
    {field_alternative, 1, .inherited = true},
    [90] =
        {field_body, 3},
    {field_clause, 1},
    [92] =
        {field_attribute, 2},
    {field_name, 1},
    [94] =
        {field_value, 1},
    [95] =
        {field_value, 0, .inherited = true},
    {field_value, 1, .inherited = true},
    [97] =
        {field_alternative, 4},
    {field_condition, 1},
    {field_consequence, 3},
    [100] =
        {field_alternative, 4, .inherited = true},
    {field_condition, 1},
    {field_consequence, 3},
    [103] =
        {field_alternative, 3, .inherited = true},
    {field_alternative, 4},
    {field_condition, 1},
    [106] =
        {field_end, 4},
    {field_name, 0},
    {field_start, 2},
    [109] =
        {field_name, 1},
    {field_value, 4},
    [111] =
        {field_alternative, 4, .inherited = true},
    {field_alternative, 5},
    {field_condition, 1},
    {field_consequence, 3},
    [115] =
        {field_end, 4},
    {field_name, 0},
    {field_start, 2},
    {field_step, 6},
};

static const TSSymbol ts_alias_sequences[PRODUCTION_ID_COUNT][MAX_ALIAS_SEQUENCE_LENGTH] = {
    [0] = {0},
    [23] = {
        [1] = sym__block_string_content,
    },
};

static const uint16_t ts_non_terminal_alias_map[] = {
    aux_sym__doublequote_string_content,
    2,
    aux_sym__doublequote_string_content,
    sym__block_string_content,
    aux_sym__singlequote_string_content,
    2,
    aux_sym__singlequote_string_content,
    sym__block_string_content,
    0,
};

static const TSStateId ts_primary_state_ids[STATE_COUNT] = {
    [0] = 0,
    [1] = 1,
    [2] = 2,
    [3] = 2,
    [4] = 4,
    [5] = 5,
    [6] = 6,
    [7] = 7,
    [8] = 7,
    [9] = 9,
    [10] = 10,
    [11] = 11,
    [12] = 12,
    [13] = 9,
    [14] = 11,
    [15] = 10,
    [16] = 16,
    [17] = 12,
    [18] = 12,
    [19] = 19,
    [20] = 20,
    [21] = 21,
    [22] = 6,
    [23] = 23,
    [24] = 24,
    [25] = 25,
    [26] = 26,
    [27] = 27,
    [28] = 28,
    [29] = 29,
    [30] = 30,
    [31] = 31,
    [32] = 32,
    [33] = 33,
    [34] = 34,
    [35] = 35,
    [36] = 36,
    [37] = 37,
    [38] = 38,
    [39] = 39,
    [40] = 40,
    [41] = 23,
    [42] = 24,
    [43] = 43,
    [44] = 25,
    [45] = 26,
    [46] = 46,
    [47] = 47,
    [48] = 48,
    [49] = 49,
    [50] = 50,
    [51] = 51,
    [52] = 52,
    [53] = 53,
    [54] = 29,
    [55] = 28,
    [56] = 56,
    [57] = 33,
    [58] = 58,
    [59] = 39,
    [60] = 38,
    [61] = 30,
    [62] = 40,
    [63] = 63,
    [64] = 64,
    [65] = 35,
    [66] = 66,
    [67] = 67,
    [68] = 27,
    [69] = 36,
    [70] = 34,
    [71] = 71,
    [72] = 72,
    [73] = 32,
    [74] = 37,
    [75] = 31,
    [76] = 76,
    [77] = 77,
    [78] = 78,
    [79] = 79,
    [80] = 43,
    [81] = 66,
    [82] = 64,
    [83] = 50,
    [84] = 51,
    [85] = 52,
    [86] = 76,
    [87] = 46,
    [88] = 48,
    [89] = 53,
    [90] = 58,
    [91] = 72,
    [92] = 63,
    [93] = 49,
    [94] = 56,
    [95] = 67,
    [96] = 47,
    [97] = 71,
    [98] = 78,
    [99] = 77,
    [100] = 79,
    [101] = 37,
    [102] = 35,
    [103] = 27,
    [104] = 31,
    [105] = 40,
    [106] = 30,
    [107] = 32,
    [108] = 34,
    [109] = 25,
    [110] = 26,
    [111] = 24,
    [112] = 39,
    [113] = 36,
    [114] = 33,
    [115] = 38,
    [116] = 28,
    [117] = 29,
    [118] = 118,
    [119] = 119,
    [120] = 120,
    [121] = 121,
    [122] = 121,
    [123] = 121,
    [124] = 121,
    [125] = 125,
    [126] = 118,
    [127] = 127,
    [128] = 128,
    [129] = 129,
    [130] = 130,
    [131] = 131,
    [132] = 53,
    [133] = 133,
    [134] = 50,
    [135] = 47,
    [136] = 72,
    [137] = 29,
    [138] = 28,
    [139] = 40,
    [140] = 67,
    [141] = 52,
    [142] = 66,
    [143] = 71,
    [144] = 38,
    [145] = 58,
    [146] = 51,
    [147] = 119,
    [148] = 56,
    [149] = 63,
    [150] = 150,
    [151] = 64,
    [152] = 48,
    [153] = 49,
    [154] = 37,
    [155] = 31,
    [156] = 32,
    [157] = 46,
    [158] = 158,
    [159] = 159,
    [160] = 160,
    [161] = 161,
    [162] = 162,
    [163] = 163,
    [164] = 163,
    [165] = 165,
    [166] = 166,
    [167] = 167,
    [168] = 167,
    [169] = 166,
    [170] = 120,
    [171] = 166,
    [172] = 172,
    [173] = 173,
    [174] = 174,
    [175] = 175,
    [176] = 176,
    [177] = 177,
    [178] = 176,
    [179] = 177,
    [180] = 175,
    [181] = 174,
    [182] = 182,
    [183] = 177,
    [184] = 184,
    [185] = 185,
    [186] = 186,
    [187] = 187,
    [188] = 188,
    [189] = 189,
    [190] = 190,
    [191] = 191,
    [192] = 192,
    [193] = 193,
    [194] = 194,
    [195] = 195,
    [196] = 184,
    [197] = 197,
    [198] = 198,
    [199] = 199,
    [200] = 129,
    [201] = 128,
    [202] = 202,
    [203] = 203,
    [204] = 204,
    [205] = 205,
    [206] = 206,
    [207] = 207,
    [208] = 173,
    [209] = 209,
    [210] = 210,
    [211] = 195,
    [212] = 212,
    [213] = 194,
    [214] = 214,
    [215] = 215,
    [216] = 193,
    [217] = 217,
    [218] = 192,
    [219] = 172,
    [220] = 191,
    [221] = 190,
    [222] = 189,
    [223] = 223,
    [224] = 188,
    [225] = 187,
    [226] = 186,
    [227] = 176,
    [228] = 228,
    [229] = 229,
    [230] = 230,
    [231] = 231,
    [232] = 232,
    [233] = 232,
    [234] = 234,
    [235] = 235,
    [236] = 236,
    [237] = 237,
    [238] = 186,
    [239] = 187,
    [240] = 188,
    [241] = 189,
    [242] = 190,
    [243] = 191,
    [244] = 244,
    [245] = 172,
    [246] = 192,
    [247] = 247,
    [248] = 248,
    [249] = 193,
    [250] = 184,
    [251] = 194,
    [252] = 252,
    [253] = 253,
    [254] = 254,
    [255] = 255,
    [256] = 256,
    [257] = 125,
    [258] = 131,
    [259] = 195,
    [260] = 260,
    [261] = 185,
    [262] = 262,
    [263] = 263,
    [264] = 160,
    [265] = 161,
    [266] = 266,
    [267] = 150,
    [268] = 159,
    [269] = 269,
    [270] = 162,
    [271] = 228,
    [272] = 214,
    [273] = 262,
    [274] = 274,
    [275] = 217,
    [276] = 212,
    [277] = 203,
    [278] = 210,
    [279] = 197,
    [280] = 205,
    [281] = 247,
    [282] = 215,
    [283] = 198,
    [284] = 256,
    [285] = 285,
    [286] = 255,
    [287] = 254,
    [288] = 199,
    [289] = 202,
    [290] = 260,
    [291] = 231,
    [292] = 206,
    [293] = 248,
    [294] = 235,
    [295] = 236,
    [296] = 253,
    [297] = 204,
    [298] = 252,
    [299] = 207,
    [300] = 263,
    [301] = 301,
    [302] = 301,
    [303] = 303,
    [304] = 304,
    [305] = 305,
    [306] = 301,
    [307] = 307,
    [308] = 308,
    [309] = 308,
    [310] = 310,
    [311] = 308,
    [312] = 312,
    [313] = 313,
    [314] = 314,
    [315] = 315,
    [316] = 315,
    [317] = 310,
    [318] = 318,
    [319] = 319,
    [320] = 312,
    [321] = 312,
    [322] = 322,
    [323] = 323,
    [324] = 323,
    [325] = 323,
    [326] = 326,
    [327] = 327,
    [328] = 326,
    [329] = 326,
    [330] = 330,
    [331] = 331,
    [332] = 332,
    [333] = 333,
    [334] = 334,
    [335] = 333,
    [336] = 336,
    [337] = 337,
    [338] = 338,
    [339] = 339,
    [340] = 339,
    [341] = 337,
    [342] = 336,
    [343] = 343,
    [344] = 344,
    [345] = 345,
    [346] = 346,
    [347] = 347,
    [348] = 348,
    [349] = 349,
    [350] = 350,
    [351] = 351,
    [352] = 352,
    [353] = 353,
    [354] = 354,
    [355] = 353,
    [356] = 356,
    [357] = 357,
    [358] = 358,
    [359] = 351,
    [360] = 360,
    [361] = 353,
    [362] = 356,
    [363] = 358,
    [364] = 364,
    [365] = 351,
    [366] = 366,
    [367] = 367,
    [368] = 368,
    [369] = 356,
    [370] = 368,
    [371] = 351,
    [372] = 356,
    [373] = 358,
    [374] = 353,
    [375] = 358,
    [376] = 352,
    [377] = 377,
    [378] = 378,
    [379] = 379,
    [380] = 380,
    [381] = 381,
    [382] = 382,
    [383] = 383,
    [384] = 384,
    [385] = 385,
    [386] = 386,
    [387] = 387,
    [388] = 388,
    [389] = 389,
    [390] = 379,
    [391] = 391,
    [392] = 392,
    [393] = 386,
    [394] = 394,
    [395] = 395,
    [396] = 385,
    [397] = 391,
    [398] = 386,
    [399] = 391,
    [400] = 400,
    [401] = 401,
    [402] = 402,
    [403] = 403,
    [404] = 404,
    [405] = 405,
    [406] = 406,
    [407] = 407,
    [408] = 408,
    [409] = 409,
    [410] = 410,
    [411] = 411,
    [412] = 412,
    [413] = 413,
    [414] = 414,
    [415] = 415,
    [416] = 416,
    [417] = 417,
    [418] = 406,
    [419] = 419,
    [420] = 419,
    [421] = 421,
    [422] = 408,
    [423] = 423,
    [424] = 424,
    [425] = 425,
    [426] = 424,
    [427] = 421,
    [428] = 428,
    [429] = 428,
    [430] = 430,
    [431] = 431,
    [432] = 432,
    [433] = 416,
    [434] = 410,
    [435] = 435,
    [436] = 412,
    [437] = 435,
    [438] = 417,
    [439] = 411,
    [440] = 440,
    [441] = 441,
    [442] = 442,
    [443] = 414,
    [444] = 444,
    [445] = 445,
    [446] = 431,
    [447] = 447,
    [448] = 448,
    [449] = 406,
    [450] = 419,
    [451] = 451,
    [452] = 424,
    [453] = 453,
    [454] = 454,
    [455] = 455,
    [456] = 456,
    [457] = 457,
    [458] = 458,
    [459] = 414,
    [460] = 460,
    [461] = 414,
    [462] = 406,
    [463] = 451,
    [464] = 440,
    [465] = 465,
    [466] = 466,
    [467] = 432,
    [468] = 447,
    [469] = 455,
    [470] = 470,
    [471] = 466,
    [472] = 451,
    [473] = 473,
    [474] = 415,
    [475] = 451,
    [476] = 425,
    [477] = 477,
    [478] = 478,
};

static inline bool sym_escape_sequence_character_set_1(int32_t c) {
    return (c < 'a' ? (c < '\'' ? (c < '"' ? c == '\n' : c <= '"') : (c <= '\'' || c == '\\')) : (c <= 'b' || (c < 'r' ? (c < 'n' ? c == 'f' : c <= 'n') : (c <= 'r' || (c >= 't' && c <= 'v')))));
}

static bool ts_lex(TSLexer *lexer, TSStateId state) {
    START_LEXER();
    eof = lexer->eof(lexer);
    switch (state) {
        case 0:
            if (eof) {
                ADVANCE(20);
            }
            if (lookahead == '"') {
                ADVANCE(40);
            }
            if (lookahead == '#') {
                ADVANCE(78);
            }
            if (lookahead == '%') {
                ADVANCE(73);
            }
            if (lookahead == '&') {
                ADVANCE(65);
            }
            if (lookahead == '\'') {
                ADVANCE(41);
            }
            if (lookahead == '(') {
                ADVANCE(53);
            }
            if (lookahead == ')') {
                ADVANCE(54);
            }
            if (lookahead == '*') {
                ADVANCE(70);
            }
            if (lookahead == '+') {
                ADVANCE(68);
            }
            if (lookahead == ',') {
                ADVANCE(24);
            }
            if (lookahead == '-') {
                ADVANCE(69);
            }
            if (lookahead == '.') {
                ADVANCE(26);
            }
            if (lookahead == '/') {
                ADVANCE(71);
            }
            if (lookahead == '0') {
                ADVANCE(34);
            }
            if (lookahead == ':') {
                ADVANCE(29);
            }
            if (lookahead == ';') {
                ADVANCE(22);
            }
            if (lookahead == '<') {
                ADVANCE(30);
            }
            if (lookahead == '=') {
                ADVANCE(23);
            }
            if (lookahead == '>') {
                ADVANCE(32);
            }
            if (lookahead == '[') {
                ADVANCE(55);
            }
            if (lookahead == '\\') {
                ADVANCE(8);
            }
            if (lookahead == ']') {
                ADVANCE(56);
            }
            if (lookahead == '^') {
                ADVANCE(76);
            }
            if (lookahead == '{') {
                ADVANCE(57);
            }
            if (lookahead == '|') {
                ADVANCE(63);
            }
            if (lookahead == '}') {
                ADVANCE(58);
            }
            if (lookahead == '~') {
                ADVANCE(64);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                SKIP(17)
            }
            if (('1' <= lookahead && lookahead <= '9')) {
                ADVANCE(35);
            }
            if (lookahead != 0 &&
                lookahead > 31 &&
                (lookahead < 127 || 159 < lookahead)) {
                ADVANCE(79);
            }
            END_STATE();
        case 1:
            if (lookahead == '"') {
                ADVANCE(40);
            }
            if (lookahead == '-') {
                ADVANCE(43);
            }
            if (lookahead == '\\') {
                ADVANCE(8);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                ADVANCE(42);
            }
            if (lookahead != 0) {
                ADVANCE(44);
            }
            END_STATE();
        case 2:
            if (lookahead == '\'') {
                ADVANCE(41);
            }
            if (lookahead == '-') {
                ADVANCE(46);
            }
            if (lookahead == '\\') {
                ADVANCE(8);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                ADVANCE(45);
            }
            if (lookahead != 0) {
                ADVANCE(47);
            }
            END_STATE();
        case 3:
            if (lookahead == '-') {
                ADVANCE(80);
            }
            END_STATE();
        case 4:
            if (lookahead == '-') {
                ADVANCE(3);
            }
            if (lookahead == '>') {
                ADVANCE(31);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                SKIP(4)
            }
            END_STATE();
        case 5:
            if (lookahead == '.') {
                ADVANCE(14);
            }
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(36);
            }
            END_STATE();
        case 6:
            if (lookahead == 'L') {
                ADVANCE(33);
            }
            END_STATE();
        case 7:
            if (lookahead == 'L') {
                ADVANCE(6);
            }
            END_STATE();
        case 8:
            if (lookahead == 'u') {
                ADVANCE(9);
            }
            if (lookahead == 'x') {
                ADVANCE(16);
            }
            if (lookahead == 'z') {
                ADVANCE(49);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(51);
            }
            if (sym_escape_sequence_character_set_1(lookahead)) {
                ADVANCE(48);
            }
            END_STATE();
        case 9:
            if (lookahead == '{') {
                ADVANCE(15);
            }
            END_STATE();
        case 10:
            if (lookahead == '}') {
                ADVANCE(48);
            }
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(10);
            }
            END_STATE();
        case 11:
            if (lookahead == '+' ||
                lookahead == '-') {
                ADVANCE(12);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(39);
            }
            END_STATE();
        case 12:
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(39);
            }
            END_STATE();
        case 13:
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(48);
            }
            END_STATE();
        case 14:
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(38);
            }
            END_STATE();
        case 15:
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(10);
            }
            END_STATE();
        case 16:
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(13);
            }
            END_STATE();
        case 17:
            if (eof) {
                ADVANCE(20);
            }
            if (lookahead == '"') {
                ADVANCE(40);
            }
            if (lookahead == '#') {
                ADVANCE(78);
            }
            if (lookahead == '%') {
                ADVANCE(73);
            }
            if (lookahead == '&') {
                ADVANCE(65);
            }
            if (lookahead == '\'') {
                ADVANCE(41);
            }
            if (lookahead == '(') {
                ADVANCE(53);
            }
            if (lookahead == ')') {
                ADVANCE(54);
            }
            if (lookahead == '*') {
                ADVANCE(70);
            }
            if (lookahead == '+') {
                ADVANCE(68);
            }
            if (lookahead == ',') {
                ADVANCE(24);
            }
            if (lookahead == '-') {
                ADVANCE(69);
            }
            if (lookahead == '.') {
                ADVANCE(26);
            }
            if (lookahead == '/') {
                ADVANCE(71);
            }
            if (lookahead == '0') {
                ADVANCE(34);
            }
            if (lookahead == ':') {
                ADVANCE(29);
            }
            if (lookahead == ';') {
                ADVANCE(22);
            }
            if (lookahead == '<') {
                ADVANCE(30);
            }
            if (lookahead == '=') {
                ADVANCE(23);
            }
            if (lookahead == '>') {
                ADVANCE(32);
            }
            if (lookahead == '[') {
                ADVANCE(55);
            }
            if (lookahead == ']') {
                ADVANCE(56);
            }
            if (lookahead == '^') {
                ADVANCE(76);
            }
            if (lookahead == '{') {
                ADVANCE(57);
            }
            if (lookahead == '|') {
                ADVANCE(63);
            }
            if (lookahead == '}') {
                ADVANCE(58);
            }
            if (lookahead == '~') {
                ADVANCE(64);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                SKIP(17)
            }
            if (('1' <= lookahead && lookahead <= '9')) {
                ADVANCE(35);
            }
            if (lookahead != 0 &&
                lookahead > 31 &&
                lookahead != '\\' &&
                (lookahead < 127 || 159 < lookahead)) {
                ADVANCE(79);
            }
            END_STATE();
        case 18:
            if (eof) {
                ADVANCE(20);
            }
            if (lookahead == '"') {
                ADVANCE(40);
            }
            if (lookahead == '#') {
                ADVANCE(77);
            }
            if (lookahead == '%') {
                ADVANCE(73);
            }
            if (lookahead == '&') {
                ADVANCE(65);
            }
            if (lookahead == '\'') {
                ADVANCE(41);
            }
            if (lookahead == '(') {
                ADVANCE(53);
            }
            if (lookahead == ')') {
                ADVANCE(54);
            }
            if (lookahead == '*') {
                ADVANCE(70);
            }
            if (lookahead == '+') {
                ADVANCE(68);
            }
            if (lookahead == ',') {
                ADVANCE(24);
            }
            if (lookahead == '-') {
                ADVANCE(69);
            }
            if (lookahead == '.') {
                ADVANCE(26);
            }
            if (lookahead == '/') {
                ADVANCE(71);
            }
            if (lookahead == '0') {
                ADVANCE(34);
            }
            if (lookahead == ':') {
                ADVANCE(29);
            }
            if (lookahead == ';') {
                ADVANCE(22);
            }
            if (lookahead == '<') {
                ADVANCE(30);
            }
            if (lookahead == '=') {
                ADVANCE(23);
            }
            if (lookahead == '>') {
                ADVANCE(32);
            }
            if (lookahead == '[') {
                ADVANCE(55);
            }
            if (lookahead == '^') {
                ADVANCE(76);
            }
            if (lookahead == '{') {
                ADVANCE(57);
            }
            if (lookahead == '|') {
                ADVANCE(63);
            }
            if (lookahead == '}') {
                ADVANCE(58);
            }
            if (lookahead == '~') {
                ADVANCE(64);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                SKIP(18)
            }
            if (('1' <= lookahead && lookahead <= '9')) {
                ADVANCE(35);
            }
            if (lookahead != 0 &&
                lookahead > 31 &&
                lookahead != '\\' &&
                lookahead != ']' &&
                (lookahead < 127 || 159 < lookahead)) {
                ADVANCE(79);
            }
            END_STATE();
        case 19:
            if (eof) {
                ADVANCE(20);
            }
            if (lookahead == '"') {
                ADVANCE(40);
            }
            if (lookahead == '%') {
                ADVANCE(73);
            }
            if (lookahead == '&') {
                ADVANCE(65);
            }
            if (lookahead == '\'') {
                ADVANCE(41);
            }
            if (lookahead == '(') {
                ADVANCE(53);
            }
            if (lookahead == ')') {
                ADVANCE(54);
            }
            if (lookahead == '*') {
                ADVANCE(70);
            }
            if (lookahead == '+') {
                ADVANCE(68);
            }
            if (lookahead == ',') {
                ADVANCE(24);
            }
            if (lookahead == '-') {
                ADVANCE(69);
            }
            if (lookahead == '.') {
                ADVANCE(27);
            }
            if (lookahead == '/') {
                ADVANCE(71);
            }
            if (lookahead == ':') {
                ADVANCE(28);
            }
            if (lookahead == ';') {
                ADVANCE(22);
            }
            if (lookahead == '<') {
                ADVANCE(30);
            }
            if (lookahead == '=') {
                ADVANCE(23);
            }
            if (lookahead == '>') {
                ADVANCE(32);
            }
            if (lookahead == '[') {
                ADVANCE(55);
            }
            if (lookahead == ']') {
                ADVANCE(56);
            }
            if (lookahead == '^') {
                ADVANCE(76);
            }
            if (lookahead == '{') {
                ADVANCE(57);
            }
            if (lookahead == '|') {
                ADVANCE(63);
            }
            if (lookahead == '}') {
                ADVANCE(58);
            }
            if (lookahead == '~') {
                ADVANCE(64);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                SKIP(19)
            }
            if (lookahead != 0 &&
                lookahead > 31 &&
                lookahead != '#' &&
                (lookahead < '0' || '9' < lookahead) &&
                lookahead != '\\' &&
                (lookahead < 127 || 159 < lookahead)) {
                ADVANCE(79);
            }
            END_STATE();
        case 20:
            ACCEPT_TOKEN(ts_builtin_sym_end);
            END_STATE();
        case 21:
            ACCEPT_TOKEN(sym_hash_bang_line);
            if (lookahead != 0 &&
                lookahead != '\n') {
                ADVANCE(21);
            }
            END_STATE();
        case 22:
            ACCEPT_TOKEN(anon_sym_SEMI);
            END_STATE();
        case 23:
            ACCEPT_TOKEN(anon_sym_EQ);
            if (lookahead == '=') {
                ADVANCE(60);
            }
            END_STATE();
        case 24:
            ACCEPT_TOKEN(anon_sym_COMMA);
            END_STATE();
        case 25:
            ACCEPT_TOKEN(anon_sym_COLON_COLON);
            END_STATE();
        case 26:
            ACCEPT_TOKEN(anon_sym_DOT);
            if (lookahead == '.') {
                ADVANCE(75);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(37);
            }
            END_STATE();
        case 27:
            ACCEPT_TOKEN(anon_sym_DOT);
            if (lookahead == '.') {
                ADVANCE(74);
            }
            END_STATE();
        case 28:
            ACCEPT_TOKEN(anon_sym_COLON);
            END_STATE();
        case 29:
            ACCEPT_TOKEN(anon_sym_COLON);
            if (lookahead == ':') {
                ADVANCE(25);
            }
            END_STATE();
        case 30:
            ACCEPT_TOKEN(anon_sym_LT);
            if (lookahead == '<') {
                ADVANCE(66);
            }
            if (lookahead == '=') {
                ADVANCE(59);
            }
            END_STATE();
        case 31:
            ACCEPT_TOKEN(anon_sym_GT);
            END_STATE();
        case 32:
            ACCEPT_TOKEN(anon_sym_GT);
            if (lookahead == '=') {
                ADVANCE(62);
            }
            if (lookahead == '>') {
                ADVANCE(67);
            }
            END_STATE();
        case 33:
            ACCEPT_TOKEN(sym_number);
            END_STATE();
        case 34:
            ACCEPT_TOKEN(sym_number);
            if (lookahead == '.') {
                ADVANCE(37);
            }
            if (lookahead == 'E' ||
                lookahead == 'e') {
                ADVANCE(11);
            }
            if (lookahead == 'I' ||
                lookahead == 'i') {
                ADVANCE(33);
            }
            if (lookahead == 'L') {
                ADVANCE(6);
            }
            if (lookahead == 'U') {
                ADVANCE(7);
            }
            if (lookahead == 'X' ||
                lookahead == 'x') {
                ADVANCE(5);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(35);
            }
            END_STATE();
        case 35:
            ACCEPT_TOKEN(sym_number);
            if (lookahead == '.') {
                ADVANCE(37);
            }
            if (lookahead == 'E' ||
                lookahead == 'e') {
                ADVANCE(11);
            }
            if (lookahead == 'I' ||
                lookahead == 'i') {
                ADVANCE(33);
            }
            if (lookahead == 'L') {
                ADVANCE(6);
            }
            if (lookahead == 'U') {
                ADVANCE(7);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(35);
            }
            END_STATE();
        case 36:
            ACCEPT_TOKEN(sym_number);
            if (lookahead == '.') {
                ADVANCE(38);
            }
            if (lookahead == 'I' ||
                lookahead == 'i') {
                ADVANCE(33);
            }
            if (lookahead == 'L') {
                ADVANCE(6);
            }
            if (lookahead == 'P' ||
                lookahead == 'p') {
                ADVANCE(11);
            }
            if (lookahead == 'U') {
                ADVANCE(7);
            }
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(36);
            }
            END_STATE();
        case 37:
            ACCEPT_TOKEN(sym_number);
            if (lookahead == 'E' ||
                lookahead == 'e') {
                ADVANCE(11);
            }
            if (lookahead == 'I' ||
                lookahead == 'i') {
                ADVANCE(33);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(37);
            }
            END_STATE();
        case 38:
            ACCEPT_TOKEN(sym_number);
            if (lookahead == 'I' ||
                lookahead == 'i') {
                ADVANCE(33);
            }
            if (lookahead == 'P' ||
                lookahead == 'p') {
                ADVANCE(11);
            }
            if (('0' <= lookahead && lookahead <= '9') ||
                ('A' <= lookahead && lookahead <= 'F') ||
                ('a' <= lookahead && lookahead <= 'f')) {
                ADVANCE(38);
            }
            END_STATE();
        case 39:
            ACCEPT_TOKEN(sym_number);
            if (lookahead == 'I' ||
                lookahead == 'i') {
                ADVANCE(33);
            }
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(39);
            }
            END_STATE();
        case 40:
            ACCEPT_TOKEN(anon_sym_DQUOTE);
            END_STATE();
        case 41:
            ACCEPT_TOKEN(anon_sym_SQUOTE);
            END_STATE();
        case 42:
            ACCEPT_TOKEN(aux_sym__doublequote_string_content_token1);
            if (lookahead == '-') {
                ADVANCE(43);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                ADVANCE(42);
            }
            if (lookahead != 0 &&
                lookahead != '"' &&
                lookahead != '\\') {
                ADVANCE(44);
            }
            END_STATE();
        case 43:
            ACCEPT_TOKEN(aux_sym__doublequote_string_content_token1);
            if (lookahead == '-') {
                ADVANCE(44);
            }
            if (lookahead != 0 &&
                lookahead != '"' &&
                lookahead != '\\') {
                ADVANCE(44);
            }
            END_STATE();
        case 44:
            ACCEPT_TOKEN(aux_sym__doublequote_string_content_token1);
            if (lookahead != 0 &&
                lookahead != '"' &&
                lookahead != '\\') {
                ADVANCE(44);
            }
            END_STATE();
        case 45:
            ACCEPT_TOKEN(aux_sym__singlequote_string_content_token1);
            if (lookahead == '-') {
                ADVANCE(46);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                ADVANCE(45);
            }
            if (lookahead != 0 &&
                lookahead != '\'' &&
                lookahead != '\\') {
                ADVANCE(47);
            }
            END_STATE();
        case 46:
            ACCEPT_TOKEN(aux_sym__singlequote_string_content_token1);
            if (lookahead == '-') {
                ADVANCE(47);
            }
            if (lookahead != 0 &&
                lookahead != '\'' &&
                lookahead != '\\') {
                ADVANCE(47);
            }
            END_STATE();
        case 47:
            ACCEPT_TOKEN(aux_sym__singlequote_string_content_token1);
            if (lookahead != 0 &&
                lookahead != '\'' &&
                lookahead != '\\') {
                ADVANCE(47);
            }
            END_STATE();
        case 48:
            ACCEPT_TOKEN(sym_escape_sequence);
            END_STATE();
        case 49:
            ACCEPT_TOKEN(sym_escape_sequence);
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                ADVANCE(49);
            }
            END_STATE();
        case 50:
            ACCEPT_TOKEN(sym_escape_sequence);
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(48);
            }
            END_STATE();
        case 51:
            ACCEPT_TOKEN(sym_escape_sequence);
            if (('0' <= lookahead && lookahead <= '9')) {
                ADVANCE(50);
            }
            END_STATE();
        case 52:
            ACCEPT_TOKEN(sym_vararg_expression);
            END_STATE();
        case 53:
            ACCEPT_TOKEN(anon_sym_LPAREN);
            END_STATE();
        case 54:
            ACCEPT_TOKEN(anon_sym_RPAREN);
            END_STATE();
        case 55:
            ACCEPT_TOKEN(anon_sym_LBRACK);
            END_STATE();
        case 56:
            ACCEPT_TOKEN(anon_sym_RBRACK);
            END_STATE();
        case 57:
            ACCEPT_TOKEN(anon_sym_LBRACE);
            END_STATE();
        case 58:
            ACCEPT_TOKEN(anon_sym_RBRACE);
            END_STATE();
        case 59:
            ACCEPT_TOKEN(anon_sym_LT_EQ);
            END_STATE();
        case 60:
            ACCEPT_TOKEN(anon_sym_EQ_EQ);
            END_STATE();
        case 61:
            ACCEPT_TOKEN(anon_sym_TILDE_EQ);
            END_STATE();
        case 62:
            ACCEPT_TOKEN(anon_sym_GT_EQ);
            END_STATE();
        case 63:
            ACCEPT_TOKEN(anon_sym_PIPE);
            END_STATE();
        case 64:
            ACCEPT_TOKEN(anon_sym_TILDE);
            if (lookahead == '=') {
                ADVANCE(61);
            }
            END_STATE();
        case 65:
            ACCEPT_TOKEN(anon_sym_AMP);
            END_STATE();
        case 66:
            ACCEPT_TOKEN(anon_sym_LT_LT);
            END_STATE();
        case 67:
            ACCEPT_TOKEN(anon_sym_GT_GT);
            END_STATE();
        case 68:
            ACCEPT_TOKEN(anon_sym_PLUS);
            END_STATE();
        case 69:
            ACCEPT_TOKEN(anon_sym_DASH);
            if (lookahead == '-') {
                ADVANCE(80);
            }
            END_STATE();
        case 70:
            ACCEPT_TOKEN(anon_sym_STAR);
            END_STATE();
        case 71:
            ACCEPT_TOKEN(anon_sym_SLASH);
            if (lookahead == '/') {
                ADVANCE(72);
            }
            END_STATE();
        case 72:
            ACCEPT_TOKEN(anon_sym_SLASH_SLASH);
            END_STATE();
        case 73:
            ACCEPT_TOKEN(anon_sym_PERCENT);
            END_STATE();
        case 74:
            ACCEPT_TOKEN(anon_sym_DOT_DOT);
            END_STATE();
        case 75:
            ACCEPT_TOKEN(anon_sym_DOT_DOT);
            if (lookahead == '.') {
                ADVANCE(52);
            }
            END_STATE();
        case 76:
            ACCEPT_TOKEN(anon_sym_CARET);
            END_STATE();
        case 77:
            ACCEPT_TOKEN(anon_sym_POUND);
            END_STATE();
        case 78:
            ACCEPT_TOKEN(anon_sym_POUND);
            if (lookahead != 0 &&
                lookahead != '\n') {
                ADVANCE(21);
            }
            END_STATE();
        case 79:
            ACCEPT_TOKEN(sym_identifier);
            if (lookahead != 0 &&
                lookahead > ' ' &&
                lookahead != '"' &&
                lookahead != '#' &&
                (lookahead < '%' || '/' < lookahead) &&
                (lookahead < ':' || '>' < lookahead) &&
                (lookahead < '[' || '^' < lookahead) &&
                (lookahead < '{' || 159 < lookahead)) {
                ADVANCE(79);
            }
            END_STATE();
        case 80:
            ACCEPT_TOKEN(anon_sym_DASH_DASH);
            END_STATE();
        case 81:
            ACCEPT_TOKEN(anon_sym_DASH_DASH);
            if (lookahead != 0 &&
                lookahead != '\n' &&
                lookahead != '\r') {
                ADVANCE(84);
            }
            END_STATE();
        case 82:
            ACCEPT_TOKEN(aux_sym_comment_token1);
            if (lookahead == '-') {
                ADVANCE(83);
            }
            if (lookahead == '\t' ||
                lookahead == ' ') {
                ADVANCE(82);
            }
            if (lookahead != 0 &&
                lookahead != '\n' &&
                lookahead != '\r') {
                ADVANCE(84);
            }
            END_STATE();
        case 83:
            ACCEPT_TOKEN(aux_sym_comment_token1);
            if (lookahead == '-') {
                ADVANCE(81);
            }
            if (lookahead != 0 &&
                lookahead != '\n' &&
                lookahead != '\r') {
                ADVANCE(84);
            }
            END_STATE();
        case 84:
            ACCEPT_TOKEN(aux_sym_comment_token1);
            if (lookahead != 0 &&
                lookahead != '\n' &&
                lookahead != '\r') {
                ADVANCE(84);
            }
            END_STATE();
        default:
            return false;
    }
}

static bool ts_lex_keywords(TSLexer *lexer, TSStateId state) {
    START_LEXER();
    eof = lexer->eof(lexer);
    switch (state) {
        case 0:
            if (lookahead == 'a') {
                ADVANCE(1);
            }
            if (lookahead == 'b') {
                ADVANCE(2);
            }
            if (lookahead == 'd') {
                ADVANCE(3);
            }
            if (lookahead == 'e') {
                ADVANCE(4);
            }
            if (lookahead == 'f') {
                ADVANCE(5);
            }
            if (lookahead == 'g') {
                ADVANCE(6);
            }
            if (lookahead == 'i') {
                ADVANCE(7);
            }
            if (lookahead == 'l') {
                ADVANCE(8);
            }
            if (lookahead == 'n') {
                ADVANCE(9);
            }
            if (lookahead == 'o') {
                ADVANCE(10);
            }
            if (lookahead == 'r') {
                ADVANCE(11);
            }
            if (lookahead == 't') {
                ADVANCE(12);
            }
            if (lookahead == 'u') {
                ADVANCE(13);
            }
            if (lookahead == 'w') {
                ADVANCE(14);
            }
            if (lookahead == '\t' ||
                lookahead == '\n' ||
                lookahead == '\r' ||
                lookahead == ' ') {
                SKIP(0)
            }
            END_STATE();
        case 1:
            if (lookahead == 'n') {
                ADVANCE(15);
            }
            END_STATE();
        case 2:
            if (lookahead == 'r') {
                ADVANCE(16);
            }
            END_STATE();
        case 3:
            if (lookahead == 'o') {
                ADVANCE(17);
            }
            END_STATE();
        case 4:
            if (lookahead == 'l') {
                ADVANCE(18);
            }
            if (lookahead == 'n') {
                ADVANCE(19);
            }
            END_STATE();
        case 5:
            if (lookahead == 'a') {
                ADVANCE(20);
            }
            if (lookahead == 'o') {
                ADVANCE(21);
            }
            if (lookahead == 'u') {
                ADVANCE(22);
            }
            END_STATE();
        case 6:
            if (lookahead == 'o') {
                ADVANCE(23);
            }
            END_STATE();
        case 7:
            if (lookahead == 'f') {
                ADVANCE(24);
            }
            if (lookahead == 'n') {
                ADVANCE(25);
            }
            END_STATE();
        case 8:
            if (lookahead == 'o') {
                ADVANCE(26);
            }
            END_STATE();
        case 9:
            if (lookahead == 'i') {
                ADVANCE(27);
            }
            if (lookahead == 'o') {
                ADVANCE(28);
            }
            END_STATE();
        case 10:
            if (lookahead == 'r') {
                ADVANCE(29);
            }
            END_STATE();
        case 11:
            if (lookahead == 'e') {
                ADVANCE(30);
            }
            END_STATE();
        case 12:
            if (lookahead == 'h') {
                ADVANCE(31);
            }
            if (lookahead == 'r') {
                ADVANCE(32);
            }
            END_STATE();
        case 13:
            if (lookahead == 'n') {
                ADVANCE(33);
            }
            END_STATE();
        case 14:
            if (lookahead == 'h') {
                ADVANCE(34);
            }
            END_STATE();
        case 15:
            if (lookahead == 'd') {
                ADVANCE(35);
            }
            END_STATE();
        case 16:
            if (lookahead == 'e') {
                ADVANCE(36);
            }
            END_STATE();
        case 17:
            ACCEPT_TOKEN(anon_sym_do);
            END_STATE();
        case 18:
            if (lookahead == 's') {
                ADVANCE(37);
            }
            END_STATE();
        case 19:
            if (lookahead == 'd') {
                ADVANCE(38);
            }
            END_STATE();
        case 20:
            if (lookahead == 'l') {
                ADVANCE(39);
            }
            END_STATE();
        case 21:
            if (lookahead == 'r') {
                ADVANCE(40);
            }
            END_STATE();
        case 22:
            if (lookahead == 'n') {
                ADVANCE(41);
            }
            END_STATE();
        case 23:
            if (lookahead == 't') {
                ADVANCE(42);
            }
            END_STATE();
        case 24:
            ACCEPT_TOKEN(anon_sym_if);
            END_STATE();
        case 25:
            ACCEPT_TOKEN(anon_sym_in);
            END_STATE();
        case 26:
            if (lookahead == 'c') {
                ADVANCE(43);
            }
            END_STATE();
        case 27:
            if (lookahead == 'l') {
                ADVANCE(44);
            }
            END_STATE();
        case 28:
            if (lookahead == 't') {
                ADVANCE(45);
            }
            END_STATE();
        case 29:
            ACCEPT_TOKEN(anon_sym_or);
            END_STATE();
        case 30:
            if (lookahead == 'p') {
                ADVANCE(46);
            }
            if (lookahead == 't') {
                ADVANCE(47);
            }
            END_STATE();
        case 31:
            if (lookahead == 'e') {
                ADVANCE(48);
            }
            END_STATE();
        case 32:
            if (lookahead == 'u') {
                ADVANCE(49);
            }
            END_STATE();
        case 33:
            if (lookahead == 't') {
                ADVANCE(50);
            }
            END_STATE();
        case 34:
            if (lookahead == 'i') {
                ADVANCE(51);
            }
            END_STATE();
        case 35:
            ACCEPT_TOKEN(anon_sym_and);
            END_STATE();
        case 36:
            if (lookahead == 'a') {
                ADVANCE(52);
            }
            END_STATE();
        case 37:
            if (lookahead == 'e') {
                ADVANCE(53);
            }
            END_STATE();
        case 38:
            ACCEPT_TOKEN(anon_sym_end);
            END_STATE();
        case 39:
            if (lookahead == 's') {
                ADVANCE(54);
            }
            END_STATE();
        case 40:
            ACCEPT_TOKEN(anon_sym_for);
            END_STATE();
        case 41:
            if (lookahead == 'c') {
                ADVANCE(55);
            }
            END_STATE();
        case 42:
            if (lookahead == 'o') {
                ADVANCE(56);
            }
            END_STATE();
        case 43:
            if (lookahead == 'a') {
                ADVANCE(57);
            }
            END_STATE();
        case 44:
            ACCEPT_TOKEN(sym_nil);
            END_STATE();
        case 45:
            ACCEPT_TOKEN(anon_sym_not);
            END_STATE();
        case 46:
            if (lookahead == 'e') {
                ADVANCE(58);
            }
            END_STATE();
        case 47:
            if (lookahead == 'u') {
                ADVANCE(59);
            }
            END_STATE();
        case 48:
            if (lookahead == 'n') {
                ADVANCE(60);
            }
            END_STATE();
        case 49:
            if (lookahead == 'e') {
                ADVANCE(61);
            }
            END_STATE();
        case 50:
            if (lookahead == 'i') {
                ADVANCE(62);
            }
            END_STATE();
        case 51:
            if (lookahead == 'l') {
                ADVANCE(63);
            }
            END_STATE();
        case 52:
            if (lookahead == 'k') {
                ADVANCE(64);
            }
            END_STATE();
        case 53:
            ACCEPT_TOKEN(anon_sym_else);
            if (lookahead == 'i') {
                ADVANCE(65);
            }
            END_STATE();
        case 54:
            if (lookahead == 'e') {
                ADVANCE(66);
            }
            END_STATE();
        case 55:
            if (lookahead == 't') {
                ADVANCE(67);
            }
            END_STATE();
        case 56:
            ACCEPT_TOKEN(anon_sym_goto);
            END_STATE();
        case 57:
            if (lookahead == 'l') {
                ADVANCE(68);
            }
            END_STATE();
        case 58:
            if (lookahead == 'a') {
                ADVANCE(69);
            }
            END_STATE();
        case 59:
            if (lookahead == 'r') {
                ADVANCE(70);
            }
            END_STATE();
        case 60:
            ACCEPT_TOKEN(anon_sym_then);
            END_STATE();
        case 61:
            ACCEPT_TOKEN(sym_true);
            END_STATE();
        case 62:
            if (lookahead == 'l') {
                ADVANCE(71);
            }
            END_STATE();
        case 63:
            if (lookahead == 'e') {
                ADVANCE(72);
            }
            END_STATE();
        case 64:
            ACCEPT_TOKEN(sym_break_statement);
            END_STATE();
        case 65:
            if (lookahead == 'f') {
                ADVANCE(73);
            }
            END_STATE();
        case 66:
            ACCEPT_TOKEN(sym_false);
            END_STATE();
        case 67:
            if (lookahead == 'i') {
                ADVANCE(74);
            }
            END_STATE();
        case 68:
            ACCEPT_TOKEN(anon_sym_local);
            END_STATE();
        case 69:
            if (lookahead == 't') {
                ADVANCE(75);
            }
            END_STATE();
        case 70:
            if (lookahead == 'n') {
                ADVANCE(76);
            }
            END_STATE();
        case 71:
            ACCEPT_TOKEN(anon_sym_until);
            END_STATE();
        case 72:
            ACCEPT_TOKEN(anon_sym_while);
            END_STATE();
        case 73:
            ACCEPT_TOKEN(anon_sym_elseif);
            END_STATE();
        case 74:
            if (lookahead == 'o') {
                ADVANCE(77);
            }
            END_STATE();
        case 75:
            ACCEPT_TOKEN(anon_sym_repeat);
            END_STATE();
        case 76:
            ACCEPT_TOKEN(anon_sym_return);
            END_STATE();
        case 77:
            if (lookahead == 'n') {
                ADVANCE(78);
            }
            END_STATE();
        case 78:
            ACCEPT_TOKEN(anon_sym_function);
            END_STATE();
        default:
            return false;
    }
}

static const TSLexMode ts_lex_modes[STATE_COUNT] = {
    [0] = {.lex_state = 0, .external_lex_state = 1},
    [1] = {.lex_state = 0, .external_lex_state = 2},
    [2] = {.lex_state = 18, .external_lex_state = 2},
    [3] = {.lex_state = 18, .external_lex_state = 2},
    [4] = {.lex_state = 18, .external_lex_state = 2},
    [5] = {.lex_state = 18, .external_lex_state = 2},
    [6] = {.lex_state = 18, .external_lex_state = 2},
    [7] = {.lex_state = 18, .external_lex_state = 2},
    [8] = {.lex_state = 18, .external_lex_state = 2},
    [9] = {.lex_state = 18, .external_lex_state = 2},
    [10] = {.lex_state = 18, .external_lex_state = 2},
    [11] = {.lex_state = 18, .external_lex_state = 2},
    [12] = {.lex_state = 18, .external_lex_state = 2},
    [13] = {.lex_state = 18, .external_lex_state = 2},
    [14] = {.lex_state = 18, .external_lex_state = 2},
    [15] = {.lex_state = 18, .external_lex_state = 2},
    [16] = {.lex_state = 18, .external_lex_state = 2},
    [17] = {.lex_state = 18, .external_lex_state = 2},
    [18] = {.lex_state = 18, .external_lex_state = 2},
    [19] = {.lex_state = 18, .external_lex_state = 2},
    [20] = {.lex_state = 18, .external_lex_state = 2},
    [21] = {.lex_state = 18, .external_lex_state = 2},
    [22] = {.lex_state = 18, .external_lex_state = 2},
    [23] = {.lex_state = 18, .external_lex_state = 2},
    [24] = {.lex_state = 18, .external_lex_state = 2},
    [25] = {.lex_state = 18, .external_lex_state = 2},
    [26] = {.lex_state = 18, .external_lex_state = 2},
    [27] = {.lex_state = 18, .external_lex_state = 2},
    [28] = {.lex_state = 18, .external_lex_state = 2},
    [29] = {.lex_state = 18, .external_lex_state = 2},
    [30] = {.lex_state = 18, .external_lex_state = 2},
    [31] = {.lex_state = 18, .external_lex_state = 2},
    [32] = {.lex_state = 18, .external_lex_state = 2},
    [33] = {.lex_state = 18, .external_lex_state = 2},
    [34] = {.lex_state = 18, .external_lex_state = 2},
    [35] = {.lex_state = 18, .external_lex_state = 2},
    [36] = {.lex_state = 18, .external_lex_state = 2},
    [37] = {.lex_state = 18, .external_lex_state = 2},
    [38] = {.lex_state = 18, .external_lex_state = 2},
    [39] = {.lex_state = 18, .external_lex_state = 2},
    [40] = {.lex_state = 18, .external_lex_state = 2},
    [41] = {.lex_state = 18, .external_lex_state = 2},
    [42] = {.lex_state = 18, .external_lex_state = 2},
    [43] = {.lex_state = 18, .external_lex_state = 2},
    [44] = {.lex_state = 18, .external_lex_state = 2},
    [45] = {.lex_state = 18, .external_lex_state = 2},
    [46] = {.lex_state = 18, .external_lex_state = 2},
    [47] = {.lex_state = 18, .external_lex_state = 2},
    [48] = {.lex_state = 18, .external_lex_state = 2},
    [49] = {.lex_state = 18, .external_lex_state = 2},
    [50] = {.lex_state = 18, .external_lex_state = 2},
    [51] = {.lex_state = 18, .external_lex_state = 2},
    [52] = {.lex_state = 18, .external_lex_state = 2},
    [53] = {.lex_state = 18, .external_lex_state = 2},
    [54] = {.lex_state = 18, .external_lex_state = 2},
    [55] = {.lex_state = 18, .external_lex_state = 2},
    [56] = {.lex_state = 18, .external_lex_state = 2},
    [57] = {.lex_state = 18, .external_lex_state = 2},
    [58] = {.lex_state = 18, .external_lex_state = 2},
    [59] = {.lex_state = 18, .external_lex_state = 2},
    [60] = {.lex_state = 18, .external_lex_state = 2},
    [61] = {.lex_state = 18, .external_lex_state = 2},
    [62] = {.lex_state = 18, .external_lex_state = 2},
    [63] = {.lex_state = 18, .external_lex_state = 2},
    [64] = {.lex_state = 18, .external_lex_state = 2},
    [65] = {.lex_state = 18, .external_lex_state = 2},
    [66] = {.lex_state = 18, .external_lex_state = 2},
    [67] = {.lex_state = 18, .external_lex_state = 2},
    [68] = {.lex_state = 18, .external_lex_state = 2},
    [69] = {.lex_state = 18, .external_lex_state = 2},
    [70] = {.lex_state = 18, .external_lex_state = 2},
    [71] = {.lex_state = 18, .external_lex_state = 2},
    [72] = {.lex_state = 18, .external_lex_state = 2},
    [73] = {.lex_state = 18, .external_lex_state = 2},
    [74] = {.lex_state = 18, .external_lex_state = 2},
    [75] = {.lex_state = 18, .external_lex_state = 2},
    [76] = {.lex_state = 18, .external_lex_state = 2},
    [77] = {.lex_state = 18, .external_lex_state = 2},
    [78] = {.lex_state = 18, .external_lex_state = 2},
    [79] = {.lex_state = 18, .external_lex_state = 2},
    [80] = {.lex_state = 18, .external_lex_state = 2},
    [81] = {.lex_state = 18, .external_lex_state = 2},
    [82] = {.lex_state = 18, .external_lex_state = 2},
    [83] = {.lex_state = 18, .external_lex_state = 2},
    [84] = {.lex_state = 18, .external_lex_state = 2},
    [85] = {.lex_state = 18, .external_lex_state = 2},
    [86] = {.lex_state = 18, .external_lex_state = 2},
    [87] = {.lex_state = 18, .external_lex_state = 2},
    [88] = {.lex_state = 18, .external_lex_state = 2},
    [89] = {.lex_state = 18, .external_lex_state = 2},
    [90] = {.lex_state = 18, .external_lex_state = 2},
    [91] = {.lex_state = 18, .external_lex_state = 2},
    [92] = {.lex_state = 18, .external_lex_state = 2},
    [93] = {.lex_state = 18, .external_lex_state = 2},
    [94] = {.lex_state = 18, .external_lex_state = 2},
    [95] = {.lex_state = 18, .external_lex_state = 2},
    [96] = {.lex_state = 18, .external_lex_state = 2},
    [97] = {.lex_state = 18, .external_lex_state = 2},
    [98] = {.lex_state = 18, .external_lex_state = 2},
    [99] = {.lex_state = 18, .external_lex_state = 2},
    [100] = {.lex_state = 18, .external_lex_state = 2},
    [101] = {.lex_state = 19, .external_lex_state = 2},
    [102] = {.lex_state = 19, .external_lex_state = 2},
    [103] = {.lex_state = 19, .external_lex_state = 2},
    [104] = {.lex_state = 19, .external_lex_state = 2},
    [105] = {.lex_state = 19, .external_lex_state = 2},
    [106] = {.lex_state = 19, .external_lex_state = 2},
    [107] = {.lex_state = 19, .external_lex_state = 2},
    [108] = {.lex_state = 19, .external_lex_state = 2},
    [109] = {.lex_state = 19, .external_lex_state = 2},
    [110] = {.lex_state = 19, .external_lex_state = 2},
    [111] = {.lex_state = 19, .external_lex_state = 2},
    [112] = {.lex_state = 19, .external_lex_state = 2},
    [113] = {.lex_state = 19, .external_lex_state = 2},
    [114] = {.lex_state = 19, .external_lex_state = 2},
    [115] = {.lex_state = 19, .external_lex_state = 2},
    [116] = {.lex_state = 19, .external_lex_state = 2},
    [117] = {.lex_state = 19, .external_lex_state = 2},
    [118] = {.lex_state = 18, .external_lex_state = 2},
    [119] = {.lex_state = 18, .external_lex_state = 2},
    [120] = {.lex_state = 18, .external_lex_state = 2},
    [121] = {.lex_state = 18, .external_lex_state = 2},
    [122] = {.lex_state = 18, .external_lex_state = 2},
    [123] = {.lex_state = 18, .external_lex_state = 2},
    [124] = {.lex_state = 18, .external_lex_state = 2},
    [125] = {.lex_state = 18, .external_lex_state = 2},
    [126] = {.lex_state = 18, .external_lex_state = 2},
    [127] = {.lex_state = 18, .external_lex_state = 2},
    [128] = {.lex_state = 18, .external_lex_state = 2},
    [129] = {.lex_state = 18, .external_lex_state = 2},
    [130] = {.lex_state = 18, .external_lex_state = 2},
    [131] = {.lex_state = 18, .external_lex_state = 2},
    [132] = {.lex_state = 19, .external_lex_state = 3},
    [133] = {.lex_state = 18, .external_lex_state = 2},
    [134] = {.lex_state = 19, .external_lex_state = 3},
    [135] = {.lex_state = 19, .external_lex_state = 3},
    [136] = {.lex_state = 19, .external_lex_state = 3},
    [137] = {.lex_state = 19, .external_lex_state = 3},
    [138] = {.lex_state = 19, .external_lex_state = 3},
    [139] = {.lex_state = 19, .external_lex_state = 3},
    [140] = {.lex_state = 19, .external_lex_state = 3},
    [141] = {.lex_state = 19, .external_lex_state = 3},
    [142] = {.lex_state = 19, .external_lex_state = 3},
    [143] = {.lex_state = 19, .external_lex_state = 3},
    [144] = {.lex_state = 19, .external_lex_state = 3},
    [145] = {.lex_state = 19, .external_lex_state = 3},
    [146] = {.lex_state = 19, .external_lex_state = 3},
    [147] = {.lex_state = 18, .external_lex_state = 2},
    [148] = {.lex_state = 19, .external_lex_state = 3},
    [149] = {.lex_state = 19, .external_lex_state = 3},
    [150] = {.lex_state = 18, .external_lex_state = 2},
    [151] = {.lex_state = 19, .external_lex_state = 3},
    [152] = {.lex_state = 19, .external_lex_state = 3},
    [153] = {.lex_state = 19, .external_lex_state = 3},
    [154] = {.lex_state = 19, .external_lex_state = 3},
    [155] = {.lex_state = 19, .external_lex_state = 3},
    [156] = {.lex_state = 19, .external_lex_state = 3},
    [157] = {.lex_state = 19, .external_lex_state = 3},
    [158] = {.lex_state = 19, .external_lex_state = 2},
    [159] = {.lex_state = 18, .external_lex_state = 2},
    [160] = {.lex_state = 18, .external_lex_state = 2},
    [161] = {.lex_state = 18, .external_lex_state = 2},
    [162] = {.lex_state = 18, .external_lex_state = 2},
    [163] = {.lex_state = 18, .external_lex_state = 2},
    [164] = {.lex_state = 18, .external_lex_state = 2},
    [165] = {.lex_state = 18, .external_lex_state = 2},
    [166] = {.lex_state = 18, .external_lex_state = 2},
    [167] = {.lex_state = 18, .external_lex_state = 2},
    [168] = {.lex_state = 18, .external_lex_state = 2},
    [169] = {.lex_state = 18, .external_lex_state = 2},
    [170] = {.lex_state = 18, .external_lex_state = 2},
    [171] = {.lex_state = 18, .external_lex_state = 2},
    [172] = {.lex_state = 18, .external_lex_state = 2},
    [173] = {.lex_state = 18, .external_lex_state = 2},
    [174] = {.lex_state = 18, .external_lex_state = 2},
    [175] = {.lex_state = 18, .external_lex_state = 2},
    [176] = {.lex_state = 18, .external_lex_state = 2},
    [177] = {.lex_state = 18, .external_lex_state = 2},
    [178] = {.lex_state = 18, .external_lex_state = 2},
    [179] = {.lex_state = 18, .external_lex_state = 2},
    [180] = {.lex_state = 18, .external_lex_state = 2},
    [181] = {.lex_state = 18, .external_lex_state = 2},
    [182] = {.lex_state = 18, .external_lex_state = 2},
    [183] = {.lex_state = 18, .external_lex_state = 2},
    [184] = {.lex_state = 18, .external_lex_state = 2},
    [185] = {.lex_state = 18, .external_lex_state = 2},
    [186] = {.lex_state = 18, .external_lex_state = 2},
    [187] = {.lex_state = 18, .external_lex_state = 2},
    [188] = {.lex_state = 18, .external_lex_state = 2},
    [189] = {.lex_state = 18, .external_lex_state = 2},
    [190] = {.lex_state = 18, .external_lex_state = 2},
    [191] = {.lex_state = 18, .external_lex_state = 2},
    [192] = {.lex_state = 18, .external_lex_state = 2},
    [193] = {.lex_state = 18, .external_lex_state = 2},
    [194] = {.lex_state = 18, .external_lex_state = 2},
    [195] = {.lex_state = 18, .external_lex_state = 2},
    [196] = {.lex_state = 18, .external_lex_state = 2},
    [197] = {.lex_state = 18, .external_lex_state = 2},
    [198] = {.lex_state = 18, .external_lex_state = 2},
    [199] = {.lex_state = 18, .external_lex_state = 2},
    [200] = {.lex_state = 18, .external_lex_state = 2},
    [201] = {.lex_state = 18, .external_lex_state = 2},
    [202] = {.lex_state = 18, .external_lex_state = 2},
    [203] = {.lex_state = 18, .external_lex_state = 2},
    [204] = {.lex_state = 18, .external_lex_state = 2},
    [205] = {.lex_state = 18, .external_lex_state = 2},
    [206] = {.lex_state = 18, .external_lex_state = 2},
    [207] = {.lex_state = 18, .external_lex_state = 2},
    [208] = {.lex_state = 18, .external_lex_state = 2},
    [209] = {.lex_state = 18, .external_lex_state = 2},
    [210] = {.lex_state = 18, .external_lex_state = 2},
    [211] = {.lex_state = 18, .external_lex_state = 2},
    [212] = {.lex_state = 18, .external_lex_state = 2},
    [213] = {.lex_state = 18, .external_lex_state = 2},
    [214] = {.lex_state = 18, .external_lex_state = 2},
    [215] = {.lex_state = 18, .external_lex_state = 2},
    [216] = {.lex_state = 18, .external_lex_state = 2},
    [217] = {.lex_state = 18, .external_lex_state = 2},
    [218] = {.lex_state = 18, .external_lex_state = 2},
    [219] = {.lex_state = 18, .external_lex_state = 2},
    [220] = {.lex_state = 18, .external_lex_state = 2},
    [221] = {.lex_state = 18, .external_lex_state = 2},
    [222] = {.lex_state = 18, .external_lex_state = 2},
    [223] = {.lex_state = 18, .external_lex_state = 2},
    [224] = {.lex_state = 18, .external_lex_state = 2},
    [225] = {.lex_state = 18, .external_lex_state = 2},
    [226] = {.lex_state = 18, .external_lex_state = 2},
    [227] = {.lex_state = 18, .external_lex_state = 2},
    [228] = {.lex_state = 18, .external_lex_state = 2},
    [229] = {.lex_state = 18, .external_lex_state = 2},
    [230] = {.lex_state = 18, .external_lex_state = 2},
    [231] = {.lex_state = 18, .external_lex_state = 2},
    [232] = {.lex_state = 18, .external_lex_state = 2},
    [233] = {.lex_state = 18, .external_lex_state = 2},
    [234] = {.lex_state = 18, .external_lex_state = 2},
    [235] = {.lex_state = 18, .external_lex_state = 2},
    [236] = {.lex_state = 18, .external_lex_state = 2},
    [237] = {.lex_state = 18, .external_lex_state = 2},
    [238] = {.lex_state = 18, .external_lex_state = 2},
    [239] = {.lex_state = 18, .external_lex_state = 2},
    [240] = {.lex_state = 18, .external_lex_state = 2},
    [241] = {.lex_state = 18, .external_lex_state = 2},
    [242] = {.lex_state = 18, .external_lex_state = 2},
    [243] = {.lex_state = 18, .external_lex_state = 2},
    [244] = {.lex_state = 18, .external_lex_state = 2},
    [245] = {.lex_state = 18, .external_lex_state = 2},
    [246] = {.lex_state = 18, .external_lex_state = 2},
    [247] = {.lex_state = 18, .external_lex_state = 2},
    [248] = {.lex_state = 18, .external_lex_state = 2},
    [249] = {.lex_state = 18, .external_lex_state = 2},
    [250] = {.lex_state = 18, .external_lex_state = 2},
    [251] = {.lex_state = 18, .external_lex_state = 2},
    [252] = {.lex_state = 18, .external_lex_state = 2},
    [253] = {.lex_state = 18, .external_lex_state = 2},
    [254] = {.lex_state = 18, .external_lex_state = 2},
    [255] = {.lex_state = 18, .external_lex_state = 2},
    [256] = {.lex_state = 18, .external_lex_state = 2},
    [257] = {.lex_state = 18, .external_lex_state = 2},
    [258] = {.lex_state = 18, .external_lex_state = 2},
    [259] = {.lex_state = 18, .external_lex_state = 2},
    [260] = {.lex_state = 18, .external_lex_state = 2},
    [261] = {.lex_state = 18, .external_lex_state = 2},
    [262] = {.lex_state = 18, .external_lex_state = 2},
    [263] = {.lex_state = 18, .external_lex_state = 2},
    [264] = {.lex_state = 18, .external_lex_state = 2},
    [265] = {.lex_state = 18, .external_lex_state = 2},
    [266] = {.lex_state = 19, .external_lex_state = 3},
    [267] = {.lex_state = 18, .external_lex_state = 2},
    [268] = {.lex_state = 18, .external_lex_state = 2},
    [269] = {.lex_state = 19, .external_lex_state = 3},
    [270] = {.lex_state = 18, .external_lex_state = 2},
    [271] = {.lex_state = 18, .external_lex_state = 2},
    [272] = {.lex_state = 18, .external_lex_state = 2},
    [273] = {.lex_state = 18, .external_lex_state = 2},
    [274] = {.lex_state = 18, .external_lex_state = 2},
    [275] = {.lex_state = 18, .external_lex_state = 2},
    [276] = {.lex_state = 18, .external_lex_state = 2},
    [277] = {.lex_state = 18, .external_lex_state = 2},
    [278] = {.lex_state = 18, .external_lex_state = 2},
    [279] = {.lex_state = 18, .external_lex_state = 2},
    [280] = {.lex_state = 18, .external_lex_state = 2},
    [281] = {.lex_state = 18, .external_lex_state = 2},
    [282] = {.lex_state = 18, .external_lex_state = 2},
    [283] = {.lex_state = 18, .external_lex_state = 2},
    [284] = {.lex_state = 18, .external_lex_state = 2},
    [285] = {.lex_state = 18, .external_lex_state = 2},
    [286] = {.lex_state = 18, .external_lex_state = 2},
    [287] = {.lex_state = 18, .external_lex_state = 2},
    [288] = {.lex_state = 18, .external_lex_state = 2},
    [289] = {.lex_state = 18, .external_lex_state = 2},
    [290] = {.lex_state = 18, .external_lex_state = 2},
    [291] = {.lex_state = 18, .external_lex_state = 2},
    [292] = {.lex_state = 18, .external_lex_state = 2},
    [293] = {.lex_state = 18, .external_lex_state = 2},
    [294] = {.lex_state = 18, .external_lex_state = 2},
    [295] = {.lex_state = 18, .external_lex_state = 2},
    [296] = {.lex_state = 18, .external_lex_state = 2},
    [297] = {.lex_state = 18, .external_lex_state = 2},
    [298] = {.lex_state = 18, .external_lex_state = 2},
    [299] = {.lex_state = 18, .external_lex_state = 2},
    [300] = {.lex_state = 18, .external_lex_state = 2},
    [301] = {.lex_state = 19, .external_lex_state = 3},
    [302] = {.lex_state = 19, .external_lex_state = 3},
    [303] = {.lex_state = 19, .external_lex_state = 3},
    [304] = {.lex_state = 19, .external_lex_state = 3},
    [305] = {.lex_state = 19, .external_lex_state = 3},
    [306] = {.lex_state = 19, .external_lex_state = 3},
    [307] = {.lex_state = 19, .external_lex_state = 3},
    [308] = {.lex_state = 19, .external_lex_state = 3},
    [309] = {.lex_state = 19, .external_lex_state = 3},
    [310] = {.lex_state = 19, .external_lex_state = 3},
    [311] = {.lex_state = 19, .external_lex_state = 3},
    [312] = {.lex_state = 19, .external_lex_state = 3},
    [313] = {.lex_state = 19, .external_lex_state = 3},
    [314] = {.lex_state = 19, .external_lex_state = 3},
    [315] = {.lex_state = 19, .external_lex_state = 3},
    [316] = {.lex_state = 19, .external_lex_state = 3},
    [317] = {.lex_state = 19, .external_lex_state = 3},
    [318] = {.lex_state = 19, .external_lex_state = 3},
    [319] = {.lex_state = 19, .external_lex_state = 3},
    [320] = {.lex_state = 19, .external_lex_state = 3},
    [321] = {.lex_state = 19, .external_lex_state = 3},
    [322] = {.lex_state = 18, .external_lex_state = 2},
    [323] = {.lex_state = 0, .external_lex_state = 2},
    [324] = {.lex_state = 0, .external_lex_state = 2},
    [325] = {.lex_state = 0, .external_lex_state = 2},
    [326] = {.lex_state = 0, .external_lex_state = 2},
    [327] = {.lex_state = 0, .external_lex_state = 2},
    [328] = {.lex_state = 0, .external_lex_state = 2},
    [329] = {.lex_state = 0, .external_lex_state = 2},
    [330] = {.lex_state = 0, .external_lex_state = 3},
    [331] = {.lex_state = 0, .external_lex_state = 3},
    [332] = {.lex_state = 0, .external_lex_state = 3},
    [333] = {.lex_state = 0, .external_lex_state = 3},
    [334] = {.lex_state = 0, .external_lex_state = 2},
    [335] = {.lex_state = 0, .external_lex_state = 3},
    [336] = {.lex_state = 0, .external_lex_state = 3},
    [337] = {.lex_state = 0, .external_lex_state = 3},
    [338] = {.lex_state = 0, .external_lex_state = 3},
    [339] = {.lex_state = 0, .external_lex_state = 3},
    [340] = {.lex_state = 0, .external_lex_state = 3},
    [341] = {.lex_state = 0, .external_lex_state = 3},
    [342] = {.lex_state = 0, .external_lex_state = 3},
    [343] = {.lex_state = 0, .external_lex_state = 3},
    [344] = {.lex_state = 0, .external_lex_state = 3},
    [345] = {.lex_state = 0, .external_lex_state = 3},
    [346] = {.lex_state = 0, .external_lex_state = 3},
    [347] = {.lex_state = 0, .external_lex_state = 3},
    [348] = {.lex_state = 0, .external_lex_state = 3},
    [349] = {.lex_state = 0, .external_lex_state = 2},
    [350] = {.lex_state = 0, .external_lex_state = 3},
    [351] = {.lex_state = 1, .external_lex_state = 3},
    [352] = {.lex_state = 0, .external_lex_state = 3},
    [353] = {.lex_state = 2, .external_lex_state = 3},
    [354] = {.lex_state = 1, .external_lex_state = 3},
    [355] = {.lex_state = 2, .external_lex_state = 3},
    [356] = {.lex_state = 1, .external_lex_state = 3},
    [357] = {.lex_state = 0, .external_lex_state = 3},
    [358] = {.lex_state = 2, .external_lex_state = 3},
    [359] = {.lex_state = 1, .external_lex_state = 3},
    [360] = {.lex_state = 2, .external_lex_state = 3},
    [361] = {.lex_state = 2, .external_lex_state = 3},
    [362] = {.lex_state = 1, .external_lex_state = 3},
    [363] = {.lex_state = 2, .external_lex_state = 3},
    [364] = {.lex_state = 0, .external_lex_state = 3},
    [365] = {.lex_state = 1, .external_lex_state = 3},
    [366] = {.lex_state = 0, .external_lex_state = 3},
    [367] = {.lex_state = 0, .external_lex_state = 3},
    [368] = {.lex_state = 0, .external_lex_state = 3},
    [369] = {.lex_state = 1, .external_lex_state = 3},
    [370] = {.lex_state = 0, .external_lex_state = 3},
    [371] = {.lex_state = 1, .external_lex_state = 3},
    [372] = {.lex_state = 1, .external_lex_state = 3},
    [373] = {.lex_state = 2, .external_lex_state = 3},
    [374] = {.lex_state = 2, .external_lex_state = 3},
    [375] = {.lex_state = 2, .external_lex_state = 3},
    [376] = {.lex_state = 0, .external_lex_state = 3},
    [377] = {.lex_state = 2, .external_lex_state = 3},
    [378] = {.lex_state = 0, .external_lex_state = 3},
    [379] = {.lex_state = 0, .external_lex_state = 3},
    [380] = {.lex_state = 0, .external_lex_state = 3},
    [381] = {.lex_state = 0, .external_lex_state = 3},
    [382] = {.lex_state = 0, .external_lex_state = 3},
    [383] = {.lex_state = 0, .external_lex_state = 3},
    [384] = {.lex_state = 0, .external_lex_state = 3},
    [385] = {.lex_state = 0, .external_lex_state = 3},
    [386] = {.lex_state = 0, .external_lex_state = 3},
    [387] = {.lex_state = 1, .external_lex_state = 3},
    [388] = {.lex_state = 0, .external_lex_state = 3},
    [389] = {.lex_state = 0, .external_lex_state = 3},
    [390] = {.lex_state = 0, .external_lex_state = 3},
    [391] = {.lex_state = 0, .external_lex_state = 3},
    [392] = {.lex_state = 0, .external_lex_state = 3},
    [393] = {.lex_state = 0, .external_lex_state = 3},
    [394] = {.lex_state = 0, .external_lex_state = 3},
    [395] = {.lex_state = 0, .external_lex_state = 3},
    [396] = {.lex_state = 0, .external_lex_state = 3},
    [397] = {.lex_state = 0, .external_lex_state = 3},
    [398] = {.lex_state = 0, .external_lex_state = 3},
    [399] = {.lex_state = 0, .external_lex_state = 3},
    [400] = {.lex_state = 0, .external_lex_state = 3},
    [401] = {.lex_state = 0, .external_lex_state = 3},
    [402] = {.lex_state = 0, .external_lex_state = 3},
    [403] = {.lex_state = 0, .external_lex_state = 3},
    [404] = {.lex_state = 0, .external_lex_state = 3},
    [405] = {.lex_state = 0, .external_lex_state = 3},
    [406] = {.lex_state = 0, .external_lex_state = 3},
    [407] = {.lex_state = 0, .external_lex_state = 3},
    [408] = {.lex_state = 0, .external_lex_state = 3},
    [409] = {.lex_state = 0, .external_lex_state = 3},
    [410] = {.lex_state = 0, .external_lex_state = 3},
    [411] = {.lex_state = 0, .external_lex_state = 3},
    [412] = {.lex_state = 0, .external_lex_state = 3},
    [413] = {.lex_state = 0, .external_lex_state = 3},
    [414] = {.lex_state = 0, .external_lex_state = 4},
    [415] = {.lex_state = 0, .external_lex_state = 3},
    [416] = {.lex_state = 0, .external_lex_state = 3},
    [417] = {.lex_state = 0, .external_lex_state = 3},
    [418] = {.lex_state = 0, .external_lex_state = 3},
    [419] = {.lex_state = 0, .external_lex_state = 3},
    [420] = {.lex_state = 0, .external_lex_state = 3},
    [421] = {.lex_state = 0, .external_lex_state = 3},
    [422] = {.lex_state = 0, .external_lex_state = 3},
    [423] = {.lex_state = 0, .external_lex_state = 3},
    [424] = {.lex_state = 0, .external_lex_state = 3},
    [425] = {.lex_state = 0, .external_lex_state = 3},
    [426] = {.lex_state = 0, .external_lex_state = 3},
    [427] = {.lex_state = 0, .external_lex_state = 3},
    [428] = {.lex_state = 0, .external_lex_state = 3},
    [429] = {.lex_state = 0, .external_lex_state = 3},
    [430] = {.lex_state = 0, .external_lex_state = 3},
    [431] = {.lex_state = 0, .external_lex_state = 3},
    [432] = {.lex_state = 4, .external_lex_state = 3},
    [433] = {.lex_state = 0, .external_lex_state = 3},
    [434] = {.lex_state = 0, .external_lex_state = 3},
    [435] = {.lex_state = 0, .external_lex_state = 3},
    [436] = {.lex_state = 0, .external_lex_state = 3},
    [437] = {.lex_state = 0, .external_lex_state = 3},
    [438] = {.lex_state = 0, .external_lex_state = 3},
    [439] = {.lex_state = 0, .external_lex_state = 3},
    [440] = {.lex_state = 0, .external_lex_state = 3},
    [441] = {.lex_state = 0, .external_lex_state = 3},
    [442] = {.lex_state = 0, .external_lex_state = 3},
    [443] = {.lex_state = 0, .external_lex_state = 4},
    [444] = {.lex_state = 0, .external_lex_state = 3},
    [445] = {.lex_state = 0, .external_lex_state = 3},
    [446] = {.lex_state = 0, .external_lex_state = 3},
    [447] = {.lex_state = 0, .external_lex_state = 3},
    [448] = {.lex_state = 0, .external_lex_state = 3},
    [449] = {.lex_state = 0, .external_lex_state = 3},
    [450] = {.lex_state = 0, .external_lex_state = 3},
    [451] = {.lex_state = 0, .external_lex_state = 5},
    [452] = {.lex_state = 0, .external_lex_state = 3},
    [453] = {.lex_state = 0, .external_lex_state = 6},
    [454] = {.lex_state = 0, .external_lex_state = 3},
    [455] = {.lex_state = 0, .external_lex_state = 3},
    [456] = {.lex_state = 0, .external_lex_state = 7},
    [457] = {.lex_state = 0, .external_lex_state = 3},
    [458] = {.lex_state = 82, .external_lex_state = 3},
    [459] = {.lex_state = 0, .external_lex_state = 4},
    [460] = {.lex_state = 0, .external_lex_state = 3},
    [461] = {.lex_state = 0, .external_lex_state = 4},
    [462] = {.lex_state = 0, .external_lex_state = 3},
    [463] = {.lex_state = 0, .external_lex_state = 5},
    [464] = {.lex_state = 0, .external_lex_state = 3},
    [465] = {.lex_state = 0, .external_lex_state = 3},
    [466] = {.lex_state = 0, .external_lex_state = 3},
    [467] = {.lex_state = 4, .external_lex_state = 3},
    [468] = {.lex_state = 0, .external_lex_state = 3},
    [469] = {.lex_state = 0, .external_lex_state = 3},
    [470] = {.lex_state = 0, .external_lex_state = 3},
    [471] = {.lex_state = 0, .external_lex_state = 3},
    [472] = {.lex_state = 0, .external_lex_state = 5},
    [473] = {.lex_state = 0, .external_lex_state = 3},
    [474] = {.lex_state = 0, .external_lex_state = 3},
    [475] = {.lex_state = 0, .external_lex_state = 5},
    [476] = {.lex_state = 0, .external_lex_state = 3},
    [477] = {(TSStateId)(-1)},
    [478] = {(TSStateId)(-1)},
};

enum {
    ts_external_token__block_comment_start = 0,
    ts_external_token__block_comment_content = 1,
    ts_external_token__block_comment_end = 2,
    ts_external_token__block_string_start = 3,
    ts_external_token__block_string_content = 4,
    ts_external_token__block_string_end = 5,
};

static const TSSymbol ts_external_scanner_symbol_map[EXTERNAL_TOKEN_COUNT] = {
    [ts_external_token__block_comment_start] = sym__block_comment_start,
    [ts_external_token__block_comment_content] = sym__block_comment_content,
    [ts_external_token__block_comment_end] = sym__block_comment_end,
    [ts_external_token__block_string_start] = sym__block_string_start,
    [ts_external_token__block_string_content] = sym__block_string_content,
    [ts_external_token__block_string_end] = sym__block_string_end,
};

static const bool ts_external_scanner_states[8][EXTERNAL_TOKEN_COUNT] = {
    [1] = {
        [ts_external_token__block_comment_start] = true,
        [ts_external_token__block_comment_content] = true,
        [ts_external_token__block_comment_end] = true,
        [ts_external_token__block_string_start] = true,
        [ts_external_token__block_string_content] = true,
        [ts_external_token__block_string_end] = true,
    },
    [2] = {
        [ts_external_token__block_comment_start] = true,
        [ts_external_token__block_string_start] = true,
    },
    [3] = {
        [ts_external_token__block_comment_start] = true,
    },
    [4] = {
        [ts_external_token__block_comment_start] = true,
        [ts_external_token__block_string_end] = true,
    },
    [5] = {
        [ts_external_token__block_comment_start] = true,
        [ts_external_token__block_string_content] = true,
    },
    [6] = {
        [ts_external_token__block_comment_start] = true,
        [ts_external_token__block_comment_content] = true,
    },
    [7] = {
        [ts_external_token__block_comment_start] = true,
        [ts_external_token__block_comment_end] = true,
    },
};

static const uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
    [0] = {
        [sym_comment] = STATE(0),
        [ts_builtin_sym_end] = ACTIONS(1),
        [sym_identifier] = ACTIONS(1),
        [sym_hash_bang_line] = ACTIONS(1),
        [anon_sym_return] = ACTIONS(1),
        [anon_sym_SEMI] = ACTIONS(1),
        [anon_sym_EQ] = ACTIONS(1),
        [anon_sym_COMMA] = ACTIONS(1),
        [anon_sym_COLON_COLON] = ACTIONS(1),
        [sym_break_statement] = ACTIONS(1),
        [anon_sym_goto] = ACTIONS(1),
        [anon_sym_do] = ACTIONS(1),
        [anon_sym_end] = ACTIONS(1),
        [anon_sym_while] = ACTIONS(1),
        [anon_sym_repeat] = ACTIONS(1),
        [anon_sym_until] = ACTIONS(1),
        [anon_sym_if] = ACTIONS(1),
        [anon_sym_then] = ACTIONS(1),
        [anon_sym_elseif] = ACTIONS(1),
        [anon_sym_else] = ACTIONS(1),
        [anon_sym_for] = ACTIONS(1),
        [anon_sym_in] = ACTIONS(1),
        [anon_sym_function] = ACTIONS(1),
        [anon_sym_local] = ACTIONS(1),
        [anon_sym_DOT] = ACTIONS(1),
        [anon_sym_COLON] = ACTIONS(1),
        [anon_sym_LT] = ACTIONS(1),
        [anon_sym_GT] = ACTIONS(1),
        [sym_nil] = ACTIONS(1),
        [sym_false] = ACTIONS(1),
        [sym_true] = ACTIONS(1),
        [sym_number] = ACTIONS(1),
        [anon_sym_DQUOTE] = ACTIONS(1),
        [anon_sym_SQUOTE] = ACTIONS(1),
        [sym_escape_sequence] = ACTIONS(1),
        [sym_vararg_expression] = ACTIONS(1),
        [anon_sym_LPAREN] = ACTIONS(1),
        [anon_sym_RPAREN] = ACTIONS(1),
        [anon_sym_LBRACK] = ACTIONS(1),
        [anon_sym_RBRACK] = ACTIONS(1),
        [anon_sym_LBRACE] = ACTIONS(1),
        [anon_sym_RBRACE] = ACTIONS(1),
        [anon_sym_or] = ACTIONS(1),
        [anon_sym_and] = ACTIONS(1),
        [anon_sym_LT_EQ] = ACTIONS(1),
        [anon_sym_EQ_EQ] = ACTIONS(1),
        [anon_sym_TILDE_EQ] = ACTIONS(1),
        [anon_sym_GT_EQ] = ACTIONS(1),
        [anon_sym_PIPE] = ACTIONS(1),
        [anon_sym_TILDE] = ACTIONS(1),
        [anon_sym_AMP] = ACTIONS(1),
        [anon_sym_LT_LT] = ACTIONS(1),
        [anon_sym_GT_GT] = ACTIONS(1),
        [anon_sym_PLUS] = ACTIONS(1),
        [anon_sym_DASH] = ACTIONS(1),
        [anon_sym_STAR] = ACTIONS(1),
        [anon_sym_SLASH] = ACTIONS(1),
        [anon_sym_SLASH_SLASH] = ACTIONS(1),
        [anon_sym_PERCENT] = ACTIONS(1),
        [anon_sym_DOT_DOT] = ACTIONS(1),
        [anon_sym_CARET] = ACTIONS(1),
        [anon_sym_not] = ACTIONS(1),
        [anon_sym_POUND] = ACTIONS(1),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_comment_content] = ACTIONS(1),
        [sym__block_comment_end] = ACTIONS(1),
        [sym__block_string_start] = ACTIONS(1),
        [sym__block_string_content] = ACTIONS(1),
        [sym__block_string_end] = ACTIONS(1),
    },
    [1] = {
        [sym_chunk] = STATE(407),
        [sym_statement] = STATE(298),
        [sym_return_statement] = STATE(441),
        [sym_empty_statement] = STATE(293),
        [sym_assignment_statement] = STATE(293),
        [sym__variable_assignment_varlist] = STATE(439),
        [sym_label_statement] = STATE(293),
        [sym_goto_statement] = STATE(293),
        [sym_do_statement] = STATE(293),
        [sym_while_statement] = STATE(293),
        [sym_repeat_statement] = STATE(293),
        [sym_if_statement] = STATE(293),
        [sym_for_statement] = STATE(293),
        [sym_declaration] = STATE(296),
        [sym_function_declaration] = STATE(287),
        [sym__local_function_declaration] = STATE(286),
        [sym_variable_declaration] = STATE(284),
        [sym_expression] = STATE(100),
        [sym_string] = STATE(97),
        [sym__quote_string] = STATE(55),
        [sym__block_string] = STATE(54),
        [sym_function_definition] = STATE(97),
        [sym__prefix_expression] = STATE(325),
        [sym_variable] = STATE(41),
        [sym_bracket_index_expression] = STATE(44),
        [sym_dot_index_expression] = STATE(44),
        [sym_function_call] = STATE(61),
        [sym_method_index_expression] = STATE(329),
        [sym_parenthesized_expression] = STATE(61),
        [sym_table_constructor] = STATE(97),
        [sym_binary_expression] = STATE(97),
        [sym_unary_expression] = STATE(97),
        [sym_comment] = STATE(1),
        [aux_sym_chunk_repeat1] = STATE(20),
        [ts_builtin_sym_end] = ACTIONS(7),
        [sym_identifier] = ACTIONS(9),
        [sym_hash_bang_line] = ACTIONS(11),
        [anon_sym_return] = ACTIONS(13),
        [anon_sym_SEMI] = ACTIONS(15),
        [anon_sym_COLON_COLON] = ACTIONS(17),
        [sym_break_statement] = ACTIONS(19),
        [anon_sym_goto] = ACTIONS(21),
        [anon_sym_do] = ACTIONS(23),
        [anon_sym_while] = ACTIONS(25),
        [anon_sym_repeat] = ACTIONS(27),
        [anon_sym_if] = ACTIONS(29),
        [anon_sym_for] = ACTIONS(31),
        [anon_sym_function] = ACTIONS(33),
        [anon_sym_local] = ACTIONS(35),
        [sym_nil] = ACTIONS(37),
        [sym_false] = ACTIONS(37),
        [sym_true] = ACTIONS(37),
        [sym_number] = ACTIONS(39),
        [anon_sym_DQUOTE] = ACTIONS(41),
        [anon_sym_SQUOTE] = ACTIONS(43),
        [sym_vararg_expression] = ACTIONS(39),
        [anon_sym_LPAREN] = ACTIONS(45),
        [anon_sym_LBRACE] = ACTIONS(47),
        [anon_sym_TILDE] = ACTIONS(49),
        [anon_sym_DASH] = ACTIONS(51),
        [anon_sym_not] = ACTIONS(51),
        [anon_sym_POUND] = ACTIONS(51),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_string_start] = ACTIONS(53),
    },
    [2] = {
        [sym__block] = STATE(340),
        [sym_statement] = STATE(252),
        [sym_return_statement] = STATE(350),
        [sym_empty_statement] = STATE(248),
        [sym_assignment_statement] = STATE(248),
        [sym__variable_assignment_varlist] = STATE(411),
        [sym_label_statement] = STATE(248),
        [sym_goto_statement] = STATE(248),
        [sym_do_statement] = STATE(248),
        [sym_while_statement] = STATE(248),
        [sym_repeat_statement] = STATE(248),
        [sym_if_statement] = STATE(248),
        [sym_elseif_statement] = STATE(401),
        [sym_else_statement] = STATE(428),
        [sym_for_statement] = STATE(248),
        [sym_declaration] = STATE(253),
        [sym_function_declaration] = STATE(254),
        [sym__local_function_declaration] = STATE(255),
        [sym_variable_declaration] = STATE(256),
        [sym_expression] = STATE(79),
        [sym_string] = STATE(71),
        [sym__quote_string] = STATE(28),
        [sym__block_string] = STATE(29),
        [sym_function_definition] = STATE(71),
        [sym__prefix_expression] = STATE(323),
        [sym_variable] = STATE(23),
        [sym_bracket_index_expression] = STATE(25),
        [sym_dot_index_expression] = STATE(25),
        [sym_function_call] = STATE(30),
        [sym_method_index_expression] = STATE(328),
        [sym_parenthesized_expression] = STATE(30),
        [sym_table_constructor] = STATE(71),
        [sym_binary_expression] = STATE(71),
        [sym_unary_expression] = STATE(71),
        [sym_comment] = STATE(2),
        [aux_sym_chunk_repeat1] = STATE(5),
        [aux_sym_if_statement_repeat1] = STATE(341),
        [sym_identifier] = ACTIONS(55),
        [anon_sym_return] = ACTIONS(57),
        [anon_sym_SEMI] = ACTIONS(59),
        [anon_sym_COLON_COLON] = ACTIONS(61),
        [sym_break_statement] = ACTIONS(63),
        [anon_sym_goto] = ACTIONS(65),
        [anon_sym_do] = ACTIONS(67),
        [anon_sym_end] = ACTIONS(69),
        [anon_sym_while] = ACTIONS(71),
        [anon_sym_repeat] = ACTIONS(73),
        [anon_sym_if] = ACTIONS(75),
        [anon_sym_elseif] = ACTIONS(77),
        [anon_sym_else] = ACTIONS(79),
        [anon_sym_for] = ACTIONS(81),
        [anon_sym_function] = ACTIONS(83),
        [anon_sym_local] = ACTIONS(85),
        [sym_nil] = ACTIONS(87),
        [sym_false] = ACTIONS(87),
        [sym_true] = ACTIONS(87),
        [sym_number] = ACTIONS(89),
        [anon_sym_DQUOTE] = ACTIONS(91),
        [anon_sym_SQUOTE] = ACTIONS(93),
        [sym_vararg_expression] = ACTIONS(89),
        [anon_sym_LPAREN] = ACTIONS(95),
        [anon_sym_LBRACE] = ACTIONS(97),
        [anon_sym_TILDE] = ACTIONS(99),
        [anon_sym_DASH] = ACTIONS(101),
        [anon_sym_not] = ACTIONS(101),
        [anon_sym_POUND] = ACTIONS(99),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_string_start] = ACTIONS(103),
    },
    [3] = {
        [sym__block] = STATE(339),
        [sym_statement] = STATE(252),
        [sym_return_statement] = STATE(350),
        [sym_empty_statement] = STATE(248),
        [sym_assignment_statement] = STATE(248),
        [sym__variable_assignment_varlist] = STATE(411),
        [sym_label_statement] = STATE(248),
        [sym_goto_statement] = STATE(248),
        [sym_do_statement] = STATE(248),
        [sym_while_statement] = STATE(248),
        [sym_repeat_statement] = STATE(248),
        [sym_if_statement] = STATE(248),
        [sym_elseif_statement] = STATE(401),
        [sym_else_statement] = STATE(429),
        [sym_for_statement] = STATE(248),
        [sym_declaration] = STATE(253),
        [sym_function_declaration] = STATE(254),
        [sym__local_function_declaration] = STATE(255),
        [sym_variable_declaration] = STATE(256),
        [sym_expression] = STATE(79),
        [sym_string] = STATE(71),
        [sym__quote_string] = STATE(28),
        [sym__block_string] = STATE(29),
        [sym_function_definition] = STATE(71),
        [sym__prefix_expression] = STATE(323),
        [sym_variable] = STATE(23),
        [sym_bracket_index_expression] = STATE(25),
        [sym_dot_index_expression] = STATE(25),
        [sym_function_call] = STATE(30),
        [sym_method_index_expression] = STATE(328),
        [sym_parenthesized_expression] = STATE(30),
        [sym_table_constructor] = STATE(71),
        [sym_binary_expression] = STATE(71),
        [sym_unary_expression] = STATE(71),
        [sym_comment] = STATE(3),
        [aux_sym_chunk_repeat1] = STATE(5),
        [aux_sym_if_statement_repeat1] = STATE(337),
        [sym_identifier] = ACTIONS(55),
        [anon_sym_return] = ACTIONS(57),
        [anon_sym_SEMI] = ACTIONS(59),
        [anon_sym_COLON_COLON] = ACTIONS(61),
        [sym_break_statement] = ACTIONS(63),
        [anon_sym_goto] = ACTIONS(65),
        [anon_sym_do] = ACTIONS(67),
        [anon_sym_end] = ACTIONS(105),
        [anon_sym_while] = ACTIONS(71),
        [anon_sym_repeat] = ACTIONS(73),
        [anon_sym_if] = ACTIONS(75),
        [anon_sym_elseif] = ACTIONS(77),
        [anon_sym_else] = ACTIONS(79),
        [anon_sym_for] = ACTIONS(81),
        [anon_sym_function] = ACTIONS(83),
        [anon_sym_local] = ACTIONS(85),
        [sym_nil] = ACTIONS(87),
        [sym_false] = ACTIONS(87),
        [sym_true] = ACTIONS(87),
        [sym_number] = ACTIONS(89),
        [anon_sym_DQUOTE] = ACTIONS(91),
        [anon_sym_SQUOTE] = ACTIONS(93),
        [sym_vararg_expression] = ACTIONS(89),
        [anon_sym_LPAREN] = ACTIONS(95),
        [anon_sym_LBRACE] = ACTIONS(97),
        [anon_sym_TILDE] = ACTIONS(99),
        [anon_sym_DASH] = ACTIONS(101),
        [anon_sym_not] = ACTIONS(101),
        [anon_sym_POUND] = ACTIONS(99),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_string_start] = ACTIONS(103),
    },
    [4] = {
        [sym__block] = STATE(392),
        [sym_statement] = STATE(252),
        [sym_return_statement] = STATE(350),
        [sym_empty_statement] = STATE(248),
        [sym_assignment_statement] = STATE(248),
        [sym__variable_assignment_varlist] = STATE(411),
        [sym_label_statement] = STATE(248),
        [sym_goto_statement] = STATE(248),
        [sym_do_statement] = STATE(248),
        [sym_while_statement] = STATE(248),
        [sym_repeat_statement] = STATE(248),
        [sym_if_statement] = STATE(248),
        [sym_for_statement] = STATE(248),
        [sym_declaration] = STATE(253),
        [sym_function_declaration] = STATE(254),
        [sym__local_function_declaration] = STATE(255),
        [sym_variable_declaration] = STATE(256),
        [sym_expression] = STATE(79),
        [sym_string] = STATE(71),
        [sym__quote_string] = STATE(28),
        [sym__block_string] = STATE(29),
        [sym_function_definition] = STATE(71),
        [sym__prefix_expression] = STATE(323),
        [sym_variable] = STATE(23),
        [sym_bracket_index_expression] = STATE(25),
        [sym_dot_index_expression] = STATE(25),
        [sym_function_call] = STATE(30),
        [sym_method_index_expression] = STATE(328),
        [sym_parenthesized_expression] = STATE(30),
        [sym_table_constructor] = STATE(71),
        [sym_binary_expression] = STATE(71),
        [sym_unary_expression] = STATE(71),
        [sym_comment] = STATE(4),
        [aux_sym_chunk_repeat1] = STATE(5),
        [sym_identifier] = ACTIONS(55),
        [anon_sym_return] = ACTIONS(57),
        [anon_sym_SEMI] = ACTIONS(59),
        [anon_sym_COLON_COLON] = ACTIONS(61),
        [sym_break_statement] = ACTIONS(63),
        [anon_sym_goto] = ACTIONS(65),
        [anon_sym_do] = ACTIONS(67),
        [anon_sym_end] = ACTIONS(107),
        [anon_sym_while] = ACTIONS(71),
        [anon_sym_repeat] = ACTIONS(73),
        [anon_sym_if] = ACTIONS(75),
        [anon_sym_elseif] = ACTIONS(107),
        [anon_sym_else] = ACTIONS(107),
        [anon_sym_for] = ACTIONS(81),
        [anon_sym_function] = ACTIONS(83),
        [anon_sym_local] = ACTIONS(85),
        [sym_nil] = ACTIONS(87),
        [sym_false] = ACTIONS(87),
        [sym_true] = ACTIONS(87),
        [sym_number] = ACTIONS(89),
        [anon_sym_DQUOTE] = ACTIONS(91),
        [anon_sym_SQUOTE] = ACTIONS(93),
        [sym_vararg_expression] = ACTIONS(89),
        [anon_sym_LPAREN] = ACTIONS(95),
        [anon_sym_LBRACE] = ACTIONS(97),
        [anon_sym_TILDE] = ACTIONS(99),
        [anon_sym_DASH] = ACTIONS(101),
        [anon_sym_not] = ACTIONS(101),
        [anon_sym_POUND] = ACTIONS(99),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_string_start] = ACTIONS(103),
    },
    [5] = {
        [sym_statement] = STATE(252),
        [sym_return_statement] = STATE(367),
        [sym_empty_statement] = STATE(248),
        [sym_assignment_statement] = STATE(248),
        [sym__variable_assignment_varlist] = STATE(411),
        [sym_label_statement] = STATE(248),
        [sym_goto_statement] = STATE(248),
        [sym_do_statement] = STATE(248),
        [sym_while_statement] = STATE(248),
        [sym_repeat_statement] = STATE(248),
        [sym_if_statement] = STATE(248),
        [sym_for_statement] = STATE(248),
        [sym_declaration] = STATE(253),
        [sym_function_declaration] = STATE(254),
        [sym__local_function_declaration] = STATE(255),
        [sym_variable_declaration] = STATE(256),
        [sym_expression] = STATE(79),
        [sym_string] = STATE(71),
        [sym__quote_string] = STATE(28),
        [sym__block_string] = STATE(29),
        [sym_function_definition] = STATE(71),
        [sym__prefix_expression] = STATE(323),
        [sym_variable] = STATE(23),
        [sym_bracket_index_expression] = STATE(25),
        [sym_dot_index_expression] = STATE(25),
        [sym_function_call] = STATE(30),
        [sym_method_index_expression] = STATE(328),
        [sym_parenthesized_expression] = STATE(30),
        [sym_table_constructor] = STATE(71),
        [sym_binary_expression] = STATE(71),
        [sym_unary_expression] = STATE(71),
        [sym_comment] = STATE(5),
        [aux_sym_chunk_repeat1] = STATE(6),
        [sym_identifier] = ACTIONS(55),
        [anon_sym_return] = ACTIONS(57),
        [anon_sym_SEMI] = ACTIONS(59),
        [anon_sym_COLON_COLON] = ACTIONS(61),
        [sym_break_statement] = ACTIONS(63),
        [anon_sym_goto] = ACTIONS(65),
        [anon_sym_do] = ACTIONS(67),
        [anon_sym_end] = ACTIONS(109),
        [anon_sym_while] = ACTIONS(71),
        [anon_sym_repeat] = ACTIONS(73),
        [anon_sym_until] = ACTIONS(109),
        [anon_sym_if] = ACTIONS(75),
        [anon_sym_elseif] = ACTIONS(109),
        [anon_sym_else] = ACTIONS(109),
        [anon_sym_for] = ACTIONS(81),
        [anon_sym_function] = ACTIONS(83),
        [anon_sym_local] = ACTIONS(85),
        [sym_nil] = ACTIONS(87),
        [sym_false] = ACTIONS(87),
        [sym_true] = ACTIONS(87),
        [sym_number] = ACTIONS(89),
        [anon_sym_DQUOTE] = ACTIONS(91),
        [anon_sym_SQUOTE] = ACTIONS(93),
        [sym_vararg_expression] = ACTIONS(89),
        [anon_sym_LPAREN] = ACTIONS(95),
        [anon_sym_LBRACE] = ACTIONS(97),
        [anon_sym_TILDE] = ACTIONS(99),
        [anon_sym_DASH] = ACTIONS(101),
        [anon_sym_not] = ACTIONS(101),
        [anon_sym_POUND] = ACTIONS(99),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_string_start] = ACTIONS(103),
    },
    [6] = {
        [sym_statement] = STATE(252),
        [sym_empty_statement] = STATE(248),
        [sym_assignment_statement] = STATE(248),
        [sym__variable_assignment_varlist] = STATE(411),
        [sym_label_statement] = STATE(248),
        [sym_goto_statement] = STATE(248),
        [sym_do_statement] = STATE(248),
        [sym_while_statement] = STATE(248),
        [sym_repeat_statement] = STATE(248),
        [sym_if_statement] = STATE(248),
        [sym_for_statement] = STATE(248),
        [sym_declaration] = STATE(253),
        [sym_function_declaration] = STATE(254),
        [sym__local_function_declaration] = STATE(255),
        [sym_variable_declaration] = STATE(256),
        [sym_expression] = STATE(79),
        [sym_string] = STATE(71),
        [sym__quote_string] = STATE(28),
        [sym__block_string] = STATE(29),
        [sym_function_definition] = STATE(71),
        [sym__prefix_expression] = STATE(323),
        [sym_variable] = STATE(23),
        [sym_bracket_index_expression] = STATE(25),
        [sym_dot_index_expression] = STATE(25),
        [sym_function_call] = STATE(30),
        [sym_method_index_expression] = STATE(328),
        [sym_parenthesized_expression] = STATE(30),
        [sym_table_constructor] = STATE(71),
        [sym_binary_expression] = STATE(71),
        [sym_unary_expression] = STATE(71),
        [sym_comment] = STATE(6),
        [aux_sym_chunk_repeat1] = STATE(6),
        [sym_identifier] = ACTIONS(111),
        [anon_sym_return] = ACTIONS(114),
        [anon_sym_SEMI] = ACTIONS(116),
        [anon_sym_COLON_COLON] = ACTIONS(119),
        [sym_break_statement] = ACTIONS(122),
        [anon_sym_goto] = ACTIONS(125),
        [anon_sym_do] = ACTIONS(128),
        [anon_sym_end] = ACTIONS(114),
        [anon_sym_while] = ACTIONS(131),
        [anon_sym_repeat] = ACTIONS(134),
        [anon_sym_until] = ACTIONS(114),
        [anon_sym_if] = ACTIONS(137),
        [anon_sym_elseif] = ACTIONS(114),
        [anon_sym_else] = ACTIONS(114),
        [anon_sym_for] = ACTIONS(140),
        [anon_sym_function] = ACTIONS(143),
        [anon_sym_local] = ACTIONS(146),
        [sym_nil] = ACTIONS(149),
        [sym_false] = ACTIONS(149),
        [sym_true] = ACTIONS(149),
        [sym_number] = ACTIONS(152),
        [anon_sym_DQUOTE] = ACTIONS(155),
        [anon_sym_SQUOTE] = ACTIONS(158),
        [sym_vararg_expression] = ACTIONS(152),
        [anon_sym_LPAREN] = ACTIONS(161),
        [anon_sym_LBRACE] = ACTIONS(164),
        [anon_sym_TILDE] = ACTIONS(167),
        [anon_sym_DASH] = ACTIONS(170),
        [anon_sym_not] = ACTIONS(170),
        [anon_sym_POUND] = ACTIONS(167),
        [anon_sym_DASH_DASH] = ACTIONS(3),
        [sym__block_comment_start] = ACTIONS(5),
        [sym__block_string_start] = ACTIONS(173),
    },
};

static const uint16_t ts_small_parse_table[] = {
    [0] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(176),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(7),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(446),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [155] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(178),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(8),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(431),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [310] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(180),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(9),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(427),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [465] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(182),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(10),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(433),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [620] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(184),
    1,
    anon_sym_until,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(11),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(438),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [775] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(186),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(12),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(424),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [930] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(188),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(13),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(421),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [1085] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(190),
    1,
    anon_sym_until,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(14),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(417),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [1240] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(192),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(15),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(416),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [1395] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(194),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(16),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(409),
    1,
    sym__block,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [1550] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(196),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(17),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(426),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [1705] = 45,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(57),
    1,
    anon_sym_return,
    ACTIONS(59),
    1,
    anon_sym_SEMI,
    ACTIONS(61),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(63),
    1,
    sym_break_statement,
    ACTIONS(65),
    1,
    anon_sym_goto,
    ACTIONS(67),
    1,
    anon_sym_do,
    ACTIONS(71),
    1,
    anon_sym_while,
    ACTIONS(73),
    1,
    anon_sym_repeat,
    ACTIONS(75),
    1,
    anon_sym_if,
    ACTIONS(81),
    1,
    anon_sym_for,
    ACTIONS(83),
    1,
    anon_sym_function,
    ACTIONS(85),
    1,
    anon_sym_local,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(198),
    1,
    anon_sym_end,
    STATE(5),
    1,
    aux_sym_chunk_repeat1,
    STATE(18),
    1,
    sym_comment,
    STATE(23),
    1,
    sym_variable,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(79),
    1,
    sym_expression,
    STATE(252),
    1,
    sym_statement,
    STATE(253),
    1,
    sym_declaration,
    STATE(254),
    1,
    sym_function_declaration,
    STATE(255),
    1,
    sym__local_function_declaration,
    STATE(256),
    1,
    sym_variable_declaration,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    STATE(350),
    1,
    sym_return_statement,
    STATE(411),
    1,
    sym__variable_assignment_varlist,
    STATE(452),
    1,
    sym__block,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(30),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(248),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [1860] = 44,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(13),
    1,
    anon_sym_return,
    ACTIONS(15),
    1,
    anon_sym_SEMI,
    ACTIONS(17),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(19),
    1,
    sym_break_statement,
    ACTIONS(21),
    1,
    anon_sym_goto,
    ACTIONS(23),
    1,
    anon_sym_do,
    ACTIONS(25),
    1,
    anon_sym_while,
    ACTIONS(27),
    1,
    anon_sym_repeat,
    ACTIONS(29),
    1,
    anon_sym_if,
    ACTIONS(31),
    1,
    anon_sym_for,
    ACTIONS(33),
    1,
    anon_sym_function,
    ACTIONS(35),
    1,
    anon_sym_local,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(200),
    1,
    ts_builtin_sym_end,
    STATE(19),
    1,
    sym_comment,
    STATE(21),
    1,
    aux_sym_chunk_repeat1,
    STATE(41),
    1,
    sym_variable,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(100),
    1,
    sym_expression,
    STATE(284),
    1,
    sym_variable_declaration,
    STATE(286),
    1,
    sym__local_function_declaration,
    STATE(287),
    1,
    sym_function_declaration,
    STATE(296),
    1,
    sym_declaration,
    STATE(298),
    1,
    sym_statement,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    STATE(439),
    1,
    sym__variable_assignment_varlist,
    STATE(465),
    1,
    sym_return_statement,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(61),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(293),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [2012] = 44,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(13),
    1,
    anon_sym_return,
    ACTIONS(15),
    1,
    anon_sym_SEMI,
    ACTIONS(17),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(19),
    1,
    sym_break_statement,
    ACTIONS(21),
    1,
    anon_sym_goto,
    ACTIONS(23),
    1,
    anon_sym_do,
    ACTIONS(25),
    1,
    anon_sym_while,
    ACTIONS(27),
    1,
    anon_sym_repeat,
    ACTIONS(29),
    1,
    anon_sym_if,
    ACTIONS(31),
    1,
    anon_sym_for,
    ACTIONS(33),
    1,
    anon_sym_function,
    ACTIONS(35),
    1,
    anon_sym_local,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(200),
    1,
    ts_builtin_sym_end,
    STATE(20),
    1,
    sym_comment,
    STATE(22),
    1,
    aux_sym_chunk_repeat1,
    STATE(41),
    1,
    sym_variable,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(100),
    1,
    sym_expression,
    STATE(284),
    1,
    sym_variable_declaration,
    STATE(286),
    1,
    sym__local_function_declaration,
    STATE(287),
    1,
    sym_function_declaration,
    STATE(296),
    1,
    sym_declaration,
    STATE(298),
    1,
    sym_statement,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    STATE(439),
    1,
    sym__variable_assignment_varlist,
    STATE(465),
    1,
    sym_return_statement,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(61),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(293),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [2164] = 44,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(13),
    1,
    anon_sym_return,
    ACTIONS(15),
    1,
    anon_sym_SEMI,
    ACTIONS(17),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(19),
    1,
    sym_break_statement,
    ACTIONS(21),
    1,
    anon_sym_goto,
    ACTIONS(23),
    1,
    anon_sym_do,
    ACTIONS(25),
    1,
    anon_sym_while,
    ACTIONS(27),
    1,
    anon_sym_repeat,
    ACTIONS(29),
    1,
    anon_sym_if,
    ACTIONS(31),
    1,
    anon_sym_for,
    ACTIONS(33),
    1,
    anon_sym_function,
    ACTIONS(35),
    1,
    anon_sym_local,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(202),
    1,
    ts_builtin_sym_end,
    STATE(21),
    1,
    sym_comment,
    STATE(22),
    1,
    aux_sym_chunk_repeat1,
    STATE(41),
    1,
    sym_variable,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(100),
    1,
    sym_expression,
    STATE(284),
    1,
    sym_variable_declaration,
    STATE(286),
    1,
    sym__local_function_declaration,
    STATE(287),
    1,
    sym_function_declaration,
    STATE(296),
    1,
    sym_declaration,
    STATE(298),
    1,
    sym_statement,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    STATE(404),
    1,
    sym_return_statement,
    STATE(439),
    1,
    sym__variable_assignment_varlist,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(61),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(293),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [2316] = 42,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(114),
    1,
    anon_sym_return,
    ACTIONS(204),
    1,
    ts_builtin_sym_end,
    ACTIONS(206),
    1,
    sym_identifier,
    ACTIONS(209),
    1,
    anon_sym_SEMI,
    ACTIONS(212),
    1,
    anon_sym_COLON_COLON,
    ACTIONS(215),
    1,
    sym_break_statement,
    ACTIONS(218),
    1,
    anon_sym_goto,
    ACTIONS(221),
    1,
    anon_sym_do,
    ACTIONS(224),
    1,
    anon_sym_while,
    ACTIONS(227),
    1,
    anon_sym_repeat,
    ACTIONS(230),
    1,
    anon_sym_if,
    ACTIONS(233),
    1,
    anon_sym_for,
    ACTIONS(236),
    1,
    anon_sym_function,
    ACTIONS(239),
    1,
    anon_sym_local,
    ACTIONS(248),
    1,
    anon_sym_DQUOTE,
    ACTIONS(251),
    1,
    anon_sym_SQUOTE,
    ACTIONS(254),
    1,
    anon_sym_LPAREN,
    ACTIONS(257),
    1,
    anon_sym_LBRACE,
    ACTIONS(266),
    1,
    sym__block_string_start,
    STATE(41),
    1,
    sym_variable,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(100),
    1,
    sym_expression,
    STATE(284),
    1,
    sym_variable_declaration,
    STATE(286),
    1,
    sym__local_function_declaration,
    STATE(287),
    1,
    sym_function_declaration,
    STATE(296),
    1,
    sym_declaration,
    STATE(298),
    1,
    sym_statement,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    STATE(439),
    1,
    sym__variable_assignment_varlist,
    ACTIONS(245),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(260),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(263),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(22),
    2,
    sym_comment,
    aux_sym_chunk_repeat1,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(61),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(242),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    STATE(293),
    9,
    sym_empty_statement,
    sym_assignment_statement,
    sym_label_statement,
    sym_goto_statement,
    sym_do_statement,
    sym_while_statement,
    sym_repeat_statement,
    sym_if_statement,
    sym_for_statement,
    [2463] = 10,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(273),
    1,
    anon_sym_EQ,
    ACTIONS(275),
    1,
    anon_sym_COMMA,
    STATE(23),
    1,
    sym_comment,
    STATE(400),
    1,
    aux_sym__variable_assignment_varlist_repeat1,
    ACTIONS(277),
    2,
    anon_sym_DOT,
    anon_sym_COLON,
    ACTIONS(279),
    6,
    sym__block_string_start,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    ACTIONS(271),
    18,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    sym_vararg_expression,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(269),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2543] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(24),
    1,
    sym_comment,
    ACTIONS(283),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(281),
    30,
    anon_sym_return,
    anon_sym_EQ,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2612] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(25),
    1,
    sym_comment,
    ACTIONS(287),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(285),
    30,
    anon_sym_return,
    anon_sym_EQ,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2681] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(26),
    1,
    sym_comment,
    ACTIONS(291),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(289),
    30,
    anon_sym_return,
    anon_sym_EQ,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2750] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(27),
    1,
    sym_comment,
    ACTIONS(295),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(293),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2818] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(28),
    1,
    sym_comment,
    ACTIONS(299),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(297),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2886] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(29),
    1,
    sym_comment,
    ACTIONS(299),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(297),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [2954] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(30),
    1,
    sym_comment,
    ACTIONS(277),
    2,
    anon_sym_DOT,
    anon_sym_COLON,
    ACTIONS(279),
    6,
    sym__block_string_start,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    ACTIONS(271),
    19,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    sym_vararg_expression,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(269),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3026] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(31),
    1,
    sym_comment,
    ACTIONS(303),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(301),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3094] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(32),
    1,
    sym_comment,
    ACTIONS(307),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(305),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3162] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(33),
    1,
    sym_comment,
    ACTIONS(311),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(309),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3230] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(34),
    1,
    sym_comment,
    ACTIONS(315),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(313),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3298] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(35),
    1,
    sym_comment,
    ACTIONS(319),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(317),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3366] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(36),
    1,
    sym_comment,
    ACTIONS(323),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(321),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3434] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(37),
    1,
    sym_comment,
    ACTIONS(327),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(325),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3502] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(38),
    1,
    sym_comment,
    ACTIONS(331),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(329),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3570] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(39),
    1,
    sym_comment,
    ACTIONS(335),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(333),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3638] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(40),
    1,
    sym_comment,
    ACTIONS(339),
    25,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(337),
    29,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3706] = 10,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(273),
    1,
    anon_sym_EQ,
    ACTIONS(275),
    1,
    anon_sym_COMMA,
    STATE(41),
    1,
    sym_comment,
    STATE(400),
    1,
    aux_sym__variable_assignment_varlist_repeat1,
    ACTIONS(277),
    2,
    anon_sym_DOT,
    anon_sym_COLON,
    ACTIONS(279),
    6,
    sym__block_string_start,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    ACTIONS(271),
    19,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    sym_vararg_expression,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(269),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [3783] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(42),
    1,
    sym_comment,
    ACTIONS(281),
    26,
    anon_sym_return,
    anon_sym_EQ,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(283),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [3849] = 21,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(345),
    1,
    anon_sym_COMMA,
    ACTIONS(349),
    1,
    anon_sym_or,
    ACTIONS(351),
    1,
    anon_sym_and,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(43),
    1,
    sym_comment,
    STATE(159),
    1,
    aux_sym__variable_assignment_explist_repeat1,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(343),
    10,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(341),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [3947] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(44),
    1,
    sym_comment,
    ACTIONS(285),
    26,
    anon_sym_return,
    anon_sym_EQ,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(287),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [4013] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(45),
    1,
    sym_comment,
    ACTIONS(289),
    26,
    anon_sym_return,
    anon_sym_EQ,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(291),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [4079] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(46),
    1,
    sym_comment,
    ACTIONS(377),
    23,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_POUND,
    ACTIONS(375),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [4146] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(47),
    1,
    sym_comment,
    ACTIONS(381),
    23,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_POUND,
    ACTIONS(379),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [4213] = 12,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(48),
    1,
    sym_comment,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    17,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_POUND,
    ACTIONS(379),
    24,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    [4292] = 13,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(49),
    1,
    sym_comment,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    16,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_POUND,
    ACTIONS(379),
    24,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    [4373] = 14,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(50),
    1,
    sym_comment,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    16,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_POUND,
    ACTIONS(379),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_not,
    sym_identifier,
    [4456] = 17,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(51),
    1,
    sym_comment,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(381),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(379),
    21,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_not,
    sym_identifier,
    [4545] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(351),
    1,
    anon_sym_and,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(52),
    1,
    sym_comment,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(381),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(379),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_not,
    sym_identifier,
    [4636] = 15,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(53),
    1,
    sym_comment,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    15,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_POUND,
    ACTIONS(379),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_not,
    sym_identifier,
    [4721] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(54),
    1,
    sym_comment,
    ACTIONS(297),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(299),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [4786] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(55),
    1,
    sym_comment,
    ACTIONS(297),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(299),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [4851] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(56),
    1,
    sym_comment,
    ACTIONS(385),
    24,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(383),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [4916] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(57),
    1,
    sym_comment,
    ACTIONS(309),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(311),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [4981] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(58),
    1,
    sym_comment,
    ACTIONS(389),
    24,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(387),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [5046] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(59),
    1,
    sym_comment,
    ACTIONS(333),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(335),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5111] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(60),
    1,
    sym_comment,
    ACTIONS(329),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(331),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5176] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(61),
    1,
    sym_comment,
    ACTIONS(277),
    2,
    anon_sym_DOT,
    anon_sym_COLON,
    ACTIONS(279),
    6,
    sym__block_string_start,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    ACTIONS(271),
    20,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    sym_vararg_expression,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(269),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [5245] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(62),
    1,
    sym_comment,
    ACTIONS(337),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(339),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5310] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(63),
    1,
    sym_comment,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    19,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_POUND,
    ACTIONS(379),
    24,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    [5387] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(64),
    1,
    sym_comment,
    ACTIONS(393),
    24,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(391),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [5452] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(65),
    1,
    sym_comment,
    ACTIONS(317),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(319),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5517] = 8,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(66),
    1,
    sym_comment,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    20,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_POUND,
    ACTIONS(379),
    26,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [5588] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(67),
    1,
    sym_comment,
    ACTIONS(381),
    23,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_POUND,
    ACTIONS(379),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [5655] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(68),
    1,
    sym_comment,
    ACTIONS(293),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(295),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5720] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(69),
    1,
    sym_comment,
    ACTIONS(321),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(323),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5785] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(70),
    1,
    sym_comment,
    ACTIONS(313),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(315),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [5850] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(71),
    1,
    sym_comment,
    ACTIONS(271),
    24,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    ACTIONS(269),
    27,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [5915] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(72),
    1,
    sym_comment,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    19,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_POUND,
    ACTIONS(379),
    24,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    [5992] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(73),
    1,
    sym_comment,
    ACTIONS(305),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(307),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [6057] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(74),
    1,
    sym_comment,
    ACTIONS(325),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(327),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [6122] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(75),
    1,
    sym_comment,
    ACTIONS(301),
    25,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(303),
    26,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [6187] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(349),
    1,
    anon_sym_or,
    ACTIONS(351),
    1,
    anon_sym_and,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(76),
    1,
    sym_comment,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(397),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(395),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [6280] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(349),
    1,
    anon_sym_or,
    ACTIONS(351),
    1,
    anon_sym_and,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(77),
    1,
    sym_comment,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(401),
    10,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(399),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [6372] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(349),
    1,
    anon_sym_or,
    ACTIONS(351),
    1,
    anon_sym_and,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(78),
    1,
    sym_comment,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(405),
    10,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(403),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [6464] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(349),
    1,
    anon_sym_or,
    ACTIONS(351),
    1,
    anon_sym_and,
    ACTIONS(355),
    1,
    anon_sym_PIPE,
    ACTIONS(357),
    1,
    anon_sym_TILDE,
    ACTIONS(359),
    1,
    anon_sym_AMP,
    ACTIONS(363),
    1,
    anon_sym_PLUS,
    ACTIONS(365),
    1,
    anon_sym_DASH,
    ACTIONS(369),
    1,
    anon_sym_SLASH,
    ACTIONS(371),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(373),
    1,
    anon_sym_CARET,
    STATE(79),
    1,
    sym_comment,
    ACTIONS(347),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(361),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(367),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(353),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(409),
    10,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(407),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [6556] = 21,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(411),
    1,
    anon_sym_COMMA,
    ACTIONS(415),
    1,
    anon_sym_or,
    ACTIONS(417),
    1,
    anon_sym_and,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(80),
    1,
    sym_comment,
    STATE(268),
    1,
    aux_sym__variable_assignment_explist_repeat1,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(343),
    11,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(341),
    15,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [6651] = 8,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(81),
    1,
    sym_comment,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    21,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_POUND,
    ACTIONS(379),
    22,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    [6719] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(82),
    1,
    sym_comment,
    ACTIONS(391),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(393),
    25,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [6781] = 14,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(83),
    1,
    sym_comment,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    17,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_POUND,
    ACTIONS(379),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_not,
    sym_identifier,
    [6861] = 17,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(84),
    1,
    sym_comment,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(381),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(379),
    17,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_not,
    sym_identifier,
    [6947] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(417),
    1,
    anon_sym_and,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(85),
    1,
    sym_comment,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(381),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(379),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_not,
    sym_identifier,
    [7035] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(415),
    1,
    anon_sym_or,
    ACTIONS(417),
    1,
    anon_sym_and,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(86),
    1,
    sym_comment,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(397),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(395),
    15,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [7125] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(87),
    1,
    sym_comment,
    ACTIONS(375),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(377),
    24,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_POUND,
    [7189] = 12,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(88),
    1,
    sym_comment,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    18,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_POUND,
    ACTIONS(379),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    [7265] = 15,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(89),
    1,
    sym_comment,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    16,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_POUND,
    ACTIONS(379),
    19,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_not,
    sym_identifier,
    [7347] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(90),
    1,
    sym_comment,
    ACTIONS(387),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(389),
    25,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [7409] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(91),
    1,
    sym_comment,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    ACTIONS(381),
    20,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_POUND,
    [7483] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(92),
    1,
    sym_comment,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    ACTIONS(381),
    20,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_POUND,
    [7557] = 13,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(93),
    1,
    sym_comment,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    17,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_POUND,
    ACTIONS(379),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_not,
    sym_identifier,
    [7635] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(94),
    1,
    sym_comment,
    ACTIONS(383),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(385),
    25,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [7697] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(95),
    1,
    sym_comment,
    ACTIONS(379),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(381),
    24,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_POUND,
    [7761] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(96),
    1,
    sym_comment,
    ACTIONS(379),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(381),
    24,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_POUND,
    [7825] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(97),
    1,
    sym_comment,
    ACTIONS(269),
    23,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    anon_sym_LT,
    anon_sym_GT,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_or,
    anon_sym_and,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    anon_sym_DOT_DOT,
    anon_sym_not,
    sym_identifier,
    ACTIONS(271),
    25,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_CARET,
    anon_sym_POUND,
    [7887] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(415),
    1,
    anon_sym_or,
    ACTIONS(417),
    1,
    anon_sym_and,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(98),
    1,
    sym_comment,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(405),
    11,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(403),
    15,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [7976] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(415),
    1,
    anon_sym_or,
    ACTIONS(417),
    1,
    anon_sym_and,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(99),
    1,
    sym_comment,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(401),
    11,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(399),
    15,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [8065] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(415),
    1,
    anon_sym_or,
    ACTIONS(417),
    1,
    anon_sym_and,
    ACTIONS(421),
    1,
    anon_sym_PIPE,
    ACTIONS(423),
    1,
    anon_sym_TILDE,
    ACTIONS(425),
    1,
    anon_sym_AMP,
    ACTIONS(429),
    1,
    anon_sym_PLUS,
    ACTIONS(431),
    1,
    anon_sym_DASH,
    ACTIONS(435),
    1,
    anon_sym_SLASH,
    ACTIONS(437),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(439),
    1,
    anon_sym_CARET,
    STATE(100),
    1,
    sym_comment,
    ACTIONS(413),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(427),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(433),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(419),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(409),
    11,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_POUND,
    ACTIONS(407),
    15,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_not,
    sym_identifier,
    [8154] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(101),
    1,
    sym_comment,
    ACTIONS(325),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(327),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8209] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(102),
    1,
    sym_comment,
    ACTIONS(317),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(319),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8264] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(103),
    1,
    sym_comment,
    ACTIONS(293),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(295),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8319] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(104),
    1,
    sym_comment,
    ACTIONS(301),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(303),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8374] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(105),
    1,
    sym_comment,
    ACTIONS(337),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(339),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8429] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(277),
    1,
    anon_sym_DOT,
    STATE(106),
    1,
    sym_comment,
    ACTIONS(269),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(279),
    7,
    sym__block_string_start,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    ACTIONS(271),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8488] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(107),
    1,
    sym_comment,
    ACTIONS(305),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(307),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8543] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(108),
    1,
    sym_comment,
    ACTIONS(313),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(315),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8598] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(109),
    1,
    sym_comment,
    ACTIONS(285),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(287),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8653] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(110),
    1,
    sym_comment,
    ACTIONS(289),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(291),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8708] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(111),
    1,
    sym_comment,
    ACTIONS(281),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(283),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8763] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(112),
    1,
    sym_comment,
    ACTIONS(333),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(335),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8818] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(113),
    1,
    sym_comment,
    ACTIONS(321),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(323),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8873] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(114),
    1,
    sym_comment,
    ACTIONS(309),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(311),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8928] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(115),
    1,
    sym_comment,
    ACTIONS(329),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(331),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [8983] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(116),
    1,
    sym_comment,
    ACTIONS(297),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(299),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [9038] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(117),
    1,
    sym_comment,
    ACTIONS(297),
    7,
    anon_sym_else,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(299),
    34,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_RPAREN,
    anon_sym_LBRACK,
    anon_sym_RBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [9093] = 25,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(443),
    1,
    anon_sym_SEMI,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(118),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(269),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(338),
    1,
    sym__expression_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    ACTIONS(445),
    4,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    anon_sym_else,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9184] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(471),
    1,
    anon_sym_COMMA,
    ACTIONS(473),
    1,
    anon_sym_LT,
    STATE(119),
    1,
    sym_comment,
    STATE(125),
    1,
    aux_sym__att_name_list_repeat1,
    STATE(131),
    1,
    sym__attrib,
    ACTIONS(469),
    12,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(467),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [9242] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(473),
    1,
    anon_sym_LT,
    STATE(120),
    1,
    sym_comment,
    STATE(160),
    1,
    sym__attrib,
    ACTIONS(477),
    13,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(475),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [9295] = 26,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    ACTIONS(483),
    1,
    anon_sym_RBRACE,
    STATE(121),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(348),
    1,
    sym_field,
    STATE(449),
    1,
    sym__field_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9386] = 26,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    ACTIONS(485),
    1,
    anon_sym_RBRACE,
    STATE(122),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(348),
    1,
    sym_field,
    STATE(418),
    1,
    sym__field_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9477] = 26,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    ACTIONS(487),
    1,
    anon_sym_RBRACE,
    STATE(123),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(348),
    1,
    sym_field,
    STATE(406),
    1,
    sym__field_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9568] = 26,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    ACTIONS(489),
    1,
    anon_sym_RBRACE,
    STATE(124),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(348),
    1,
    sym_field,
    STATE(462),
    1,
    sym__field_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9659] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(471),
    1,
    anon_sym_COMMA,
    STATE(125),
    1,
    sym_comment,
    STATE(128),
    1,
    aux_sym__att_name_list_repeat1,
    ACTIONS(493),
    12,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(491),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [9711] = 25,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(443),
    1,
    anon_sym_SEMI,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(495),
    1,
    ts_builtin_sym_end,
    STATE(126),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(269),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(338),
    1,
    sym__expression_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9799] = 25,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    ACTIONS(497),
    1,
    anon_sym_RBRACE,
    STATE(127),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(395),
    1,
    sym_field,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [9887] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(503),
    1,
    anon_sym_COMMA,
    STATE(128),
    2,
    sym_comment,
    aux_sym__att_name_list_repeat1,
    ACTIONS(501),
    12,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(499),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [9937] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(471),
    1,
    anon_sym_COMMA,
    STATE(128),
    1,
    aux_sym__att_name_list_repeat1,
    STATE(129),
    1,
    sym_comment,
    ACTIONS(508),
    12,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(506),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [9989] = 25,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    ACTIONS(510),
    1,
    anon_sym_RBRACE,
    STATE(130),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(395),
    1,
    sym_field,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [10077] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(471),
    1,
    anon_sym_COMMA,
    STATE(129),
    1,
    aux_sym__att_name_list_repeat1,
    STATE(131),
    1,
    sym_comment,
    ACTIONS(514),
    12,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(512),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [10129] = 15,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(132),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(379),
    3,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    17,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [10196] = 24,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(479),
    1,
    sym_identifier,
    ACTIONS(481),
    1,
    anon_sym_LBRACK,
    STATE(133),
    1,
    sym_comment,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(303),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(395),
    1,
    sym_field,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [10281] = 14,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(134),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(379),
    3,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(381),
    18,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    [10346] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(135),
    1,
    sym_comment,
    ACTIONS(379),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(381),
    26,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    [10395] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(136),
    1,
    sym_comment,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    4,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    ACTIONS(381),
    21,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    [10454] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(137),
    1,
    sym_comment,
    ACTIONS(297),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(299),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [10501] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(138),
    1,
    sym_comment,
    ACTIONS(297),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(299),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [10548] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(139),
    1,
    sym_comment,
    ACTIONS(337),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(339),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [10595] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(140),
    1,
    sym_comment,
    ACTIONS(379),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(381),
    26,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    [10644] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(379),
    1,
    anon_sym_else,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    STATE(141),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(381),
    12,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    [10717] = 8,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(142),
    1,
    sym_comment,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    5,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    ACTIONS(381),
    23,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_DOT_DOT,
    [10770] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(143),
    1,
    sym_comment,
    ACTIONS(269),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(271),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [10817] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(144),
    1,
    sym_comment,
    ACTIONS(329),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(331),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [10864] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(145),
    1,
    sym_comment,
    ACTIONS(387),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(389),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [10911] = 17,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(379),
    1,
    anon_sym_else,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(146),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(381),
    13,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    [10982] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(542),
    1,
    anon_sym_COMMA,
    ACTIONS(544),
    1,
    anon_sym_LT,
    STATE(147),
    1,
    sym_comment,
    STATE(257),
    1,
    aux_sym__att_name_list_repeat1,
    STATE(258),
    1,
    sym__attrib,
    ACTIONS(469),
    13,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(467),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [11037] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(148),
    1,
    sym_comment,
    ACTIONS(383),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(385),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [11084] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(149),
    1,
    sym_comment,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    4,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    ACTIONS(381),
    21,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    [11143] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(550),
    1,
    anon_sym_COMMA,
    STATE(150),
    2,
    sym_comment,
    aux_sym__variable_assignment_explist_repeat1,
    ACTIONS(548),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(546),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [11192] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(151),
    1,
    sym_comment,
    ACTIONS(391),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(393),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [11239] = 12,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(152),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    4,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    ACTIONS(381),
    19,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    [11300] = 13,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(153),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(379),
    4,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    ACTIONS(381),
    18,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    [11363] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(154),
    1,
    sym_comment,
    ACTIONS(325),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(327),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [11410] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(155),
    1,
    sym_comment,
    ACTIONS(301),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(303),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [11457] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(156),
    1,
    sym_comment,
    ACTIONS(305),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(307),
    27,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [11504] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    STATE(157),
    1,
    sym_comment,
    ACTIONS(375),
    6,
    anon_sym_else,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(377),
    26,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_then,
    anon_sym_elseif,
    anon_sym_RPAREN,
    anon_sym_RBRACK,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    [11553] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(553),
    1,
    anon_sym_EQ,
    STATE(158),
    1,
    sym_comment,
    ACTIONS(285),
    6,
    anon_sym_DOT,
    anon_sym_LT,
    anon_sym_GT,
    anon_sym_TILDE,
    anon_sym_DASH,
    anon_sym_SLASH,
    ACTIONS(287),
    26,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_or,
    anon_sym_and,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    anon_sym_PIPE,
    anon_sym_AMP,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    anon_sym_PLUS,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    anon_sym_DOT_DOT,
    anon_sym_CARET,
    [11602] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(345),
    1,
    anon_sym_COMMA,
    STATE(150),
    1,
    aux_sym__variable_assignment_explist_repeat1,
    STATE(159),
    1,
    sym_comment,
    ACTIONS(557),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(555),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [11653] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(160),
    1,
    sym_comment,
    ACTIONS(561),
    13,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(559),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [11700] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(161),
    1,
    sym_comment,
    ACTIONS(565),
    13,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(563),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [11747] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(571),
    1,
    anon_sym_EQ,
    STATE(162),
    1,
    sym_comment,
    ACTIONS(569),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(567),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [11795] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(80),
    1,
    sym_expression,
    STATE(163),
    1,
    sym_comment,
    STATE(271),
    1,
    sym__variable_assignment_explist,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [11877] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(43),
    1,
    sym_expression,
    STATE(164),
    1,
    sym_comment,
    STATE(228),
    1,
    sym__variable_assignment_explist,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [11959] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(165),
    1,
    sym_comment,
    STATE(269),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    STATE(454),
    1,
    sym__expression_list,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12041] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(577),
    1,
    anon_sym_RPAREN,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(166),
    1,
    sym_comment,
    STATE(301),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12123] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(43),
    1,
    sym_expression,
    STATE(167),
    1,
    sym_comment,
    STATE(210),
    1,
    sym__variable_assignment_explist,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12205] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(80),
    1,
    sym_expression,
    STATE(168),
    1,
    sym_comment,
    STATE(278),
    1,
    sym__variable_assignment_explist,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12287] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(579),
    1,
    anon_sym_RPAREN,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(169),
    1,
    sym_comment,
    STATE(302),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12369] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(544),
    1,
    anon_sym_LT,
    STATE(170),
    1,
    sym_comment,
    STATE(264),
    1,
    sym__attrib,
    ACTIONS(477),
    14,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(475),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [12419] = 23,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    ACTIONS(581),
    1,
    anon_sym_RPAREN,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(171),
    1,
    sym_comment,
    STATE(306),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12501] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(49),
    1,
    sym_expression,
    STATE(172),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12580] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(76),
    1,
    sym_expression,
    STATE(173),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12659] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(174),
    1,
    sym_comment,
    STATE(315),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12738] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(175),
    1,
    sym_comment,
    STATE(317),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12817] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(176),
    1,
    sym_comment,
    STATE(320),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12896] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(177),
    1,
    sym_comment,
    STATE(309),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [12975] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(178),
    1,
    sym_comment,
    STATE(321),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13054] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(179),
    1,
    sym_comment,
    STATE(308),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13133] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(180),
    1,
    sym_comment,
    STATE(310),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13212] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(181),
    1,
    sym_comment,
    STATE(316),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13291] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(182),
    1,
    sym_comment,
    STATE(318),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13370] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(183),
    1,
    sym_comment,
    STATE(311),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13449] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(87),
    1,
    sym_expression,
    STATE(184),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13528] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(99),
    1,
    sym_expression,
    STATE(185),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13607] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(47),
    1,
    sym_expression,
    STATE(186),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13686] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(72),
    1,
    sym_expression,
    STATE(187),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13765] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(67),
    1,
    sym_expression,
    STATE(188),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13844] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(66),
    1,
    sym_expression,
    STATE(189),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [13923] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(63),
    1,
    sym_expression,
    STATE(190),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14002] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(48),
    1,
    sym_expression,
    STATE(191),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14081] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(50),
    1,
    sym_expression,
    STATE(192),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14160] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(51),
    1,
    sym_expression,
    STATE(193),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14239] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(52),
    1,
    sym_expression,
    STATE(194),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14318] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(53),
    1,
    sym_expression,
    STATE(195),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14397] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(46),
    1,
    sym_expression,
    STATE(196),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [14476] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(197),
    1,
    sym_comment,
    ACTIONS(585),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(583),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14521] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(198),
    1,
    sym_comment,
    ACTIONS(589),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(587),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14566] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(199),
    1,
    sym_comment,
    ACTIONS(593),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(591),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14611] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(542),
    1,
    anon_sym_COMMA,
    STATE(200),
    1,
    sym_comment,
    STATE(201),
    1,
    aux_sym__att_name_list_repeat1,
    ACTIONS(508),
    13,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(506),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14660] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(595),
    1,
    anon_sym_COMMA,
    STATE(201),
    2,
    sym_comment,
    aux_sym__att_name_list_repeat1,
    ACTIONS(501),
    13,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(499),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14707] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(202),
    1,
    sym_comment,
    ACTIONS(600),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(598),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14752] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(203),
    1,
    sym_comment,
    ACTIONS(604),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(602),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14797] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(204),
    1,
    sym_comment,
    ACTIONS(608),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(606),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14842] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(205),
    1,
    sym_comment,
    ACTIONS(612),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(610),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14887] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(206),
    1,
    sym_comment,
    ACTIONS(616),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(614),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14932] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(207),
    1,
    sym_comment,
    ACTIONS(620),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(618),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [14977] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(86),
    1,
    sym_expression,
    STATE(208),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15056] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(209),
    1,
    sym_comment,
    STATE(319),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15135] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(210),
    1,
    sym_comment,
    ACTIONS(624),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(622),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [15180] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(89),
    1,
    sym_expression,
    STATE(211),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15259] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(212),
    1,
    sym_comment,
    ACTIONS(628),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(626),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [15304] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(85),
    1,
    sym_expression,
    STATE(213),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15383] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(214),
    1,
    sym_comment,
    ACTIONS(632),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(630),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [15428] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(215),
    1,
    sym_comment,
    ACTIONS(636),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(634),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [15473] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(84),
    1,
    sym_expression,
    STATE(216),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15552] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(217),
    1,
    sym_comment,
    ACTIONS(640),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(638),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [15597] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(83),
    1,
    sym_expression,
    STATE(218),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15676] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(93),
    1,
    sym_expression,
    STATE(219),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15755] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(88),
    1,
    sym_expression,
    STATE(220),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15834] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(92),
    1,
    sym_expression,
    STATE(221),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15913] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(81),
    1,
    sym_expression,
    STATE(222),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [15992] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(223),
    1,
    sym_comment,
    STATE(307),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16071] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(95),
    1,
    sym_expression,
    STATE(224),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16150] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(91),
    1,
    sym_expression,
    STATE(225),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16229] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(96),
    1,
    sym_expression,
    STATE(226),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16308] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(227),
    1,
    sym_comment,
    STATE(312),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16387] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(228),
    1,
    sym_comment,
    ACTIONS(644),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(642),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [16432] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(229),
    1,
    sym_comment,
    STATE(305),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16511] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(230),
    1,
    sym_comment,
    STATE(266),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16590] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(231),
    1,
    sym_comment,
    ACTIONS(648),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(646),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [16635] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(78),
    1,
    sym_expression,
    STATE(232),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16714] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(9),
    1,
    sym_identifier,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(573),
    1,
    anon_sym_function,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(98),
    1,
    sym_expression,
    STATE(233),
    1,
    sym_comment,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(329),
    1,
    sym_method_index_expression,
    ACTIONS(39),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(49),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(51),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(37),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(61),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(97),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16793] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(234),
    1,
    sym_comment,
    STATE(304),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [16872] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(235),
    1,
    sym_comment,
    ACTIONS(652),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(650),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [16917] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(236),
    1,
    sym_comment,
    ACTIONS(656),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(654),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [16962] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(237),
    1,
    sym_comment,
    STATE(314),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17041] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(135),
    1,
    sym_expression,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(238),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17120] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(136),
    1,
    sym_expression,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(239),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17199] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(140),
    1,
    sym_expression,
    STATE(240),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17278] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(142),
    1,
    sym_expression,
    STATE(241),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17357] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(149),
    1,
    sym_expression,
    STATE(242),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17436] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(152),
    1,
    sym_expression,
    STATE(243),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17515] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(244),
    1,
    sym_comment,
    STATE(313),
    1,
    sym_expression,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17594] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(153),
    1,
    sym_expression,
    STATE(245),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17673] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(134),
    1,
    sym_expression,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(246),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17752] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(247),
    1,
    sym_comment,
    ACTIONS(660),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(658),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [17797] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(248),
    1,
    sym_comment,
    ACTIONS(409),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(407),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [17842] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(146),
    1,
    sym_expression,
    STATE(249),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [17921] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(157),
    1,
    sym_expression,
    STATE(250),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [18000] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(141),
    1,
    sym_expression,
    STATE(251),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [18079] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(252),
    1,
    sym_comment,
    ACTIONS(664),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(662),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18124] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(253),
    1,
    sym_comment,
    ACTIONS(668),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(666),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18169] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(254),
    1,
    sym_comment,
    ACTIONS(672),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(670),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18214] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(255),
    1,
    sym_comment,
    ACTIONS(676),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(674),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18259] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(256),
    1,
    sym_comment,
    ACTIONS(680),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(678),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18304] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(542),
    1,
    anon_sym_COMMA,
    STATE(201),
    1,
    aux_sym__att_name_list_repeat1,
    STATE(257),
    1,
    sym_comment,
    ACTIONS(493),
    13,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(491),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18353] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(542),
    1,
    anon_sym_COMMA,
    STATE(200),
    1,
    aux_sym__att_name_list_repeat1,
    STATE(258),
    1,
    sym_comment,
    ACTIONS(514),
    13,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(512),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18402] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(441),
    1,
    sym_identifier,
    ACTIONS(447),
    1,
    anon_sym_function,
    ACTIONS(453),
    1,
    anon_sym_DQUOTE,
    ACTIONS(455),
    1,
    anon_sym_SQUOTE,
    ACTIONS(457),
    1,
    anon_sym_LPAREN,
    ACTIONS(459),
    1,
    anon_sym_LBRACE,
    ACTIONS(465),
    1,
    sym__block_string_start,
    STATE(132),
    1,
    sym_expression,
    STATE(137),
    1,
    sym__block_string,
    STATE(138),
    1,
    sym__quote_string,
    STATE(259),
    1,
    sym_comment,
    STATE(324),
    1,
    sym__prefix_expression,
    STATE(326),
    1,
    sym_method_index_expression,
    ACTIONS(451),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(461),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(463),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(109),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(449),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(106),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(143),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [18481] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(260),
    1,
    sym_comment,
    ACTIONS(684),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(682),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18526] = 22,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(55),
    1,
    sym_identifier,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(95),
    1,
    anon_sym_LPAREN,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(575),
    1,
    anon_sym_function,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(77),
    1,
    sym_expression,
    STATE(261),
    1,
    sym_comment,
    STATE(323),
    1,
    sym__prefix_expression,
    STATE(328),
    1,
    sym_method_index_expression,
    ACTIONS(89),
    2,
    sym_number,
    sym_vararg_expression,
    ACTIONS(99),
    2,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(101),
    2,
    anon_sym_DASH,
    anon_sym_not,
    STATE(25),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    ACTIONS(87),
    3,
    sym_nil,
    sym_false,
    sym_true,
    STATE(30),
    3,
    sym_variable,
    sym_function_call,
    sym_parenthesized_expression,
    STATE(71),
    5,
    sym_string,
    sym_function_definition,
    sym_table_constructor,
    sym_binary_expression,
    sym_unary_expression,
    [18605] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(262),
    1,
    sym_comment,
    ACTIONS(688),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(686),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18650] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(263),
    1,
    sym_comment,
    ACTIONS(692),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(690),
    20,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_until,
    anon_sym_if,
    anon_sym_elseif,
    anon_sym_else,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18695] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(264),
    1,
    sym_comment,
    ACTIONS(561),
    14,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(559),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18739] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(265),
    1,
    sym_comment,
    ACTIONS(565),
    14,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_EQ,
    anon_sym_COMMA,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(563),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18783] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(696),
    1,
    anon_sym_else,
    ACTIONS(698),
    1,
    anon_sym_or,
    STATE(266),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(694),
    8,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    anon_sym_RPAREN,
    [18855] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(700),
    1,
    anon_sym_COMMA,
    STATE(267),
    2,
    sym_comment,
    aux_sym__variable_assignment_explist_repeat1,
    ACTIONS(548),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(546),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18901] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(411),
    1,
    anon_sym_COMMA,
    STATE(267),
    1,
    aux_sym__variable_assignment_explist_repeat1,
    STATE(268),
    1,
    sym_comment,
    ACTIONS(557),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(555),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [18949] = 21,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(707),
    1,
    anon_sym_else,
    STATE(269),
    1,
    sym_comment,
    STATE(332),
    1,
    aux_sym__expression_list_repeat1,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    ACTIONS(703),
    6,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [19025] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(709),
    1,
    anon_sym_EQ,
    STATE(270),
    1,
    sym_comment,
    ACTIONS(569),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(567),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19070] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(271),
    1,
    sym_comment,
    ACTIONS(644),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(642),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19112] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(272),
    1,
    sym_comment,
    ACTIONS(632),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(630),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19154] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(273),
    1,
    sym_comment,
    ACTIONS(688),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(686),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19196] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(274),
    1,
    sym_comment,
    ACTIONS(713),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(711),
    17,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19238] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(275),
    1,
    sym_comment,
    ACTIONS(640),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(638),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19280] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(276),
    1,
    sym_comment,
    ACTIONS(628),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(626),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19322] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(277),
    1,
    sym_comment,
    ACTIONS(604),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(602),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19364] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(278),
    1,
    sym_comment,
    ACTIONS(624),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(622),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19406] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(279),
    1,
    sym_comment,
    ACTIONS(585),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(583),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19448] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(280),
    1,
    sym_comment,
    ACTIONS(612),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(610),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19490] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(281),
    1,
    sym_comment,
    ACTIONS(660),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(658),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19532] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(282),
    1,
    sym_comment,
    ACTIONS(636),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(634),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19574] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(283),
    1,
    sym_comment,
    ACTIONS(589),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(587),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19616] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(284),
    1,
    sym_comment,
    ACTIONS(680),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(678),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19658] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(285),
    1,
    sym_comment,
    ACTIONS(717),
    11,
    sym__block_string_start,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(715),
    17,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_end,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19700] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(286),
    1,
    sym_comment,
    ACTIONS(676),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(674),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19742] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(287),
    1,
    sym_comment,
    ACTIONS(672),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(670),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19784] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(288),
    1,
    sym_comment,
    ACTIONS(593),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(591),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19826] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(289),
    1,
    sym_comment,
    ACTIONS(600),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(598),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19868] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(290),
    1,
    sym_comment,
    ACTIONS(684),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(682),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19910] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(291),
    1,
    sym_comment,
    ACTIONS(648),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(646),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19952] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(292),
    1,
    sym_comment,
    ACTIONS(616),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(614),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [19994] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(293),
    1,
    sym_comment,
    ACTIONS(409),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(407),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20036] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(294),
    1,
    sym_comment,
    ACTIONS(652),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(650),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20078] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(295),
    1,
    sym_comment,
    ACTIONS(656),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(654),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20120] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(296),
    1,
    sym_comment,
    ACTIONS(668),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(666),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20162] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(297),
    1,
    sym_comment,
    ACTIONS(608),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(606),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20204] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(298),
    1,
    sym_comment,
    ACTIONS(664),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(662),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20246] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(299),
    1,
    sym_comment,
    ACTIONS(620),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(618),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20288] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(300),
    1,
    sym_comment,
    ACTIONS(692),
    12,
    sym__block_string_start,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_COLON_COLON,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    ACTIONS(690),
    16,
    anon_sym_return,
    sym_break_statement,
    anon_sym_goto,
    anon_sym_do,
    anon_sym_while,
    anon_sym_repeat,
    anon_sym_if,
    anon_sym_for,
    anon_sym_function,
    anon_sym_local,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    [20330] = 20,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(719),
    1,
    anon_sym_RPAREN,
    STATE(301),
    1,
    sym_comment,
    STATE(397),
    1,
    aux_sym__expression_list_repeat1,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20398] = 20,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(721),
    1,
    anon_sym_RPAREN,
    STATE(302),
    1,
    sym_comment,
    STATE(399),
    1,
    aux_sym__expression_list_repeat1,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20466] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    STATE(303),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(723),
    3,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_RBRACE,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20530] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    STATE(304),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(725),
    3,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_RBRACE,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20594] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    STATE(305),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(727),
    3,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_RBRACE,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20658] = 20,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(729),
    1,
    anon_sym_RPAREN,
    STATE(306),
    1,
    sym_comment,
    STATE(391),
    1,
    aux_sym__expression_list_repeat1,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20726] = 19,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(731),
    1,
    anon_sym_COMMA,
    ACTIONS(733),
    1,
    anon_sym_do,
    STATE(307),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20791] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(735),
    1,
    anon_sym_RPAREN,
    STATE(308),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20853] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(737),
    1,
    anon_sym_RPAREN,
    STATE(309),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20915] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(739),
    1,
    anon_sym_do,
    STATE(310),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [20977] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(741),
    1,
    anon_sym_RPAREN,
    STATE(311),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21039] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(743),
    1,
    anon_sym_RBRACK,
    STATE(312),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21101] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(745),
    1,
    anon_sym_do,
    STATE(313),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21163] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(747),
    1,
    anon_sym_COMMA,
    STATE(314),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21225] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(749),
    1,
    anon_sym_then,
    STATE(315),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21287] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(751),
    1,
    anon_sym_then,
    STATE(316),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21349] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(753),
    1,
    anon_sym_do,
    STATE(317),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21411] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(755),
    1,
    anon_sym_then,
    STATE(318),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21473] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(757),
    1,
    anon_sym_RBRACK,
    STATE(319),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21535] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(759),
    1,
    anon_sym_RBRACK,
    STATE(320),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21597] = 18,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(516),
    1,
    anon_sym_PIPE,
    ACTIONS(518),
    1,
    anon_sym_TILDE,
    ACTIONS(520),
    1,
    anon_sym_AMP,
    ACTIONS(524),
    1,
    anon_sym_PLUS,
    ACTIONS(526),
    1,
    anon_sym_DASH,
    ACTIONS(530),
    1,
    anon_sym_SLASH,
    ACTIONS(532),
    1,
    anon_sym_DOT_DOT,
    ACTIONS(534),
    1,
    anon_sym_CARET,
    ACTIONS(538),
    1,
    anon_sym_and,
    ACTIONS(698),
    1,
    anon_sym_or,
    ACTIONS(761),
    1,
    anon_sym_RBRACK,
    STATE(321),
    1,
    sym_comment,
    ACTIONS(522),
    2,
    anon_sym_LT_LT,
    anon_sym_GT_GT,
    ACTIONS(536),
    2,
    anon_sym_LT,
    anon_sym_GT,
    ACTIONS(528),
    3,
    anon_sym_STAR,
    anon_sym_SLASH_SLASH,
    anon_sym_PERCENT,
    ACTIONS(540),
    4,
    anon_sym_LT_EQ,
    anon_sym_EQ_EQ,
    anon_sym_TILDE_EQ,
    anon_sym_GT_EQ,
    [21659] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(322),
    1,
    sym_comment,
    ACTIONS(763),
    7,
    anon_sym_function,
    sym_nil,
    sym_false,
    sym_true,
    anon_sym_DASH,
    anon_sym_not,
    sym_identifier,
    ACTIONS(765),
    11,
    sym__block_string_start,
    sym_number,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    sym_vararg_expression,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    anon_sym_RBRACE,
    anon_sym_TILDE,
    anon_sym_POUND,
    [21691] = 15,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(767),
    1,
    anon_sym_DOT,
    ACTIONS(769),
    1,
    anon_sym_COLON,
    ACTIONS(771),
    1,
    anon_sym_LPAREN,
    ACTIONS(773),
    1,
    anon_sym_LBRACK,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(35),
    1,
    sym_arguments,
    STATE(323),
    1,
    sym_comment,
    STATE(27),
    2,
    sym_string,
    sym_table_constructor,
    [21738] = 15,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(769),
    1,
    anon_sym_COLON,
    ACTIONS(775),
    1,
    anon_sym_DOT,
    ACTIONS(777),
    1,
    anon_sym_DQUOTE,
    ACTIONS(779),
    1,
    anon_sym_SQUOTE,
    ACTIONS(781),
    1,
    anon_sym_LPAREN,
    ACTIONS(783),
    1,
    anon_sym_LBRACK,
    ACTIONS(785),
    1,
    anon_sym_LBRACE,
    ACTIONS(787),
    1,
    sym__block_string_start,
    STATE(102),
    1,
    sym_arguments,
    STATE(116),
    1,
    sym__quote_string,
    STATE(117),
    1,
    sym__block_string,
    STATE(324),
    1,
    sym_comment,
    STATE(103),
    2,
    sym_string,
    sym_table_constructor,
    [21785] = 15,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(769),
    1,
    anon_sym_COLON,
    ACTIONS(789),
    1,
    anon_sym_DOT,
    ACTIONS(791),
    1,
    anon_sym_LPAREN,
    ACTIONS(793),
    1,
    anon_sym_LBRACK,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(65),
    1,
    sym_arguments,
    STATE(325),
    1,
    sym_comment,
    STATE(68),
    2,
    sym_string,
    sym_table_constructor,
    [21832] = 12,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(777),
    1,
    anon_sym_DQUOTE,
    ACTIONS(779),
    1,
    anon_sym_SQUOTE,
    ACTIONS(781),
    1,
    anon_sym_LPAREN,
    ACTIONS(785),
    1,
    anon_sym_LBRACE,
    ACTIONS(787),
    1,
    sym__block_string_start,
    STATE(102),
    1,
    sym_arguments,
    STATE(116),
    1,
    sym__quote_string,
    STATE(117),
    1,
    sym__block_string,
    STATE(326),
    1,
    sym_comment,
    STATE(103),
    2,
    sym_string,
    sym_table_constructor,
    [21870] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(327),
    1,
    sym_comment,
    ACTIONS(795),
    2,
    anon_sym_EQ,
    anon_sym_COMMA,
    ACTIONS(279),
    8,
    sym__block_string_start,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    [21894] = 12,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(91),
    1,
    anon_sym_DQUOTE,
    ACTIONS(93),
    1,
    anon_sym_SQUOTE,
    ACTIONS(97),
    1,
    anon_sym_LBRACE,
    ACTIONS(103),
    1,
    sym__block_string_start,
    ACTIONS(771),
    1,
    anon_sym_LPAREN,
    STATE(28),
    1,
    sym__quote_string,
    STATE(29),
    1,
    sym__block_string,
    STATE(35),
    1,
    sym_arguments,
    STATE(328),
    1,
    sym_comment,
    STATE(27),
    2,
    sym_string,
    sym_table_constructor,
    [21932] = 12,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(41),
    1,
    anon_sym_DQUOTE,
    ACTIONS(43),
    1,
    anon_sym_SQUOTE,
    ACTIONS(47),
    1,
    anon_sym_LBRACE,
    ACTIONS(53),
    1,
    sym__block_string_start,
    ACTIONS(791),
    1,
    anon_sym_LPAREN,
    STATE(54),
    1,
    sym__block_string,
    STATE(55),
    1,
    sym__quote_string,
    STATE(65),
    1,
    sym_arguments,
    STATE(329),
    1,
    sym_comment,
    STATE(68),
    2,
    sym_string,
    sym_table_constructor,
    [21970] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(696),
    1,
    anon_sym_else,
    ACTIONS(797),
    1,
    anon_sym_COMMA,
    STATE(330),
    2,
    sym_comment,
    aux_sym__expression_list_repeat1,
    ACTIONS(694),
    7,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    anon_sym_RPAREN,
    [21996] = 10,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(45),
    1,
    anon_sym_LPAREN,
    ACTIONS(800),
    1,
    sym_identifier,
    STATE(325),
    1,
    sym__prefix_expression,
    STATE(327),
    1,
    sym_variable,
    STATE(329),
    1,
    sym_method_index_expression,
    STATE(331),
    1,
    sym_comment,
    STATE(44),
    2,
    sym_bracket_index_expression,
    sym_dot_index_expression,
    STATE(334),
    2,
    sym_function_call,
    sym_parenthesized_expression,
    [22029] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(804),
    1,
    anon_sym_else,
    STATE(330),
    1,
    aux_sym__expression_list_repeat1,
    STATE(332),
    1,
    sym_comment,
    ACTIONS(802),
    6,
    ts_builtin_sym_end,
    anon_sym_SEMI,
    anon_sym_do,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [22056] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(806),
    1,
    sym_identifier,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(18),
    1,
    sym_parameters,
    STATE(64),
    1,
    sym__function_body,
    STATE(333),
    1,
    sym_comment,
    STATE(380),
    1,
    sym__function_name_dot_index_expression,
    STATE(383),
    1,
    sym__function_name_prefix_expression,
    STATE(396),
    1,
    sym__function_name,
    STATE(430),
    1,
    sym__function_name_method_index_expression,
    [22090] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(334),
    1,
    sym_comment,
    ACTIONS(279),
    8,
    sym__block_string_start,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACK,
    anon_sym_LBRACE,
    [22110] = 11,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(806),
    1,
    sym_identifier,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(17),
    1,
    sym_parameters,
    STATE(82),
    1,
    sym__function_body,
    STATE(335),
    1,
    sym_comment,
    STATE(380),
    1,
    sym__function_name_dot_index_expression,
    STATE(383),
    1,
    sym__function_name_prefix_expression,
    STATE(385),
    1,
    sym__function_name,
    STATE(430),
    1,
    sym__function_name_method_index_expression,
    [22144] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(79),
    1,
    anon_sym_else,
    ACTIONS(810),
    1,
    anon_sym_end,
    ACTIONS(812),
    1,
    anon_sym_elseif,
    STATE(336),
    1,
    sym_comment,
    STATE(343),
    1,
    aux_sym_if_statement_repeat1,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(437),
    1,
    sym_else_statement,
    [22172] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(79),
    1,
    anon_sym_else,
    ACTIONS(812),
    1,
    anon_sym_elseif,
    ACTIONS(814),
    1,
    anon_sym_end,
    STATE(337),
    1,
    sym_comment,
    STATE(343),
    1,
    aux_sym_if_statement_repeat1,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(436),
    1,
    sym_else_statement,
    [22200] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(818),
    1,
    anon_sym_SEMI,
    ACTIONS(820),
    1,
    anon_sym_else,
    STATE(338),
    1,
    sym_comment,
    ACTIONS(816),
    4,
    ts_builtin_sym_end,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [22222] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(79),
    1,
    anon_sym_else,
    ACTIONS(812),
    1,
    anon_sym_elseif,
    ACTIONS(822),
    1,
    anon_sym_end,
    STATE(336),
    1,
    aux_sym_if_statement_repeat1,
    STATE(339),
    1,
    sym_comment,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(434),
    1,
    sym_else_statement,
    [22250] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(79),
    1,
    anon_sym_else,
    ACTIONS(812),
    1,
    anon_sym_elseif,
    ACTIONS(824),
    1,
    anon_sym_end,
    STATE(340),
    1,
    sym_comment,
    STATE(342),
    1,
    aux_sym_if_statement_repeat1,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(410),
    1,
    sym_else_statement,
    [22278] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(79),
    1,
    anon_sym_else,
    ACTIONS(812),
    1,
    anon_sym_elseif,
    ACTIONS(826),
    1,
    anon_sym_end,
    STATE(341),
    1,
    sym_comment,
    STATE(343),
    1,
    aux_sym_if_statement_repeat1,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(412),
    1,
    sym_else_statement,
    [22306] = 9,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(79),
    1,
    anon_sym_else,
    ACTIONS(812),
    1,
    anon_sym_elseif,
    ACTIONS(828),
    1,
    anon_sym_end,
    STATE(342),
    1,
    sym_comment,
    STATE(343),
    1,
    aux_sym_if_statement_repeat1,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(435),
    1,
    sym_else_statement,
    [22334] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(830),
    1,
    anon_sym_end,
    ACTIONS(832),
    1,
    anon_sym_elseif,
    ACTIONS(835),
    1,
    anon_sym_else,
    STATE(401),
    1,
    sym_elseif_statement,
    STATE(343),
    2,
    sym_comment,
    aux_sym_if_statement_repeat1,
    [22357] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(840),
    1,
    anon_sym_RBRACE,
    STATE(133),
    1,
    sym__field_sep,
    ACTIONS(837),
    2,
    anon_sym_SEMI,
    anon_sym_COMMA,
    STATE(344),
    2,
    sym_comment,
    aux_sym__field_list_repeat1,
    [22378] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(820),
    1,
    anon_sym_else,
    STATE(345),
    1,
    sym_comment,
    ACTIONS(816),
    4,
    ts_builtin_sym_end,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [22397] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(510),
    1,
    anon_sym_RBRACE,
    STATE(127),
    1,
    sym__field_sep,
    STATE(344),
    1,
    aux_sym__field_list_repeat1,
    STATE(346),
    1,
    sym_comment,
    ACTIONS(842),
    2,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [22420] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(846),
    1,
    anon_sym_else,
    STATE(347),
    1,
    sym_comment,
    ACTIONS(844),
    4,
    ts_builtin_sym_end,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [22439] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(848),
    1,
    anon_sym_RBRACE,
    STATE(130),
    1,
    sym__field_sep,
    STATE(346),
    1,
    aux_sym__field_list_repeat1,
    STATE(348),
    1,
    sym_comment,
    ACTIONS(842),
    2,
    anon_sym_SEMI,
    anon_sym_COMMA,
    [22462] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(349),
    1,
    sym_comment,
    ACTIONS(850),
    5,
    sym__block_string_start,
    anon_sym_DQUOTE,
    anon_sym_SQUOTE,
    anon_sym_LPAREN,
    anon_sym_LBRACE,
    [22479] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(109),
    1,
    anon_sym_else,
    STATE(350),
    1,
    sym_comment,
    ACTIONS(852),
    3,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [22497] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(854),
    1,
    anon_sym_DQUOTE,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    STATE(351),
    1,
    sym_comment,
    STATE(372),
    1,
    aux_sym__doublequote_string_content,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22517] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(860),
    1,
    sym_identifier,
    STATE(352),
    1,
    sym_comment,
    STATE(460),
    1,
    sym__name_list,
    STATE(468),
    2,
    sym_for_generic_clause,
    sym_for_numeric_clause,
    [22537] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(862),
    1,
    anon_sym_SQUOTE,
    STATE(353),
    1,
    sym_comment,
    STATE(360),
    1,
    aux_sym__singlequote_string_content,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22557] = 5,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(866),
    1,
    anon_sym_DQUOTE,
    ACTIONS(868),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    STATE(354),
    2,
    aux_sym__doublequote_string_content,
    sym_comment,
    [22575] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(871),
    1,
    anon_sym_SQUOTE,
    STATE(355),
    1,
    sym_comment,
    STATE(360),
    1,
    aux_sym__singlequote_string_content,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22595] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(871),
    1,
    anon_sym_DQUOTE,
    STATE(354),
    1,
    aux_sym__doublequote_string_content,
    STATE(356),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22615] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(873),
    1,
    sym_identifier,
    ACTIONS(875),
    1,
    sym_vararg_expression,
    ACTIONS(877),
    1,
    anon_sym_RPAREN,
    STATE(357),
    1,
    sym_comment,
    STATE(442),
    1,
    sym__parameter_list,
    [22637] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(879),
    1,
    anon_sym_SQUOTE,
    STATE(355),
    1,
    aux_sym__singlequote_string_content,
    STATE(358),
    1,
    sym_comment,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22657] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(879),
    1,
    anon_sym_DQUOTE,
    STATE(356),
    1,
    aux_sym__doublequote_string_content,
    STATE(359),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22677] = 5,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(881),
    1,
    anon_sym_SQUOTE,
    ACTIONS(883),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    STATE(360),
    2,
    aux_sym__singlequote_string_content,
    sym_comment,
    [22695] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(886),
    1,
    anon_sym_SQUOTE,
    STATE(360),
    1,
    aux_sym__singlequote_string_content,
    STATE(361),
    1,
    sym_comment,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22715] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(886),
    1,
    anon_sym_DQUOTE,
    STATE(354),
    1,
    aux_sym__doublequote_string_content,
    STATE(362),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22735] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(888),
    1,
    anon_sym_SQUOTE,
    STATE(361),
    1,
    aux_sym__singlequote_string_content,
    STATE(363),
    1,
    sym_comment,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22755] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(890),
    1,
    anon_sym_COMMA,
    ACTIONS(893),
    2,
    anon_sym_in,
    anon_sym_RPAREN,
    STATE(364),
    2,
    sym_comment,
    aux_sym__name_list_repeat1,
    [22773] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(888),
    1,
    anon_sym_DQUOTE,
    STATE(362),
    1,
    aux_sym__doublequote_string_content,
    STATE(365),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22793] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(895),
    1,
    anon_sym_EQ,
    ACTIONS(897),
    1,
    anon_sym_COMMA,
    ACTIONS(899),
    1,
    anon_sym_in,
    STATE(366),
    1,
    sym_comment,
    STATE(394),
    1,
    aux_sym__name_list_repeat1,
    [22815] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(903),
    1,
    anon_sym_else,
    STATE(367),
    1,
    sym_comment,
    ACTIONS(901),
    3,
    anon_sym_end,
    anon_sym_until,
    anon_sym_elseif,
    [22833] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(905),
    1,
    sym_identifier,
    ACTIONS(907),
    1,
    anon_sym_function,
    STATE(270),
    1,
    sym__att_name_list,
    STATE(290),
    1,
    sym__local_variable_assignment,
    STATE(368),
    1,
    sym_comment,
    [22855] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(909),
    1,
    anon_sym_DQUOTE,
    STATE(354),
    1,
    aux_sym__doublequote_string_content,
    STATE(369),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22875] = 7,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(911),
    1,
    sym_identifier,
    ACTIONS(913),
    1,
    anon_sym_function,
    STATE(162),
    1,
    sym__att_name_list,
    STATE(260),
    1,
    sym__local_variable_assignment,
    STATE(370),
    1,
    sym_comment,
    [22897] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(915),
    1,
    anon_sym_DQUOTE,
    STATE(369),
    1,
    aux_sym__doublequote_string_content,
    STATE(371),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22917] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(862),
    1,
    anon_sym_DQUOTE,
    STATE(354),
    1,
    aux_sym__doublequote_string_content,
    STATE(372),
    1,
    sym_comment,
    ACTIONS(856),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [22937] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(915),
    1,
    anon_sym_SQUOTE,
    STATE(373),
    1,
    sym_comment,
    STATE(374),
    1,
    aux_sym__singlequote_string_content,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22957] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(909),
    1,
    anon_sym_SQUOTE,
    STATE(360),
    1,
    aux_sym__singlequote_string_content,
    STATE(374),
    1,
    sym_comment,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22977] = 6,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(854),
    1,
    anon_sym_SQUOTE,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    STATE(353),
    1,
    aux_sym__singlequote_string_content,
    STATE(375),
    1,
    sym_comment,
    ACTIONS(864),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [22997] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(860),
    1,
    sym_identifier,
    STATE(376),
    1,
    sym_comment,
    STATE(460),
    1,
    sym__name_list,
    STATE(447),
    2,
    sym_for_generic_clause,
    sym_for_numeric_clause,
    [23017] = 5,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(917),
    1,
    anon_sym_SQUOTE,
    STATE(377),
    1,
    sym_comment,
    ACTIONS(919),
    2,
    aux_sym__singlequote_string_content_token1,
    sym_escape_sequence,
    [23034] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(378),
    1,
    sym_comment,
    ACTIONS(921),
    3,
    anon_sym_COMMA,
    anon_sym_in,
    anon_sym_RPAREN,
    [23049] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(17),
    1,
    sym_parameters,
    STATE(276),
    1,
    sym__function_body,
    STATE(379),
    1,
    sym_comment,
    [23068] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(380),
    1,
    sym_comment,
    ACTIONS(923),
    3,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LPAREN,
    [23083] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(925),
    1,
    anon_sym_EQ,
    ACTIONS(927),
    1,
    anon_sym_COMMA,
    STATE(381),
    2,
    sym_comment,
    aux_sym__variable_assignment_varlist_repeat1,
    [23100] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(382),
    1,
    sym_comment,
    ACTIONS(930),
    3,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LPAREN,
    [23115] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(932),
    1,
    anon_sym_DOT,
    ACTIONS(934),
    1,
    anon_sym_COLON,
    ACTIONS(936),
    1,
    anon_sym_LPAREN,
    STATE(383),
    1,
    sym_comment,
    [23134] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(938),
    1,
    anon_sym_COMMA,
    ACTIONS(940),
    1,
    anon_sym_RPAREN,
    STATE(384),
    1,
    sym_comment,
    STATE(389),
    1,
    aux_sym__name_list_repeat1,
    [23153] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(17),
    1,
    sym_parameters,
    STATE(291),
    1,
    sym__function_body,
    STATE(385),
    1,
    sym_comment,
    [23172] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(12),
    1,
    sym_parameters,
    STATE(151),
    1,
    sym__function_body,
    STATE(386),
    1,
    sym_comment,
    [23191] = 5,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(942),
    1,
    anon_sym_DQUOTE,
    STATE(387),
    1,
    sym_comment,
    ACTIONS(944),
    2,
    aux_sym__doublequote_string_content_token1,
    sym_escape_sequence,
    [23208] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(388),
    1,
    sym_comment,
    ACTIONS(946),
    3,
    anon_sym_DOT,
    anon_sym_COLON,
    anon_sym_LPAREN,
    [23223] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(948),
    1,
    anon_sym_COMMA,
    ACTIONS(950),
    1,
    anon_sym_RPAREN,
    STATE(364),
    1,
    aux_sym__name_list_repeat1,
    STATE(389),
    1,
    sym_comment,
    [23242] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(18),
    1,
    sym_parameters,
    STATE(212),
    1,
    sym__function_body,
    STATE(390),
    1,
    sym_comment,
    [23261] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(952),
    1,
    anon_sym_RPAREN,
    STATE(330),
    1,
    aux_sym__expression_list_repeat1,
    STATE(391),
    1,
    sym_comment,
    [23280] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(956),
    1,
    anon_sym_else,
    STATE(392),
    1,
    sym_comment,
    ACTIONS(954),
    2,
    anon_sym_end,
    anon_sym_elseif,
    [23297] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(18),
    1,
    sym_parameters,
    STATE(64),
    1,
    sym__function_body,
    STATE(393),
    1,
    sym_comment,
    [23316] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(897),
    1,
    anon_sym_COMMA,
    ACTIONS(958),
    1,
    anon_sym_in,
    STATE(364),
    1,
    aux_sym__name_list_repeat1,
    STATE(394),
    1,
    sym_comment,
    [23335] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    STATE(395),
    1,
    sym_comment,
    ACTIONS(840),
    3,
    anon_sym_SEMI,
    anon_sym_COMMA,
    anon_sym_RBRACE,
    [23350] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(18),
    1,
    sym_parameters,
    STATE(231),
    1,
    sym__function_body,
    STATE(396),
    1,
    sym_comment,
    [23369] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(960),
    1,
    anon_sym_RPAREN,
    STATE(330),
    1,
    aux_sym__expression_list_repeat1,
    STATE(397),
    1,
    sym_comment,
    [23388] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(808),
    1,
    anon_sym_LPAREN,
    STATE(17),
    1,
    sym_parameters,
    STATE(82),
    1,
    sym__function_body,
    STATE(398),
    1,
    sym_comment,
    [23407] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(705),
    1,
    anon_sym_COMMA,
    ACTIONS(962),
    1,
    anon_sym_RPAREN,
    STATE(330),
    1,
    aux_sym__expression_list_repeat1,
    STATE(399),
    1,
    sym_comment,
    [23426] = 6,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(275),
    1,
    anon_sym_COMMA,
    ACTIONS(964),
    1,
    anon_sym_EQ,
    STATE(381),
    1,
    aux_sym__variable_assignment_varlist_repeat1,
    STATE(400),
    1,
    sym_comment,
    [23445] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(968),
    1,
    anon_sym_else,
    STATE(401),
    1,
    sym_comment,
    ACTIONS(966),
    2,
    anon_sym_end,
    anon_sym_elseif,
    [23462] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(970),
    1,
    sym_identifier,
    ACTIONS(972),
    1,
    sym_vararg_expression,
    STATE(402),
    1,
    sym_comment,
    [23478] = 5,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(970),
    1,
    sym_identifier,
    ACTIONS(974),
    1,
    sym_vararg_expression,
    STATE(403),
    1,
    sym_comment,
    [23494] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(976),
    1,
    ts_builtin_sym_end,
    STATE(404),
    1,
    sym_comment,
    [23507] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(970),
    1,
    sym_identifier,
    STATE(405),
    1,
    sym_comment,
    [23520] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(978),
    1,
    anon_sym_RBRACE,
    STATE(406),
    1,
    sym_comment,
    [23533] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(980),
    1,
    ts_builtin_sym_end,
    STATE(407),
    1,
    sym_comment,
    [23546] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(982),
    1,
    sym_identifier,
    STATE(408),
    1,
    sym_comment,
    [23559] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(984),
    1,
    anon_sym_end,
    STATE(409),
    1,
    sym_comment,
    [23572] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(986),
    1,
    anon_sym_end,
    STATE(410),
    1,
    sym_comment,
    [23585] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(988),
    1,
    anon_sym_EQ,
    STATE(411),
    1,
    sym_comment,
    [23598] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(990),
    1,
    anon_sym_end,
    STATE(412),
    1,
    sym_comment,
    [23611] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(992),
    1,
    anon_sym_RPAREN,
    STATE(413),
    1,
    sym_comment,
    [23624] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(994),
    1,
    sym__block_string_end,
    STATE(414),
    1,
    sym_comment,
    [23637] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(996),
    1,
    anon_sym_COLON_COLON,
    STATE(415),
    1,
    sym_comment,
    [23650] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(998),
    1,
    anon_sym_end,
    STATE(416),
    1,
    sym_comment,
    [23663] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1000),
    1,
    anon_sym_until,
    STATE(417),
    1,
    sym_comment,
    [23676] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1002),
    1,
    anon_sym_RBRACE,
    STATE(418),
    1,
    sym_comment,
    [23689] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1004),
    1,
    sym_identifier,
    STATE(419),
    1,
    sym_comment,
    [23702] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1006),
    1,
    sym_identifier,
    STATE(420),
    1,
    sym_comment,
    [23715] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1008),
    1,
    anon_sym_end,
    STATE(421),
    1,
    sym_comment,
    [23728] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1010),
    1,
    sym_identifier,
    STATE(422),
    1,
    sym_comment,
    [23741] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1012),
    1,
    sym_identifier,
    STATE(423),
    1,
    sym_comment,
    [23754] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1014),
    1,
    anon_sym_end,
    STATE(424),
    1,
    sym_comment,
    [23767] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1016),
    1,
    sym_identifier,
    STATE(425),
    1,
    sym_comment,
    [23780] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1018),
    1,
    anon_sym_end,
    STATE(426),
    1,
    sym_comment,
    [23793] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1020),
    1,
    anon_sym_end,
    STATE(427),
    1,
    sym_comment,
    [23806] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1022),
    1,
    anon_sym_end,
    STATE(428),
    1,
    sym_comment,
    [23819] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1024),
    1,
    anon_sym_end,
    STATE(429),
    1,
    sym_comment,
    [23832] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1026),
    1,
    anon_sym_LPAREN,
    STATE(430),
    1,
    sym_comment,
    [23845] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1028),
    1,
    anon_sym_end,
    STATE(431),
    1,
    sym_comment,
    [23858] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1030),
    1,
    anon_sym_GT,
    STATE(432),
    1,
    sym_comment,
    [23871] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1032),
    1,
    anon_sym_end,
    STATE(433),
    1,
    sym_comment,
    [23884] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1034),
    1,
    anon_sym_end,
    STATE(434),
    1,
    sym_comment,
    [23897] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1036),
    1,
    anon_sym_end,
    STATE(435),
    1,
    sym_comment,
    [23910] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1038),
    1,
    anon_sym_end,
    STATE(436),
    1,
    sym_comment,
    [23923] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1040),
    1,
    anon_sym_end,
    STATE(437),
    1,
    sym_comment,
    [23936] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1042),
    1,
    anon_sym_until,
    STATE(438),
    1,
    sym_comment,
    [23949] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1044),
    1,
    anon_sym_EQ,
    STATE(439),
    1,
    sym_comment,
    [23962] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1046),
    1,
    sym_identifier,
    STATE(440),
    1,
    sym_comment,
    [23975] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(200),
    1,
    ts_builtin_sym_end,
    STATE(441),
    1,
    sym_comment,
    [23988] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1048),
    1,
    anon_sym_RPAREN,
    STATE(442),
    1,
    sym_comment,
    [24001] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1050),
    1,
    sym__block_string_end,
    STATE(443),
    1,
    sym_comment,
    [24014] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1052),
    1,
    anon_sym_EQ,
    STATE(444),
    1,
    sym_comment,
    [24027] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1054),
    1,
    anon_sym_RPAREN,
    STATE(445),
    1,
    sym_comment,
    [24040] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1056),
    1,
    anon_sym_end,
    STATE(446),
    1,
    sym_comment,
    [24053] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1058),
    1,
    anon_sym_do,
    STATE(447),
    1,
    sym_comment,
    [24066] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1060),
    1,
    anon_sym_RPAREN,
    STATE(448),
    1,
    sym_comment,
    [24079] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1062),
    1,
    anon_sym_RBRACE,
    STATE(449),
    1,
    sym_comment,
    [24092] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1064),
    1,
    sym_identifier,
    STATE(450),
    1,
    sym_comment,
    [24105] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1066),
    1,
    sym__block_string_content,
    STATE(451),
    1,
    sym_comment,
    [24118] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1068),
    1,
    anon_sym_end,
    STATE(452),
    1,
    sym_comment,
    [24131] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1070),
    1,
    sym__block_comment_content,
    STATE(453),
    1,
    sym_comment,
    [24144] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1072),
    1,
    anon_sym_do,
    STATE(454),
    1,
    sym_comment,
    [24157] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1074),
    1,
    sym_identifier,
    STATE(455),
    1,
    sym_comment,
    [24170] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1076),
    1,
    sym__block_comment_end,
    STATE(456),
    1,
    sym_comment,
    [24183] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1078),
    1,
    sym_identifier,
    STATE(457),
    1,
    sym_comment,
    [24196] = 4,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(858),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(1080),
    1,
    aux_sym_comment_token1,
    STATE(458),
    1,
    sym_comment,
    [24209] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1082),
    1,
    sym__block_string_end,
    STATE(459),
    1,
    sym_comment,
    [24222] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1084),
    1,
    anon_sym_in,
    STATE(460),
    1,
    sym_comment,
    [24235] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1086),
    1,
    sym__block_string_end,
    STATE(461),
    1,
    sym_comment,
    [24248] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1088),
    1,
    anon_sym_RBRACE,
    STATE(462),
    1,
    sym_comment,
    [24261] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1090),
    1,
    sym__block_string_content,
    STATE(463),
    1,
    sym_comment,
    [24274] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1092),
    1,
    sym_identifier,
    STATE(464),
    1,
    sym_comment,
    [24287] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(202),
    1,
    ts_builtin_sym_end,
    STATE(465),
    1,
    sym_comment,
    [24300] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1094),
    1,
    sym_identifier,
    STATE(466),
    1,
    sym_comment,
    [24313] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1096),
    1,
    anon_sym_GT,
    STATE(467),
    1,
    sym_comment,
    [24326] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1098),
    1,
    anon_sym_do,
    STATE(468),
    1,
    sym_comment,
    [24339] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1100),
    1,
    sym_identifier,
    STATE(469),
    1,
    sym_comment,
    [24352] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1102),
    1,
    anon_sym_LPAREN,
    STATE(470),
    1,
    sym_comment,
    [24365] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1104),
    1,
    sym_identifier,
    STATE(471),
    1,
    sym_comment,
    [24378] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1106),
    1,
    sym__block_string_content,
    STATE(472),
    1,
    sym_comment,
    [24391] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1108),
    1,
    sym_identifier,
    STATE(473),
    1,
    sym_comment,
    [24404] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1110),
    1,
    anon_sym_COLON_COLON,
    STATE(474),
    1,
    sym_comment,
    [24417] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1112),
    1,
    sym__block_string_content,
    STATE(475),
    1,
    sym_comment,
    [24430] = 4,
    ACTIONS(3),
    1,
    anon_sym_DASH_DASH,
    ACTIONS(5),
    1,
    sym__block_comment_start,
    ACTIONS(1114),
    1,
    sym_identifier,
    STATE(476),
    1,
    sym_comment,
    [24443] = 1,
    ACTIONS(1116),
    1,
    ts_builtin_sym_end,
    [24447] = 1,
    ACTIONS(1118),
    1,
    ts_builtin_sym_end,
};

static const uint32_t ts_small_parse_table_map[] = {
    [SMALL_STATE(7)] = 0,
    [SMALL_STATE(8)] = 155,
    [SMALL_STATE(9)] = 310,
    [SMALL_STATE(10)] = 465,
    [SMALL_STATE(11)] = 620,
    [SMALL_STATE(12)] = 775,
    [SMALL_STATE(13)] = 930,
    [SMALL_STATE(14)] = 1085,
    [SMALL_STATE(15)] = 1240,
    [SMALL_STATE(16)] = 1395,
    [SMALL_STATE(17)] = 1550,
    [SMALL_STATE(18)] = 1705,
    [SMALL_STATE(19)] = 1860,
    [SMALL_STATE(20)] = 2012,
    [SMALL_STATE(21)] = 2164,
    [SMALL_STATE(22)] = 2316,
    [SMALL_STATE(23)] = 2463,
    [SMALL_STATE(24)] = 2543,
    [SMALL_STATE(25)] = 2612,
    [SMALL_STATE(26)] = 2681,
    [SMALL_STATE(27)] = 2750,
    [SMALL_STATE(28)] = 2818,
    [SMALL_STATE(29)] = 2886,
    [SMALL_STATE(30)] = 2954,
    [SMALL_STATE(31)] = 3026,
    [SMALL_STATE(32)] = 3094,
    [SMALL_STATE(33)] = 3162,
    [SMALL_STATE(34)] = 3230,
    [SMALL_STATE(35)] = 3298,
    [SMALL_STATE(36)] = 3366,
    [SMALL_STATE(37)] = 3434,
    [SMALL_STATE(38)] = 3502,
    [SMALL_STATE(39)] = 3570,
    [SMALL_STATE(40)] = 3638,
    [SMALL_STATE(41)] = 3706,
    [SMALL_STATE(42)] = 3783,
    [SMALL_STATE(43)] = 3849,
    [SMALL_STATE(44)] = 3947,
    [SMALL_STATE(45)] = 4013,
    [SMALL_STATE(46)] = 4079,
    [SMALL_STATE(47)] = 4146,
    [SMALL_STATE(48)] = 4213,
    [SMALL_STATE(49)] = 4292,
    [SMALL_STATE(50)] = 4373,
    [SMALL_STATE(51)] = 4456,
    [SMALL_STATE(52)] = 4545,
    [SMALL_STATE(53)] = 4636,
    [SMALL_STATE(54)] = 4721,
    [SMALL_STATE(55)] = 4786,
    [SMALL_STATE(56)] = 4851,
    [SMALL_STATE(57)] = 4916,
    [SMALL_STATE(58)] = 4981,
    [SMALL_STATE(59)] = 5046,
    [SMALL_STATE(60)] = 5111,
    [SMALL_STATE(61)] = 5176,
    [SMALL_STATE(62)] = 5245,
    [SMALL_STATE(63)] = 5310,
    [SMALL_STATE(64)] = 5387,
    [SMALL_STATE(65)] = 5452,
    [SMALL_STATE(66)] = 5517,
    [SMALL_STATE(67)] = 5588,
    [SMALL_STATE(68)] = 5655,
    [SMALL_STATE(69)] = 5720,
    [SMALL_STATE(70)] = 5785,
    [SMALL_STATE(71)] = 5850,
    [SMALL_STATE(72)] = 5915,
    [SMALL_STATE(73)] = 5992,
    [SMALL_STATE(74)] = 6057,
    [SMALL_STATE(75)] = 6122,
    [SMALL_STATE(76)] = 6187,
    [SMALL_STATE(77)] = 6280,
    [SMALL_STATE(78)] = 6372,
    [SMALL_STATE(79)] = 6464,
    [SMALL_STATE(80)] = 6556,
    [SMALL_STATE(81)] = 6651,
    [SMALL_STATE(82)] = 6719,
    [SMALL_STATE(83)] = 6781,
    [SMALL_STATE(84)] = 6861,
    [SMALL_STATE(85)] = 6947,
    [SMALL_STATE(86)] = 7035,
    [SMALL_STATE(87)] = 7125,
    [SMALL_STATE(88)] = 7189,
    [SMALL_STATE(89)] = 7265,
    [SMALL_STATE(90)] = 7347,
    [SMALL_STATE(91)] = 7409,
    [SMALL_STATE(92)] = 7483,
    [SMALL_STATE(93)] = 7557,
    [SMALL_STATE(94)] = 7635,
    [SMALL_STATE(95)] = 7697,
    [SMALL_STATE(96)] = 7761,
    [SMALL_STATE(97)] = 7825,
    [SMALL_STATE(98)] = 7887,
    [SMALL_STATE(99)] = 7976,
    [SMALL_STATE(100)] = 8065,
    [SMALL_STATE(101)] = 8154,
    [SMALL_STATE(102)] = 8209,
    [SMALL_STATE(103)] = 8264,
    [SMALL_STATE(104)] = 8319,
    [SMALL_STATE(105)] = 8374,
    [SMALL_STATE(106)] = 8429,
    [SMALL_STATE(107)] = 8488,
    [SMALL_STATE(108)] = 8543,
    [SMALL_STATE(109)] = 8598,
    [SMALL_STATE(110)] = 8653,
    [SMALL_STATE(111)] = 8708,
    [SMALL_STATE(112)] = 8763,
    [SMALL_STATE(113)] = 8818,
    [SMALL_STATE(114)] = 8873,
    [SMALL_STATE(115)] = 8928,
    [SMALL_STATE(116)] = 8983,
    [SMALL_STATE(117)] = 9038,
    [SMALL_STATE(118)] = 9093,
    [SMALL_STATE(119)] = 9184,
    [SMALL_STATE(120)] = 9242,
    [SMALL_STATE(121)] = 9295,
    [SMALL_STATE(122)] = 9386,
    [SMALL_STATE(123)] = 9477,
    [SMALL_STATE(124)] = 9568,
    [SMALL_STATE(125)] = 9659,
    [SMALL_STATE(126)] = 9711,
    [SMALL_STATE(127)] = 9799,
    [SMALL_STATE(128)] = 9887,
    [SMALL_STATE(129)] = 9937,
    [SMALL_STATE(130)] = 9989,
    [SMALL_STATE(131)] = 10077,
    [SMALL_STATE(132)] = 10129,
    [SMALL_STATE(133)] = 10196,
    [SMALL_STATE(134)] = 10281,
    [SMALL_STATE(135)] = 10346,
    [SMALL_STATE(136)] = 10395,
    [SMALL_STATE(137)] = 10454,
    [SMALL_STATE(138)] = 10501,
    [SMALL_STATE(139)] = 10548,
    [SMALL_STATE(140)] = 10595,
    [SMALL_STATE(141)] = 10644,
    [SMALL_STATE(142)] = 10717,
    [SMALL_STATE(143)] = 10770,
    [SMALL_STATE(144)] = 10817,
    [SMALL_STATE(145)] = 10864,
    [SMALL_STATE(146)] = 10911,
    [SMALL_STATE(147)] = 10982,
    [SMALL_STATE(148)] = 11037,
    [SMALL_STATE(149)] = 11084,
    [SMALL_STATE(150)] = 11143,
    [SMALL_STATE(151)] = 11192,
    [SMALL_STATE(152)] = 11239,
    [SMALL_STATE(153)] = 11300,
    [SMALL_STATE(154)] = 11363,
    [SMALL_STATE(155)] = 11410,
    [SMALL_STATE(156)] = 11457,
    [SMALL_STATE(157)] = 11504,
    [SMALL_STATE(158)] = 11553,
    [SMALL_STATE(159)] = 11602,
    [SMALL_STATE(160)] = 11653,
    [SMALL_STATE(161)] = 11700,
    [SMALL_STATE(162)] = 11747,
    [SMALL_STATE(163)] = 11795,
    [SMALL_STATE(164)] = 11877,
    [SMALL_STATE(165)] = 11959,
    [SMALL_STATE(166)] = 12041,
    [SMALL_STATE(167)] = 12123,
    [SMALL_STATE(168)] = 12205,
    [SMALL_STATE(169)] = 12287,
    [SMALL_STATE(170)] = 12369,
    [SMALL_STATE(171)] = 12419,
    [SMALL_STATE(172)] = 12501,
    [SMALL_STATE(173)] = 12580,
    [SMALL_STATE(174)] = 12659,
    [SMALL_STATE(175)] = 12738,
    [SMALL_STATE(176)] = 12817,
    [SMALL_STATE(177)] = 12896,
    [SMALL_STATE(178)] = 12975,
    [SMALL_STATE(179)] = 13054,
    [SMALL_STATE(180)] = 13133,
    [SMALL_STATE(181)] = 13212,
    [SMALL_STATE(182)] = 13291,
    [SMALL_STATE(183)] = 13370,
    [SMALL_STATE(184)] = 13449,
    [SMALL_STATE(185)] = 13528,
    [SMALL_STATE(186)] = 13607,
    [SMALL_STATE(187)] = 13686,
    [SMALL_STATE(188)] = 13765,
    [SMALL_STATE(189)] = 13844,
    [SMALL_STATE(190)] = 13923,
    [SMALL_STATE(191)] = 14002,
    [SMALL_STATE(192)] = 14081,
    [SMALL_STATE(193)] = 14160,
    [SMALL_STATE(194)] = 14239,
    [SMALL_STATE(195)] = 14318,
    [SMALL_STATE(196)] = 14397,
    [SMALL_STATE(197)] = 14476,
    [SMALL_STATE(198)] = 14521,
    [SMALL_STATE(199)] = 14566,
    [SMALL_STATE(200)] = 14611,
    [SMALL_STATE(201)] = 14660,
    [SMALL_STATE(202)] = 14707,
    [SMALL_STATE(203)] = 14752,
    [SMALL_STATE(204)] = 14797,
    [SMALL_STATE(205)] = 14842,
    [SMALL_STATE(206)] = 14887,
    [SMALL_STATE(207)] = 14932,
    [SMALL_STATE(208)] = 14977,
    [SMALL_STATE(209)] = 15056,
    [SMALL_STATE(210)] = 15135,
    [SMALL_STATE(211)] = 15180,
    [SMALL_STATE(212)] = 15259,
    [SMALL_STATE(213)] = 15304,
    [SMALL_STATE(214)] = 15383,
    [SMALL_STATE(215)] = 15428,
    [SMALL_STATE(216)] = 15473,
    [SMALL_STATE(217)] = 15552,
    [SMALL_STATE(218)] = 15597,
    [SMALL_STATE(219)] = 15676,
    [SMALL_STATE(220)] = 15755,
    [SMALL_STATE(221)] = 15834,
    [SMALL_STATE(222)] = 15913,
    [SMALL_STATE(223)] = 15992,
    [SMALL_STATE(224)] = 16071,
    [SMALL_STATE(225)] = 16150,
    [SMALL_STATE(226)] = 16229,
    [SMALL_STATE(227)] = 16308,
    [SMALL_STATE(228)] = 16387,
    [SMALL_STATE(229)] = 16432,
    [SMALL_STATE(230)] = 16511,
    [SMALL_STATE(231)] = 16590,
    [SMALL_STATE(232)] = 16635,
    [SMALL_STATE(233)] = 16714,
    [SMALL_STATE(234)] = 16793,
    [SMALL_STATE(235)] = 16872,
    [SMALL_STATE(236)] = 16917,
    [SMALL_STATE(237)] = 16962,
    [SMALL_STATE(238)] = 17041,
    [SMALL_STATE(239)] = 17120,
    [SMALL_STATE(240)] = 17199,
    [SMALL_STATE(241)] = 17278,
    [SMALL_STATE(242)] = 17357,
    [SMALL_STATE(243)] = 17436,
    [SMALL_STATE(244)] = 17515,
    [SMALL_STATE(245)] = 17594,
    [SMALL_STATE(246)] = 17673,
    [SMALL_STATE(247)] = 17752,
    [SMALL_STATE(248)] = 17797,
    [SMALL_STATE(249)] = 17842,
    [SMALL_STATE(250)] = 17921,
    [SMALL_STATE(251)] = 18000,
    [SMALL_STATE(252)] = 18079,
    [SMALL_STATE(253)] = 18124,
    [SMALL_STATE(254)] = 18169,
    [SMALL_STATE(255)] = 18214,
    [SMALL_STATE(256)] = 18259,
    [SMALL_STATE(257)] = 18304,
    [SMALL_STATE(258)] = 18353,
    [SMALL_STATE(259)] = 18402,
    [SMALL_STATE(260)] = 18481,
    [SMALL_STATE(261)] = 18526,
    [SMALL_STATE(262)] = 18605,
    [SMALL_STATE(263)] = 18650,
    [SMALL_STATE(264)] = 18695,
    [SMALL_STATE(265)] = 18739,
    [SMALL_STATE(266)] = 18783,
    [SMALL_STATE(267)] = 18855,
    [SMALL_STATE(268)] = 18901,
    [SMALL_STATE(269)] = 18949,
    [SMALL_STATE(270)] = 19025,
    [SMALL_STATE(271)] = 19070,
    [SMALL_STATE(272)] = 19112,
    [SMALL_STATE(273)] = 19154,
    [SMALL_STATE(274)] = 19196,
    [SMALL_STATE(275)] = 19238,
    [SMALL_STATE(276)] = 19280,
    [SMALL_STATE(277)] = 19322,
    [SMALL_STATE(278)] = 19364,
    [SMALL_STATE(279)] = 19406,
    [SMALL_STATE(280)] = 19448,
    [SMALL_STATE(281)] = 19490,
    [SMALL_STATE(282)] = 19532,
    [SMALL_STATE(283)] = 19574,
    [SMALL_STATE(284)] = 19616,
    [SMALL_STATE(285)] = 19658,
    [SMALL_STATE(286)] = 19700,
    [SMALL_STATE(287)] = 19742,
    [SMALL_STATE(288)] = 19784,
    [SMALL_STATE(289)] = 19826,
    [SMALL_STATE(290)] = 19868,
    [SMALL_STATE(291)] = 19910,
    [SMALL_STATE(292)] = 19952,
    [SMALL_STATE(293)] = 19994,
    [SMALL_STATE(294)] = 20036,
    [SMALL_STATE(295)] = 20078,
    [SMALL_STATE(296)] = 20120,
    [SMALL_STATE(297)] = 20162,
    [SMALL_STATE(298)] = 20204,
    [SMALL_STATE(299)] = 20246,
    [SMALL_STATE(300)] = 20288,
    [SMALL_STATE(301)] = 20330,
    [SMALL_STATE(302)] = 20398,
    [SMALL_STATE(303)] = 20466,
    [SMALL_STATE(304)] = 20530,
    [SMALL_STATE(305)] = 20594,
    [SMALL_STATE(306)] = 20658,
    [SMALL_STATE(307)] = 20726,
    [SMALL_STATE(308)] = 20791,
    [SMALL_STATE(309)] = 20853,
    [SMALL_STATE(310)] = 20915,
    [SMALL_STATE(311)] = 20977,
    [SMALL_STATE(312)] = 21039,
    [SMALL_STATE(313)] = 21101,
    [SMALL_STATE(314)] = 21163,
    [SMALL_STATE(315)] = 21225,
    [SMALL_STATE(316)] = 21287,
    [SMALL_STATE(317)] = 21349,
    [SMALL_STATE(318)] = 21411,
    [SMALL_STATE(319)] = 21473,
    [SMALL_STATE(320)] = 21535,
    [SMALL_STATE(321)] = 21597,
    [SMALL_STATE(322)] = 21659,
    [SMALL_STATE(323)] = 21691,
    [SMALL_STATE(324)] = 21738,
    [SMALL_STATE(325)] = 21785,
    [SMALL_STATE(326)] = 21832,
    [SMALL_STATE(327)] = 21870,
    [SMALL_STATE(328)] = 21894,
    [SMALL_STATE(329)] = 21932,
    [SMALL_STATE(330)] = 21970,
    [SMALL_STATE(331)] = 21996,
    [SMALL_STATE(332)] = 22029,
    [SMALL_STATE(333)] = 22056,
    [SMALL_STATE(334)] = 22090,
    [SMALL_STATE(335)] = 22110,
    [SMALL_STATE(336)] = 22144,
    [SMALL_STATE(337)] = 22172,
    [SMALL_STATE(338)] = 22200,
    [SMALL_STATE(339)] = 22222,
    [SMALL_STATE(340)] = 22250,
    [SMALL_STATE(341)] = 22278,
    [SMALL_STATE(342)] = 22306,
    [SMALL_STATE(343)] = 22334,
    [SMALL_STATE(344)] = 22357,
    [SMALL_STATE(345)] = 22378,
    [SMALL_STATE(346)] = 22397,
    [SMALL_STATE(347)] = 22420,
    [SMALL_STATE(348)] = 22439,
    [SMALL_STATE(349)] = 22462,
    [SMALL_STATE(350)] = 22479,
    [SMALL_STATE(351)] = 22497,
    [SMALL_STATE(352)] = 22517,
    [SMALL_STATE(353)] = 22537,
    [SMALL_STATE(354)] = 22557,
    [SMALL_STATE(355)] = 22575,
    [SMALL_STATE(356)] = 22595,
    [SMALL_STATE(357)] = 22615,
    [SMALL_STATE(358)] = 22637,
    [SMALL_STATE(359)] = 22657,
    [SMALL_STATE(360)] = 22677,
    [SMALL_STATE(361)] = 22695,
    [SMALL_STATE(362)] = 22715,
    [SMALL_STATE(363)] = 22735,
    [SMALL_STATE(364)] = 22755,
    [SMALL_STATE(365)] = 22773,
    [SMALL_STATE(366)] = 22793,
    [SMALL_STATE(367)] = 22815,
    [SMALL_STATE(368)] = 22833,
    [SMALL_STATE(369)] = 22855,
    [SMALL_STATE(370)] = 22875,
    [SMALL_STATE(371)] = 22897,
    [SMALL_STATE(372)] = 22917,
    [SMALL_STATE(373)] = 22937,
    [SMALL_STATE(374)] = 22957,
    [SMALL_STATE(375)] = 22977,
    [SMALL_STATE(376)] = 22997,
    [SMALL_STATE(377)] = 23017,
    [SMALL_STATE(378)] = 23034,
    [SMALL_STATE(379)] = 23049,
    [SMALL_STATE(380)] = 23068,
    [SMALL_STATE(381)] = 23083,
    [SMALL_STATE(382)] = 23100,
    [SMALL_STATE(383)] = 23115,
    [SMALL_STATE(384)] = 23134,
    [SMALL_STATE(385)] = 23153,
    [SMALL_STATE(386)] = 23172,
    [SMALL_STATE(387)] = 23191,
    [SMALL_STATE(388)] = 23208,
    [SMALL_STATE(389)] = 23223,
    [SMALL_STATE(390)] = 23242,
    [SMALL_STATE(391)] = 23261,
    [SMALL_STATE(392)] = 23280,
    [SMALL_STATE(393)] = 23297,
    [SMALL_STATE(394)] = 23316,
    [SMALL_STATE(395)] = 23335,
    [SMALL_STATE(396)] = 23350,
    [SMALL_STATE(397)] = 23369,
    [SMALL_STATE(398)] = 23388,
    [SMALL_STATE(399)] = 23407,
    [SMALL_STATE(400)] = 23426,
    [SMALL_STATE(401)] = 23445,
    [SMALL_STATE(402)] = 23462,
    [SMALL_STATE(403)] = 23478,
    [SMALL_STATE(404)] = 23494,
    [SMALL_STATE(405)] = 23507,
    [SMALL_STATE(406)] = 23520,
    [SMALL_STATE(407)] = 23533,
    [SMALL_STATE(408)] = 23546,
    [SMALL_STATE(409)] = 23559,
    [SMALL_STATE(410)] = 23572,
    [SMALL_STATE(411)] = 23585,
    [SMALL_STATE(412)] = 23598,
    [SMALL_STATE(413)] = 23611,
    [SMALL_STATE(414)] = 23624,
    [SMALL_STATE(415)] = 23637,
    [SMALL_STATE(416)] = 23650,
    [SMALL_STATE(417)] = 23663,
    [SMALL_STATE(418)] = 23676,
    [SMALL_STATE(419)] = 23689,
    [SMALL_STATE(420)] = 23702,
    [SMALL_STATE(421)] = 23715,
    [SMALL_STATE(422)] = 23728,
    [SMALL_STATE(423)] = 23741,
    [SMALL_STATE(424)] = 23754,
    [SMALL_STATE(425)] = 23767,
    [SMALL_STATE(426)] = 23780,
    [SMALL_STATE(427)] = 23793,
    [SMALL_STATE(428)] = 23806,
    [SMALL_STATE(429)] = 23819,
    [SMALL_STATE(430)] = 23832,
    [SMALL_STATE(431)] = 23845,
    [SMALL_STATE(432)] = 23858,
    [SMALL_STATE(433)] = 23871,
    [SMALL_STATE(434)] = 23884,
    [SMALL_STATE(435)] = 23897,
    [SMALL_STATE(436)] = 23910,
    [SMALL_STATE(437)] = 23923,
    [SMALL_STATE(438)] = 23936,
    [SMALL_STATE(439)] = 23949,
    [SMALL_STATE(440)] = 23962,
    [SMALL_STATE(441)] = 23975,
    [SMALL_STATE(442)] = 23988,
    [SMALL_STATE(443)] = 24001,
    [SMALL_STATE(444)] = 24014,
    [SMALL_STATE(445)] = 24027,
    [SMALL_STATE(446)] = 24040,
    [SMALL_STATE(447)] = 24053,
    [SMALL_STATE(448)] = 24066,
    [SMALL_STATE(449)] = 24079,
    [SMALL_STATE(450)] = 24092,
    [SMALL_STATE(451)] = 24105,
    [SMALL_STATE(452)] = 24118,
    [SMALL_STATE(453)] = 24131,
    [SMALL_STATE(454)] = 24144,
    [SMALL_STATE(455)] = 24157,
    [SMALL_STATE(456)] = 24170,
    [SMALL_STATE(457)] = 24183,
    [SMALL_STATE(458)] = 24196,
    [SMALL_STATE(459)] = 24209,
    [SMALL_STATE(460)] = 24222,
    [SMALL_STATE(461)] = 24235,
    [SMALL_STATE(462)] = 24248,
    [SMALL_STATE(463)] = 24261,
    [SMALL_STATE(464)] = 24274,
    [SMALL_STATE(465)] = 24287,
    [SMALL_STATE(466)] = 24300,
    [SMALL_STATE(467)] = 24313,
    [SMALL_STATE(468)] = 24326,
    [SMALL_STATE(469)] = 24339,
    [SMALL_STATE(470)] = 24352,
    [SMALL_STATE(471)] = 24365,
    [SMALL_STATE(472)] = 24378,
    [SMALL_STATE(473)] = 24391,
    [SMALL_STATE(474)] = 24404,
    [SMALL_STATE(475)] = 24417,
    [SMALL_STATE(476)] = 24430,
    [SMALL_STATE(477)] = 24443,
    [SMALL_STATE(478)] = 24447,
};

static const TSParseActionEntry ts_parse_actions[] = {
    [0] = {.entry = {.count = 0, .reusable = false}},
    [1] = {.entry = {.count = 1, .reusable = false}},
    RECOVER(),
    [3] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(458),
    [5] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(453),
    [7] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_chunk, 0),
    [9] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(44),
    [11] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(19),
    [13] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(126),
    [15] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(281),
    [17] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(440),
    [19] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(293),
    [21] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(422),
    [23] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(10),
    [25] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(180),
    [27] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(11),
    [29] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(181),
    [31] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(376),
    [33] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(335),
    [35] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(368),
    [37] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(97),
    [39] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(97),
    [41] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(351),
    [43] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(375),
    [45] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(183),
    [47] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(123),
    [49] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(184),
    [51] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(184),
    [53] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(451),
    [55] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(25),
    [57] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(118),
    [59] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(247),
    [61] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(464),
    [63] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(248),
    [65] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(408),
    [67] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(15),
    [69] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(282),
    [71] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(175),
    [73] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(14),
    [75] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(174),
    [77] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(182),
    [79] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(16),
    [81] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(352),
    [83] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(333),
    [85] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(370),
    [87] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(71),
    [89] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(71),
    [91] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(365),
    [93] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(363),
    [95] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(177),
    [97] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(121),
    [99] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(196),
    [101] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(196),
    [103] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(472),
    [105] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(215),
    [107] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_elseif_statement, 3, .production_id = 30),
    [109] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__block, 1),
    [111] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(25),
    [114] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    [116] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(247),
    [119] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(464),
    [122] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(248),
    [125] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(408),
    [128] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(15),
    [131] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(175),
    [134] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(14),
    [137] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(174),
    [140] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(352),
    [143] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(333),
    [146] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(370),
    [149] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(71),
    [152] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(71),
    [155] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(365),
    [158] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(363),
    [161] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(177),
    [164] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(121),
    [167] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(196),
    [170] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(196),
    [173] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(472),
    [176] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(272),
    [178] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(214),
    [180] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(217),
    [182] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(300),
    [184] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(185),
    [186] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(148),
    [188] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(275),
    [190] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(261),
    [192] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(263),
    [194] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_else_statement, 1),
    [196] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(94),
    [198] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(56),
    [200] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_chunk, 1),
    [202] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_chunk, 2),
    [204] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    [206] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(44),
    [209] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(281),
    [212] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(440),
    [215] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(293),
    [218] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(422),
    [221] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(10),
    [224] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(180),
    [227] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(11),
    [230] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(181),
    [233] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(376),
    [236] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(335),
    [239] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(368),
    [242] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(97),
    [245] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(97),
    [248] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(351),
    [251] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(375),
    [254] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(183),
    [257] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(123),
    [260] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(184),
    [263] = {.entry = {.count = 2, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(184),
    [266] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 2),
    SHIFT_REPEAT(451),
    [269] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_expression, 1),
    [271] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_expression, 1),
    [273] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__variable_assignment_varlist, 1, .production_id = 5),
    [275] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(331),
    [277] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__prefix_expression, 1),
    [279] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__prefix_expression, 1),
    [281] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_dot_index_expression, 3, .production_id = 26),
    [283] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_dot_index_expression, 3, .production_id = 26),
    [285] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_variable, 1),
    [287] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_variable, 1),
    [289] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_bracket_index_expression, 4, .production_id = 26),
    [291] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_bracket_index_expression, 4, .production_id = 26),
    [293] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_arguments, 1),
    [295] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_arguments, 1),
    [297] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_string, 1, .production_id = 4),
    [299] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_string, 1, .production_id = 4),
    [301] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__quote_string, 2, .production_id = 11),
    [303] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__quote_string, 2, .production_id = 11),
    [305] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_table_constructor, 2),
    [307] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_table_constructor, 2),
    [309] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_arguments, 4),
    [311] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_arguments, 4),
    [313] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_arguments, 3),
    [315] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_arguments, 3),
    [317] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_function_call, 2, .production_id = 14),
    [319] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_function_call, 2, .production_id = 14),
    [321] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_arguments, 2),
    [323] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_arguments, 2),
    [325] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__block_string, 3, .production_id = 16),
    [327] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__block_string, 3, .production_id = 16),
    [329] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__quote_string, 3, .production_id = 23),
    [331] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__quote_string, 3, .production_id = 23),
    [333] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_parenthesized_expression, 3),
    [335] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_parenthesized_expression, 3),
    [337] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_table_constructor, 3),
    [339] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_table_constructor, 3),
    [341] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__variable_assignment_explist, 1, .production_id = 12),
    [343] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__variable_assignment_explist, 1, .production_id = 12),
    [345] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(173),
    [347] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(195),
    [349] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(194),
    [351] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(193),
    [353] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(195),
    [355] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(192),
    [357] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(172),
    [359] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(191),
    [361] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(190),
    [363] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(189),
    [365] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(189),
    [367] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(188),
    [369] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(188),
    [371] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(187),
    [373] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(186),
    [375] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_unary_expression, 2, .production_id = 13),
    [377] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_unary_expression, 2, .production_id = 13),
    [379] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_binary_expression, 3, .production_id = 25),
    [381] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_binary_expression, 3, .production_id = 25),
    [383] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__function_body, 2, .production_id = 20),
    [385] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_body, 2, .production_id = 20),
    [387] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__function_body, 3, .production_id = 36),
    [389] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_body, 3, .production_id = 36),
    [391] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_function_definition, 2, .production_id = 9),
    [393] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_function_definition, 2, .production_id = 9),
    [395] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__variable_assignment_explist_repeat1, 2, .production_id = 50),
    [397] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_explist_repeat1, 2, .production_id = 50),
    [399] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_repeat_statement, 3, .production_id = 18),
    [401] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_repeat_statement, 3, .production_id = 18),
    [403] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_repeat_statement, 4, .production_id = 31),
    [405] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_repeat_statement, 4, .production_id = 31),
    [407] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_statement, 1),
    [409] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_statement, 1),
    [411] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(208),
    [413] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(211),
    [415] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(213),
    [417] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(216),
    [419] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(211),
    [421] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(218),
    [423] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(219),
    [425] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(220),
    [427] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(221),
    [429] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(222),
    [431] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(222),
    [433] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(224),
    [435] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(224),
    [437] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(225),
    [439] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(226),
    [441] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(109),
    [443] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(345),
    [445] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_return_statement, 1),
    [447] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(386),
    [449] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(143),
    [451] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(143),
    [453] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(371),
    [455] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(373),
    [457] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(179),
    [459] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(122),
    [461] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(250),
    [463] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(250),
    [465] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(463),
    [467] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__att_name_list, 1, .production_id = 5),
    [469] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__att_name_list, 1, .production_id = 5),
    [471] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(425),
    [473] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(471),
    [475] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__att_name_list_repeat1, 2, .production_id = 28),
    [477] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__att_name_list_repeat1, 2, .production_id = 28),
    [479] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(158),
    [481] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(209),
    [483] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(32),
    [485] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(156),
    [487] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(73),
    [489] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(107),
    [491] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__att_name_list, 2, .production_id = 22),
    [493] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__att_name_list, 2, .production_id = 22),
    [495] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_return_statement, 1),
    [497] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__field_list, 3),
    [499] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__att_name_list_repeat1, 2, .production_id = 39),
    [501] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__att_name_list_repeat1, 2, .production_id = 39),
    [503] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__att_name_list_repeat1, 2, .production_id = 39),
    SHIFT_REPEAT(425),
    [506] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__att_name_list, 3, .production_id = 38),
    [508] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__att_name_list, 3, .production_id = 38),
    [510] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__field_list, 2),
    [512] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__att_name_list, 2, .production_id = 21),
    [514] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__att_name_list, 2, .production_id = 21),
    [516] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(246),
    [518] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(245),
    [520] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(243),
    [522] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(242),
    [524] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(241),
    [526] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(241),
    [528] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(240),
    [530] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(240),
    [532] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(239),
    [534] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(238),
    [536] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(259),
    [538] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(249),
    [540] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(259),
    [542] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(476),
    [544] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(466),
    [546] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__variable_assignment_explist_repeat1, 2, .production_id = 51),
    [548] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_explist_repeat1, 2, .production_id = 51),
    [550] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_explist_repeat1, 2, .production_id = 51),
    SHIFT_REPEAT(173),
    [553] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(234),
    [555] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__variable_assignment_explist, 2, .production_id = 42),
    [557] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__variable_assignment_explist, 2, .production_id = 42),
    [559] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__att_name_list_repeat1, 3, .production_id = 49),
    [561] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__att_name_list_repeat1, 3, .production_id = 49),
    [563] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__attrib, 3),
    [565] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__attrib, 3),
    [567] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_variable_declaration, 2, .production_id = 10),
    [569] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_variable_declaration, 2, .production_id = 10),
    [571] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(167),
    [573] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(398),
    [575] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(393),
    [577] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(113),
    [579] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(69),
    [581] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(36),
    [583] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 7, .production_id = 57),
    [585] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 7, .production_id = 57),
    [587] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 6, .production_id = 54),
    [589] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 6, .production_id = 54),
    [591] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 6, .production_id = 53),
    [593] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 6, .production_id = 53),
    [595] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__att_name_list_repeat1, 2, .production_id = 39),
    SHIFT_REPEAT(476),
    [598] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 6, .production_id = 52),
    [600] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 6, .production_id = 52),
    [602] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_for_statement, 5, .production_id = 48),
    [604] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_for_statement, 5, .production_id = 48),
    [606] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 5, .production_id = 46),
    [608] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 5, .production_id = 46),
    [610] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 5, .production_id = 45),
    [612] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 5, .production_id = 45),
    [614] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 5, .production_id = 44),
    [616] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 5, .production_id = 44),
    [618] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_while_statement, 5, .production_id = 43),
    [620] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_while_statement, 5, .production_id = 43),
    [622] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__local_variable_assignment, 3, .production_id = 40),
    [624] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__local_variable_assignment, 3, .production_id = 40),
    [626] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__local_function_declaration, 4, .production_id = 37),
    [628] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__local_function_declaration, 4, .production_id = 37),
    [630] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_for_statement, 4, .production_id = 33),
    [632] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_for_statement, 4, .production_id = 33),
    [634] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_if_statement, 4, .production_id = 30),
    [636] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_if_statement, 4, .production_id = 30),
    [638] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_while_statement, 4, .production_id = 30),
    [640] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_while_statement, 4, .production_id = 30),
    [642] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_assignment_statement, 3, .production_id = 24),
    [644] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_assignment_statement, 3, .production_id = 24),
    [646] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_function_declaration, 3, .production_id = 19),
    [648] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_function_declaration, 3, .production_id = 19),
    [650] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_do_statement, 3, .production_id = 17),
    [652] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_do_statement, 3, .production_id = 17),
    [654] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_label_statement, 3),
    [656] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_label_statement, 3),
    [658] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_empty_statement, 1),
    [660] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_empty_statement, 1),
    [662] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_chunk_repeat1, 1),
    [664] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_chunk_repeat1, 1),
    [666] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_statement, 1, .production_id = 1),
    [668] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_statement, 1, .production_id = 1),
    [670] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_declaration, 1),
    [672] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_declaration, 1),
    [674] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_declaration, 1, .production_id = 2),
    [676] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_declaration, 1, .production_id = 2),
    [678] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_declaration, 1, .production_id = 3),
    [680] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_declaration, 1, .production_id = 3),
    [682] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_variable_declaration, 2),
    [684] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_variable_declaration, 2),
    [686] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_goto_statement, 2),
    [688] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_goto_statement, 2),
    [690] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_do_statement, 2),
    [692] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_do_statement, 2),
    [694] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__expression_list_repeat1, 2),
    [696] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__expression_list_repeat1, 2),
    [698] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(251),
    [700] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_explist_repeat1, 2, .production_id = 51),
    SHIFT_REPEAT(208),
    [703] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__expression_list, 1),
    [705] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(230),
    [707] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__expression_list, 1),
    [709] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(168),
    [711] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_parameters, 2),
    [713] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_parameters, 2),
    [715] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_parameters, 3, .production_id = 35),
    [717] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_parameters, 3, .production_id = 35),
    [719] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(108),
    [721] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(70),
    [723] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_field, 1, .production_id = 12),
    [725] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_field, 3, .production_id = 41),
    [727] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_field, 5, .production_id = 56),
    [729] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(34),
    [731] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(244),
    [733] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_for_numeric_clause, 5, .production_id = 55),
    [735] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(112),
    [737] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(39),
    [739] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(13),
    [741] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(59),
    [743] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(45),
    [745] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_for_numeric_clause, 7, .production_id = 58),
    [747] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(223),
    [749] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(3),
    [751] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(2),
    [753] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(9),
    [755] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(4),
    [757] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(444),
    [759] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(26),
    [761] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(110),
    [763] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__field_sep, 1),
    [765] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__field_sep, 1),
    [767] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(450),
    [769] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(423),
    [771] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(171),
    [773] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(176),
    [775] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(419),
    [777] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(359),
    [779] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(358),
    [781] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(166),
    [783] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(178),
    [785] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(124),
    [787] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(475),
    [789] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(420),
    [791] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(169),
    [793] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(227),
    [795] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_varlist_repeat1, 2, .production_id = 28),
    [797] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__expression_list_repeat1, 2),
    SHIFT_REPEAT(230),
    [800] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(44),
    [802] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__expression_list, 2),
    [804] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__expression_list, 2),
    [806] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(388),
    [808] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(357),
    [810] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(199),
    [812] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(182),
    [814] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(204),
    [816] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_return_statement, 2),
    [818] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(347),
    [820] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_return_statement, 2),
    [822] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(206),
    [824] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(292),
    [826] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(297),
    [828] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(288),
    [830] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_if_statement_repeat1, 2, .production_id = 47),
    [832] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym_if_statement_repeat1, 2, .production_id = 47),
    SHIFT_REPEAT(182),
    [835] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_if_statement_repeat1, 2, .production_id = 47),
    [837] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__field_list_repeat1, 2),
    SHIFT_REPEAT(322),
    [840] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__field_list_repeat1, 2),
    [842] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(322),
    [844] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_return_statement, 3),
    [846] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_return_statement, 3),
    [848] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__field_list, 1),
    [850] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_method_index_expression, 3, .production_id = 27),
    [852] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__block, 1),
    [854] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(75),
    [856] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(387),
    [858] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(458),
    [860] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(366),
    [862] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(60),
    [864] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(377),
    [866] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__doublequote_string_content, 2),
    [868] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__doublequote_string_content, 2),
    SHIFT_REPEAT(387),
    [871] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(115),
    [873] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(384),
    [875] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(445),
    [877] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(274),
    [879] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(104),
    [881] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__singlequote_string_content, 2),
    [883] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__singlequote_string_content, 2),
    SHIFT_REPEAT(377),
    [886] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(38),
    [888] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(31),
    [890] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__name_list_repeat1, 2, .production_id = 29),
    SHIFT_REPEAT(405),
    [893] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__name_list_repeat1, 2, .production_id = 29),
    [895] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(237),
    [897] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(405),
    [899] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__name_list, 1, .production_id = 5),
    [901] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__block, 2),
    [903] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym__block, 2),
    [905] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(147),
    [907] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(455),
    [909] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(144),
    [911] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(119),
    [913] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(469),
    [915] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(155),
    [917] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__singlequote_string_content, 1),
    [919] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__singlequote_string_content, 1),
    [921] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__name_list_repeat1, 2, .production_id = 28),
    [923] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_name_prefix_expression, 1, .production_id = 7),
    [925] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_varlist_repeat1, 2, .production_id = 29),
    [927] = {.entry = {.count = 2, .reusable = true}},
    REDUCE(aux_sym__variable_assignment_varlist_repeat1, 2, .production_id = 29),
    SHIFT_REPEAT(331),
    [930] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_name_dot_index_expression, 3, .production_id = 26),
    [932] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(473),
    [934] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(457),
    [936] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_name, 1),
    [938] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(403),
    [940] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__parameter_list, 1, .production_id = 5),
    [942] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym__doublequote_string_content, 1),
    [944] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym__doublequote_string_content, 1),
    [946] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_name_prefix_expression, 1),
    [948] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(402),
    [950] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__parameter_list, 2, .production_id = 15),
    [952] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(33),
    [954] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_elseif_statement, 4, .production_id = 44),
    [956] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(sym_elseif_statement, 4, .production_id = 44),
    [958] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__name_list, 2, .production_id = 15),
    [960] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(114),
    [962] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(57),
    [964] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__variable_assignment_varlist, 2, .production_id = 15),
    [966] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(aux_sym_if_statement_repeat1, 1, .production_id = 32),
    [968] = {.entry = {.count = 1, .reusable = false}},
    REDUCE(aux_sym_if_statement_repeat1, 1, .production_id = 32),
    [970] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(378),
    [972] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(448),
    [974] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(413),
    [976] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_chunk, 3),
    [978] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(62),
    [980] = {.entry = {.count = 1, .reusable = true}},
    ACCEPT_INPUT(),
    [982] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(262),
    [984] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_else_statement, 2, .production_id = 17),
    [986] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(289),
    [988] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(164),
    [990] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(283),
    [992] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__parameter_list, 3, .production_id = 5),
    [994] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(154),
    [996] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(236),
    [998] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(235),
    [1000] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(232),
    [1002] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(139),
    [1004] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(111),
    [1006] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(42),
    [1008] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(299),
    [1010] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(273),
    [1012] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(349),
    [1014] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(145),
    [1016] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(120),
    [1018] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(90),
    [1020] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(207),
    [1022] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(280),
    [1024] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(205),
    [1026] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_name, 1, .production_id = 8),
    [1028] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(203),
    [1030] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(161),
    [1032] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(294),
    [1034] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(202),
    [1036] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(279),
    [1038] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(198),
    [1040] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(197),
    [1042] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(233),
    [1044] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(163),
    [1046] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(474),
    [1048] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(285),
    [1050] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(37),
    [1052] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(229),
    [1054] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__parameter_list, 1),
    [1056] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(277),
    [1058] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(7),
    [1060] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__parameter_list, 4, .production_id = 15),
    [1062] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(40),
    [1064] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(24),
    [1066] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(461),
    [1068] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(58),
    [1070] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(456),
    [1072] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_for_generic_clause, 3, .production_id = 34),
    [1074] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(379),
    [1076] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(478),
    [1078] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(470),
    [1080] = {.entry = {.count = 1, .reusable = false}},
    SHIFT(477),
    [1082] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(101),
    [1084] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(165),
    [1086] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(74),
    [1088] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(105),
    [1090] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(414),
    [1092] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(415),
    [1094] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(467),
    [1096] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(265),
    [1098] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(8),
    [1100] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(390),
    [1102] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym__function_name_method_index_expression, 3, .production_id = 27),
    [1104] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(432),
    [1106] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(443),
    [1108] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(382),
    [1110] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(295),
    [1112] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(459),
    [1114] = {.entry = {.count = 1, .reusable = true}},
    SHIFT(170),
    [1116] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_comment, 2, .production_id = 6),
    [1118] = {.entry = {.count = 1, .reusable = true}},
    REDUCE(sym_comment, 3, .production_id = 16),
};

#ifdef __cplusplus
extern "C" {
#endif
void *tree_sitter_lua_external_scanner_create(void);
void tree_sitter_lua_external_scanner_destroy(void *);
bool tree_sitter_lua_external_scanner_scan(void *, TSLexer *, const bool *);
unsigned tree_sitter_lua_external_scanner_serialize(void *, char *);
void tree_sitter_lua_external_scanner_deserialize(void *, const char *, unsigned);

#ifdef _WIN32
#define extern __declspec(dllexport)
#endif

extern const TSLanguage *tree_sitter_lua(void) {
    static const TSLanguage language = {
        .version = LANGUAGE_VERSION,
        .symbol_count = SYMBOL_COUNT,
        .alias_count = ALIAS_COUNT,
        .token_count = TOKEN_COUNT,
        .external_token_count = EXTERNAL_TOKEN_COUNT,
        .state_count = STATE_COUNT,
        .large_state_count = LARGE_STATE_COUNT,
        .production_id_count = PRODUCTION_ID_COUNT,
        .field_count = FIELD_COUNT,
        .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
        .parse_table = &ts_parse_table[0][0],
        .small_parse_table = ts_small_parse_table,
        .small_parse_table_map = ts_small_parse_table_map,
        .parse_actions = ts_parse_actions,
        .symbol_names = ts_symbol_names,
        .field_names = ts_field_names,
        .field_map_slices = ts_field_map_slices,
        .field_map_entries = ts_field_map_entries,
        .symbol_metadata = ts_symbol_metadata,
        .public_symbol_map = ts_symbol_map,
        .alias_map = ts_non_terminal_alias_map,
        .alias_sequences = &ts_alias_sequences[0][0],
        .lex_modes = ts_lex_modes,
        .lex_fn = ts_lex,
        .keyword_lex_fn = ts_lex_keywords,
        .keyword_capture_token = sym_identifier,
        .external_scanner = {
            &ts_external_scanner_states[0][0],
            ts_external_scanner_symbol_map,
            tree_sitter_lua_external_scanner_create,
            tree_sitter_lua_external_scanner_destroy,
            tree_sitter_lua_external_scanner_scan,
            tree_sitter_lua_external_scanner_serialize,
            tree_sitter_lua_external_scanner_deserialize,
        },
        .primary_state_ids = ts_primary_state_ids,
    };
    return &language;
}
#ifdef __cplusplus
}
#endif
