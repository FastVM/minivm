...
void vm_ir_info(size_t *ptr_nops, vm_ir_block_t **ptr_blocks) {
  ...
  for (size_t i = 0; i < nblocks; i++) {
    ...
    for (size_t j = 0; j < 2; j++) {
      if (block->branch->args[j].type != VM_IR_ARG_NONE &&
          block->branch->args[j].type == VM_IR_ARG_REG &&
  ...
  for (size_t i = 0; i < nblocks; i++) {
    ...
    for (size_t j = 0; j < block->len; j++) {
      ...
      if (instr->out.type == VM_IR_ARG_REG &&
          regs[instr->out.reg] == VM_IR_INFO_REG_UNK) {
    ...
    for (size_t j = 0; j < 2; j++) {
      if (block->branch->args[j].type != VM_IR_ARG_NONE &&
          block->branch->args[j].type == VM_IR_ARG_REG &&
          regs[block->branch->args[j].reg] == VM_IR_INFO_REG_UNK) {
        ...
        if (bi == block->nargs) {
          while (ti < target->nargs) {
            ...
            if (newreg >= blocks[i].nregs) {
              all_regs[i] =
                  vm_realloc(all_regs[i], sizeof(uint8_t) * (newreg + 1));
