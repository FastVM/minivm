...
static void vm_ir_block_realloc(vm_ir_block_t *block, vm_ir_instr_t *instr) {
  if (block->len + 4 >= block->alloc) {
    ...
    block->instrs =
        vm_realloc(block->instrs, sizeof(vm_ir_arg_t) * block->alloc);
  }
...
void vm_ir_block_add_move(vm_ir_block_t *block, vm_ir_arg_t out,
                          vm_ir_arg_t arg) {
  vm_ir_block_realloc(block, vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_MOVE,
                                       .out = out, .args[0] = arg));
}

void vm_ir_block_add_add(vm_ir_block_t *block, vm_ir_arg_t out, vm_ir_arg_t lhs,
                         vm_ir_arg_t rhs) {
  vm_ir_block_realloc(block,
                      vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_ADD, .out = out,
                                .args[0] = lhs, .args[1] = rhs));
}
void vm_ir_block_add_sub(vm_ir_block_t *block, vm_ir_arg_t out, vm_ir_arg_t lhs,
                         vm_ir_arg_t rhs) {
  vm_ir_block_realloc(block,
                      vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_SUB, .out = out,
                                .args[0] = lhs, .args[1] = rhs));
}
void vm_ir_block_add_mul(vm_ir_block_t *block, vm_ir_arg_t out, vm_ir_arg_t lhs,
                         vm_ir_arg_t rhs) {
  vm_ir_block_realloc(block,
                      vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_MUL, .out = out,
                                .args[0] = lhs, .args[1] = rhs));
}
void vm_ir_block_add_div(vm_ir_block_t *block, vm_ir_arg_t out, vm_ir_arg_t lhs,
                         vm_ir_arg_t rhs) {
  vm_ir_block_realloc(block,
                      vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_DIV, .out = out,
                                .args[0] = lhs, .args[1] = rhs));
}
void vm_ir_block_add_mod(vm_ir_block_t *block, vm_ir_arg_t out, vm_ir_arg_t lhs,
                         vm_ir_arg_t rhs) {
  vm_ir_block_realloc(block,
                      vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_MOD, .out = out,
                                .args[0] = lhs, .args[1] = rhs));
}
void vm_ir_block_add_call(vm_ir_block_t *block, vm_ir_arg_t out,
                          vm_ir_arg_t func, size_t nargs, vm_ir_arg_t *args) {
  vm_ir_instr_t *instr = vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_CALL,
                                   .out = out, .args[0] = func);
  size_t i = 1;
  while (i <= nargs) {
    instr->args[i++] = *args++;
...
void vm_ir_block_add_arr(vm_ir_block_t *block, vm_ir_arg_t out,
                         vm_ir_arg_t num) {
  vm_ir_block_realloc(block, vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_ARR,
                                       .out = out, .args[0] = num));
}
void vm_ir_block_add_get(vm_ir_block_t *block, vm_ir_arg_t out, vm_ir_arg_t obj,
                         vm_ir_arg_t index) {
  vm_ir_block_realloc(block,
                      vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_GET, .out = out,
                                .args[0] = obj, .args[1] = index));
}
void vm_ir_block_add_len(vm_ir_block_t *block, vm_ir_arg_t out,
                         vm_ir_arg_t obj) {
  vm_ir_block_realloc(block, vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_LEN,
                                       .out = out, .args[0] = obj));
}
void vm_ir_block_add_type(vm_ir_block_t *block, vm_ir_arg_t out,
                          vm_ir_arg_t obj) {
  vm_ir_block_realloc(block, vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_TYPE,
                                       .out = out, .args[0] = obj));
}
void vm_ir_block_add_set(vm_ir_block_t *block, vm_ir_arg_t obj,
                         vm_ir_arg_t index, vm_ir_arg_t value) {
  vm_ir_block_realloc(block, vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_SET,
                                       .args[0] = obj, .args[1] = index,
                                       .args[2] = value));
}
void vm_ir_block_add_out(vm_ir_block_t *block, vm_ir_arg_t arg) {
  vm_ir_block_realloc(
      block, vm_ir_new(vm_ir_instr_t, .op = VM_IR_IOP_OUT, .args[0] = arg));
}

void vm_ir_block_end_jump(vm_ir_block_t *block, vm_ir_block_t *target) {
  block->branch =
      vm_ir_new(vm_ir_branch_t, .op = VM_IR_BOP_JUMP, .targets[0] = target);
}
void vm_ir_block_end_bb(vm_ir_block_t *block, vm_ir_arg_t val,
                        vm_ir_block_t *iffalse, vm_ir_block_t *iftrue) {
  block->branch =
      vm_ir_new(vm_ir_branch_t, .op = VM_IR_BOP_BOOL, .args[0] = val,
                .targets[0] = iffalse, .targets[1] = iftrue);
}
void vm_ir_block_end_blt(vm_ir_block_t *block, vm_ir_arg_t lhs, vm_ir_arg_t rhs,
                         vm_ir_block_t *iffalse, vm_ir_block_t *iftrue) {
  block->branch =
      vm_ir_new(vm_ir_branch_t, .op = VM_IR_BOP_LESS, .args[0] = lhs,
                .args[1] = rhs, .targets[0] = iffalse, .targets[1] = iftrue);
}
void vm_ir_block_end_beq(vm_ir_block_t *block, vm_ir_arg_t lhs, vm_ir_arg_t rhs,
                         vm_ir_block_t *iffalse, vm_ir_block_t *iftrue) {
  block->branch =
      vm_ir_new(vm_ir_branch_t, .op = VM_IR_BOP_EQUAL, .args[0] = lhs,
                .args[1] = rhs, .targets[0] = iffalse, .targets[1] = iftrue);
}
void vm_ir_block_end_ret(vm_ir_block_t *block, vm_ir_arg_t arg) {
  block->branch =
      vm_ir_new(vm_ir_branch_t, .op = VM_IR_BOP_RET, .args[0] = arg);
}
