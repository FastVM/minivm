...
void vm_ir_read(vm_ir_read_t *state, size_t *index) {
  ...
  while (*index < nops) {
    ...
    case VM_OPCODE_CALL: {
      ...
      state->nops = tmp;
      vm_ir_block_add_call(block, vm_ir_arg_reg(rreg),
                           vm_ir_arg_func(&blocks[func]), nargs, args);
      vm_free(args);
      break;
    }
    case VM_OPCODE_DCALL: {
      ...
      vm_ir_block_add_call(block, vm_ir_arg_reg(rreg), vm_ir_arg_reg(func),
                           nargs, args);
      vm_free(args);
      break;
    }
    case VM_OPCODE_XCALL: {
      ...
      vm_ir_block_add_call(block, vm_ir_arg_reg(rreg), vm_ir_arg_extern(func),
                           nargs, args);
      vm_free(args);
      break;
    }
    case VM_OPCODE_ADDR: {
      ...
      vm_ir_read_from(state, func);
      state->nops = tmp;
      vm_ir_block_add_move(block, vm_ir_arg_reg(reg),
                           vm_ir_arg_func(&blocks[func]));
      break;
    }
    case VM_OPCODE_RET: {
      vm_opcode_t reg = ops[(*index)++];
    ...
    case VM_OPCODE_ADD: {
      ...
      vm_ir_block_add_add(block, vm_ir_arg_reg(out), vm_ir_arg_reg(lhs),
                          vm_ir_arg_reg(rhs));
      break;
    }
    case VM_OPCODE_SUB: {
      ...
      vm_opcode_t lhs = ops[(*index)++];
      vm_opcode_t rhs = ops[(*index)++];
      vm_ir_block_add_sub(block, vm_ir_arg_reg(out), vm_ir_arg_reg(lhs),
                          vm_ir_arg_reg(rhs));
      break;
    }
    case VM_OPCODE_MUL: {
      ...
      vm_opcode_t lhs = ops[(*index)++];
      vm_opcode_t rhs = ops[(*index)++];
      vm_ir_block_add_mul(block, vm_ir_arg_reg(out), vm_ir_arg_reg(lhs),
                          vm_ir_arg_reg(rhs));
    ...
    case VM_OPCODE_DIV: {
      ...
      vm_opcode_t lhs = ops[(*index)++];
      vm_opcode_t rhs = ops[(*index)++];
      vm_ir_block_add_div(block, vm_ir_arg_reg(out), vm_ir_arg_reg(lhs),
                          vm_ir_arg_reg(rhs));
      break;
    }
    case VM_OPCODE_MOD: {
      ...
      vm_opcode_t lhs = ops[(*index)++];
      vm_opcode_t rhs = ops[(*index)++];
      vm_ir_block_add_mod(block, vm_ir_arg_reg(out), vm_ir_arg_reg(lhs),
                          vm_ir_arg_reg(rhs));
      break;
    }
    case VM_OPCODE_BB: {
    ...
    case VM_OPCODE_BEQ: {
      ...
      vm_ir_read_from(state, iff);
      vm_ir_read_from(state, ift);
      vm_ir_block_end_beq(block, vm_ir_arg_reg(lhs), vm_ir_arg_reg(rhs),
                          &blocks[iff], &blocks[ift]);
      return;
    }
    case VM_OPCODE_BLT: {
      vm_opcode_t lhs = ops[(*index)++];
      ...
      vm_ir_read_from(state, iff);
      vm_ir_read_from(state, ift);
      vm_ir_block_end_blt(block, vm_ir_arg_reg(lhs), vm_ir_arg_reg(rhs),
                          &blocks[iff], &blocks[ift]);
      return;
    }
    case VM_OPCODE_STR: {
    ...
    case VM_OPCODE_SET: {
      ...
      vm_opcode_t val = ops[(*index)++];
      vm_ir_block_add_set(block, vm_ir_arg_reg(obj), vm_ir_arg_reg(ind),
                          vm_ir_arg_reg(val));
      break;
    }
    case VM_OPCODE_GET: {
      ...
      vm_opcode_t obj = ops[(*index)++];
      vm_opcode_t ind = ops[(*index)++];
      vm_ir_block_add_get(block, vm_ir_arg_reg(reg), vm_ir_arg_reg(obj),
                          vm_ir_arg_reg(ind));
      break;
    }
    case VM_OPCODE_LEN: {
