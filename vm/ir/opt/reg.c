...
static inline size_t vm_ir_opt_use(uint8_t *used, size_t *regs, size_t reg,
                                   size_t i, size_t *max) {
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    for (size_t j = 0; j < block->branch->targets[i]->nargs; j++) {
      if (pass[j].type == VM_IR_ARG_REG) {
        pass[j].reg = vm_ir_opt_use(used, regs, pass[j].reg,
                                    block->branch->targets[i]->args[j], &max);
      }
    }
  }
  for (size_t i = 0; i < 2; i++) {
    if (block->branch->args[i].type == VM_IR_ARG_REG) {
      block->branch->args[i].reg =
          vm_ir_opt_alloc(used, regs, block->branch->args[i].reg, &max);
    }
  }
