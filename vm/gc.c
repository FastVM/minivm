...
void vm_gc_run(vm_gc_t *restrict gc) {
  ...
  }
  {
    if (gc->buf_used > gc->move_alloc) {
      ...
      gc->move_buf =
          vm_realloc(gc->move_buf, sizeof(vm_int_t) * gc->move_alloc);
    }
  }
  vm_int_t used = 0;
  {
...
vm_int_t vm_gc_arr(vm_gc_t *restrict gc, vm_int_t size) {
  ...
  for (vm_int_t i = ret; i < next_head; i++) {
    gc->buf[i].value = VM_VALUE_SET_INT(0);
  }
...
vm_value_t vm_gc_get(vm_gc_t *restrict gc, vm_value_t obj, vm_value_t ival) {
  size_t index = VM_VALUE_GET_INT(ival);
  if (index >= vm_gc_len(gc, obj)) {
    // __builtin_trap();
    fprintf(stderr, "bounds error: %zi >= %zi", index, vm_gc_len(gc, obj));
    return VM_VALUE_SET_INT(0);
  }
  return gc->buf[VM_VALUE_GET_ARR(obj) + index].value;
}

void vm_gc_set(vm_gc_t *restrict gc, vm_value_t obj, vm_value_t ival,
               vm_value_t value) {
  size_t index = VM_VALUE_GET_INT(ival);
  if (index >= vm_gc_len(gc, obj)) {
    fprintf(stderr, "bounds error: %zi >= %zi", index, vm_gc_len(gc, obj));
...
vm_int_t vm_gc_len(vm_gc_t *restrict gc, vm_value_t obj) {
  vm_int_t iptr = VM_VALUE_GET_ARR(obj);
  if (0 >= iptr || iptr >= gc->buf_used) {
    fprintf(stderr, "invalid ptr: %li\n", iptr);
    // __builtin_trap();
    return 0;
  }
  return gc->buf[iptr - 1].header.len;
}
