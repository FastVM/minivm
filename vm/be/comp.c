...
vm_opcode_t *vm_run_comp(vm_state_t *state, vm_rblock_t *rblock) {
    ...
    for (size_t ninstr = rnext->start; ninstr < block->len; ninstr++) {
        ...
        switch (instr.op) {
        case VM_IOP_CAST: {
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I8_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I16_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I32_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_I64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_I64_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U8) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U8_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U16) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U16_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U32_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_U64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_U64_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F32) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F32_F64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_I8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_I16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_I32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_I64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_I64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U8)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_U8);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U16)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_U16);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_U32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_U64)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_U64);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F32)) {
                ops[nops++].VM_OPCODE_PTR = VM_STATE_LOAD_PTR(state, VM_OPCODE_CAST_F64_F32);
                ops[nops++].reg = vm_instr_get_arg_reg(instr, 0);
                ops[nops++].reg = instr.out.reg;
            }
            if (vm_tag_eq(instr.tag, VM_TAG_F64) && vm_tag_eq(vm_instr_get_arg_type(instr, 0), VM_TAG_F64)) {
        ...
        case VM_IOP_MOVE: {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_I16)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_I32)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_I64)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_U8)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_U16)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_U32)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_U64)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_F32)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
            ...
            if (vm_tag_eq(instr.tag, VM_TAG_F64)) {
                if (vm_instr_get_arg_type(instr, 0) == VM_ARG_REG) {
    ...
    switch (branch.op) {
        ...
        case VM_BOP_RET: {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_I16)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_I32)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_I64)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_U8)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_U16)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_U32)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_U64)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_F32)) {
                if (branch.args[0].type != VM_ARG_REG) {
            ...
            if (vm_tag_eq(branch.tag, VM_TAG_F64)) {
                if (branch.args[0].type != VM_ARG_REG) {
