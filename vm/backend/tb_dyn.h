...
void vm_tb_dyn_set(vm_tb_dyn_state_t *state, vm_arg_t out, TB_Node *type, TB_Node *value) {

}

void vm_tb_dyn_set_type(vm_tb_dyn_state_t *state, vm_arg_t out, TB_Node *type, TB_Node *value) {
    vm_tb_dyn_set(state, out, vm_tb_dyn_ptr(state, type), value);
}

void vm_tb_dyn_block(vm_tb_dyn_state_t *state, vm_block_t *block) {
    printf("ENTER %zi\n", block->id);
    ...
    switch (block->branch.op) {
        ...
        case VM_BOP_JUMP: {
            vm_tb_dyn_block(state, block->branch.targets[0]);
            break;
        }
        ...
        case VM_BOP_CALL: {
...

            {
                ...
                TB_Node *call_arg = tb_inst_local(state->func, sizeof(vm_std_value_t) * (num_args / 2 + 1), 8);
...
vm_tb_dyn_func_t *vm_tb_dyn_comp(vm_tb_dyn_state_t *state, vm_block_t *entry) {
    ...
    for (size_t block_num = 0; block_num < state->blocks->len; block_num++) {
        ...
        if (block->isfunc || block == entry) {
            char buf[64];
            snprintf(buf, 63, "vm_block_func_%zi", block->id);
