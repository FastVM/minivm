...
#if (!defined(AO_HAVE_test_and_set_acquire) || defined(GC_RTEMS_PTHREADS) || defined(SN_TARGET_PS3) || defined(GC_WIN32_THREADS) || defined(BASE_ATOMIC_OPS_EMULATED) || defined(LINT2)) && defined(GC_PTHREADS)
#define USE_PTHREAD_LOCKS
#undef USE_SPIN_LOCK
#if (defined(LINT2) || defined(GC_WIN32_THREADS)) && !defined(NO_PTHREAD_TRYLOCK)
...
#if defined(GC_WIN32_THREADS) && !defined(USE_PTHREAD_LOCKS) || defined(GC_PTHREADS)
...
#elif (!defined(THREAD_LOCAL_ALLOC) || defined(USE_SPIN_LOCK)) && !defined(USE_PTHREAD_LOCKS) && !defined(THREAD_SANITIZER)
...
#define LOCK()                              \
    do {                                    \
        if (GC_need_to_lock) UNCOND_LOCK(); \
    } while (0)
#define UNLOCK()                              \
    do {                                      \
        if (GC_need_to_lock) UNCOND_UNLOCK(); \
    } while (0)
#endif
#endif
