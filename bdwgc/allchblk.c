...
STATIC int GC_hblk_fl_from_blocks(size_t blocks_needed) {
    ...
    return (int)(blocks_needed - UNIQUE_THRESHOLD) / FL_COMPRESSION + UNIQUE_THRESHOLD;
}
...
static GC_bool setup_header(hdr *hhdr, struct hblk *block, size_t byte_sz,
...
#ifdef MARK_BIT_PER_GRANULE
    {
        ...
        hhdr->hb_map = GC_obj_map[(hhdr->hb_flags & LARGE_BLOCK) != 0 ? 0 : granules];
    }
...
STATIC void GC_add_to_fl(struct hblk *h, hdr *hhdr) {
...
#if defined(GC_ASSERTIONS) && !defined(USE_MUNMAP)
    {
        ...
        GC_ASSERT(nexthdr == 0 || !HBLK_IS_FREE(nexthdr) || (GC_heapsize & SIGNB) != 0);
        /* In the last case, blocks may be too large to merge. */
        GC_ASSERT(NULL == prev || !HBLK_IS_FREE(prevhdr) || (GC_heapsize & SIGNB) != 0);
    }
#endif
    GC_ASSERT(modHBLKSZ(hhdr->hb_sz) == 0);
...
GC_INNER void GC_unmap_old(unsigned threshold) {
    ...
    for (i = 0; i <= N_HBLK_FLS; ++i) {
        ...
        for (h = GC_hblkfreelist[i]; 0 != h; h = hhdr->hb_next) {
            ...
            if ((unsigned short)(GC_gc_no - hhdr->hb_last_reclaimed) >= (unsigned short)threshold) {
#ifdef COUNT_UNMAPPED_REGIONS
...
GC_INNER void GC_merge_unmapped(void) {
    ...
    for (i = 0; i <= N_HBLK_FLS; ++i) {
        ...
        while (h != 0) {
            ...
            if (nexthdr != NULL && HBLK_IS_FREE(nexthdr) && !((size + (nextsize = nexthdr->hb_sz)) & SIGNB)
                /* no overflow */) {
...
GC_INNER struct hblk *GC_allochblk(size_t sz, int kind,
    ...
    if (EXPECT(SIZET_SAT_ADD(blocks * HBLKSIZE, align_m1) >= (GC_SIZE_MAX >> 1), FALSE))
    ...
    may_split = TRUE;
    if (GC_use_entire_heap || GC_dont_gc || GC_heapsize - GC_large_free_bytes < GC_requested_heapsize || GC_incremental || !GC_should_collect()) {
...
static GC_bool next_hblk_fits_better(hdr *hhdr, word size_avail,
    ...
    return next_size >= size_needed + next_ofs && !GC_is_black_listed(next_hbp + divHBLKSZ(next_ofs), size_needed);
}
...
retry:
    ...
    for (hbp = GC_hblkfreelist[n];; hbp = hhdr->hb_next) {
        ...
        if (IS_UNCOLLECTABLE(kind) || (kind == PTRFREE && size_needed <= MAX_BLACK_LIST_ALLOC)) {
        ...
        if (last_hbp != NULL) {
            ...
            if (may_split == AVOID_SPLIT_REMAPPED && last_hbp != hbp && !IS_MAPPED(hhdr))
                continue;
#endif
            break;
        }
        ...
        if (size_needed == HBLKSIZE && 0 == align_m1 && !GC_find_leak && IS_MAPPED(hhdr) && (++GC_drop_blacklisted_count & 3) == 0) {
            struct hblk *prev = hhdr->hb_prev;
...
GC_INNER void GC_freehblk(struct hblk *hbp) {
    ...
    /* Coalesce with successor, if possible */
    if (nexthdr != NULL && HBLK_IS_FREE(nexthdr) && IS_MAPPED(nexthdr) && !((hhdr->hb_sz + nexthdr->hb_sz) & SIGNB) /* no overflow */) {
        GC_remove_from_fl(nexthdr);
        hhdr->hb_sz += nexthdr->hb_sz;
        GC_remove_header(next);
    }
    ...
    if (prev /* != NULL */) { /* CPPCHECK */
        prevhdr = HDR(prev);
        if (IS_MAPPED(prevhdr) && !((hhdr->hb_sz + prevhdr->hb_sz) & SIGNB)) {
