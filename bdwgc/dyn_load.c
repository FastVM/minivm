...
#if !defined(MACOS) && !defined(GC_NO_TYPES) && !defined(SN_TARGET_PSP2) && !defined(_WIN32_WCE) && !defined(__CC_ARM)
#include <sys/types.h>
#endif
...
STATIC void GC_register_map_entries(const char *maps) {
    ...
    greatest_ha = GC_our_memory[GC_n_memory - 1].hs_start + GC_our_memory[GC_n_memory - 1].hs_bytes;
    ...
    for (;;) {
        ...
        if (prot[1] == 'w') {
            ...
            /* accounted for.                                   */
#ifndef THREADS
            if ((word)start <= (word)GC_stackbottom && (word)end >= (word)GC_stackbottom) {
            ...
            if ((word)start == 0xc2fffffff000UL && (word)end == 0xc30000000000UL && path[0] == '\n')
            ...
            if ((word)end <= (word)least_ha || (word)start >= (word)greatest_ha) {
            ...
            i = 0;
            while ((word)(GC_our_memory[i].hs_start + GC_our_memory[i].hs_bytes) < (word)start)
                ++i;
            ...
            while (i < GC_n_memory && (word)GC_our_memory[i].hs_start < (word)end && (word)start < (word)end) {
    ...
    for (i = 0; i < (int)info->dlpi_phnum; i++, p++) {
        if (p->p_type == PT_GNU_RELRO) {
            ...
            for (j = n_load_segs; --j >= 0;) {
                if ((word)start >= (word)load_segs[j].start && (word)start < (word)load_segs[j].end) {
                ...
                if (0 == j && 0 == GC_has_static_roots)
                    WARN(
                        "Failed to find PT_GNU_RELRO segment"
...
STATIC GC_bool GC_register_dynamic_libraries_dl_iterate_phdr(void) {
    ...
    if (did_something) {
        ...
        dataend = DATAEND;
#endif
        if (NULL == *(char *volatile *)&datastart || (word)datastart > (word)dataend)
...
#define MA_PHYS 0
    ...
    if (GC_n_heap_sects > 0) {
        heap_end = GC_heap_sects[GC_n_heap_sects - 1].hs_start + GC_heap_sects[GC_n_heap_sects - 1].hs_bytes;
        if ((word)heap_end < (word)GC_scratch_last_end_ptr)
            heap_end = GC_scratch_last_end_ptr;
    }
    for (i = 0; i < needed_sz; i++) {
        flags = addr_map[i].pr_mflags;
        if ((flags & (MA_BREAK | MA_STACK | MA_PHYS | MA_FETCHOP | MA_NOTCACHED)) != 0) goto irrelevant;
...
#else
    char *stack_top = (char *)((word)GC_approx_sp() &
                               ~(word)(GC_sysinfo.dwAllocationGranularity - 1));

    if (base == limit) return;
    if ((word)limit > (word)stack_top && (word)base < (word)GC_stackbottom) {
...
void GC_dump_meminfo(MEMORY_BASIC_INFORMATION *buf) {
    ...
    GC_printf(
        "\tAllocationProtect= 0x%lx, State= 0x%lx, Protect= 0x%lx, "
        "Type= 0x%lx\n",
...
GC_INNER void GC_register_dynamic_libraries(void) {
    ...
    while ((word)p < (word)GC_sysinfo.lpMaximumApplicationAddress) {
        ...
        if (result == 0) {
            ...
            new_limit = (char *)(((DWORD)p + GC_sysinfo.dwAllocationGranularity) & ~(GC_sysinfo.dwAllocationGranularity - 1));
        } else
#endif
        /* else */ {
            ...
            protect = buf.Protect;
            if (buf.State == MEM_COMMIT && (protect == PAGE_EXECUTE_READWRITE || protect == PAGE_EXECUTE_WRITECOPY || protect == PAGE_READWRITE || protect == PAGE_WRITECOPY) && (buf.Type == MEM_IMAGE
...
GC_INNER void GC_register_dynamic_libraries(void) {
    ...
    for (;;) {
        ...
        while (ldi) {
            ...
            GC_add_roots_inner(
                ldi->ldinfo_dataorg,
                (ptr_t)(unsigned long)ldi->ldinfo_dataorg + ldi->ldinfo_datasize,
                TRUE);
...
#ifdef DARWIN_DEBUG
            GC_log_printf(
                "Adding on-demand section __DATA,%s at"
...
#ifdef DARWIN_DEBUG
            GC_log_printf(
                "Removing on-demand section __DATA,%s at"
                " %p-%p (%lu bytes) from image %s\n",
                secnam,
...
GC_INNER void GC_register_dynamic_libraries(void) {
    ...
    for (; p != NIL; p = p->lf_prev) {
        for (q = p->lf_ls; q != NIL; q = q->ls_next) {
            if ((q->ls_flags & PCR_IL_SegFlags_Traced_MASK) == PCR_IL_SegFlags_Traced_on) {
