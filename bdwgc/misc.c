...
GC_API void *GC_CALL GC_base(void *p) {
    ...
    r = (ptr_t)((word)r & ~(WORDS_TO_BYTES(1) - 1));
    {
        ...
        r -= HBLKDISPL(r) % sz;
        limit = r + sz;
        if (((word)limit > (word)(h + 1) && sz <= HBLKSIZE) || (word)p >= (word)limit)
            return NULL;
    }
    return (void *)r;
}
...
#endif
    pstats->bytes_reclaimed_since_gc = GC_bytes_found > 0 ? (word)GC_bytes_found : 0;
...
#define SPIN_COUNT 4000
...
#if defined(LINT2) && !(defined(GC_ASSERTIONS) && defined(GC_ALWAYS_MULTITHREADED))
        GC_disable();
#else
        GC_dont_gc = 1;
#endif
...
#endif
    {
        ...
        if (0 != interval_string) {
            ...
            if (interval <= 0) {
                WARN(
                    "GC_LARGE_ALLOC_WARN_INTERVAL environment variable has"
                    " bad value - ignoring\n",
                    0);
            } else {
    ...
    GC_init_size_map();
#ifdef PCR
    if (PCR_IL_Lock(PCR_Bool_false, PCR_allSigsBlocked, PCR_waitForever) != PCR_ERes_okay) {
...
#define IF_NEED_TO_LOCK(x) \
    if (GC_parallel || GC_need_to_lock) x
#else
#define IF_NEED_TO_LOCK(x) \
    if (GC_need_to_lock) x
#endif
#else
#define IF_NEED_TO_LOCK(x)
#endif /* !THREADS */
...
static GC_bool getWinRTLogPath(wchar_t *buf, size_t bufLen) {
    ...
    if (SUCCEEDED(WindowsCreateStringReference(
        ...
        if (SUCCEEDED(appDataStatics->lpVtbl->get_Current(appDataStatics,
            ...
            buf[0] = '\0';
            if (wcsncat_s(buf, bufLen, tempPathBuf, tempPathLen) == 0 && wcscat_s(buf, bufLen, L"\\") == 0 && wcscat_s(buf, bufLen, TEXT(GC_LOG_STD_NAME)) == 0)
                result = TRUE;
...
#if defined(ECOS) || defined(PLATFORM_WRITE) || defined(SN_TARGET_PSP2) || defined(NOSYS)
#ifdef ECOS
...
GC_API unsigned GC_CALL GC_new_kind_inner(void **fl, GC_word descr,
    ...
    GC_ASSERT(clear == TRUE || (descr == 0 && adjust == FALSE && clear == FALSE));
...
GC_API void GC_CALL GC_set_stackbottom(void *gc_thread_handle,
    ...
    GC_ASSERT(NULL == GC_blocked_sp && NULL == GC_traced_stack_sect); /* for now */
...
static void GC_CALLBACK block_add_size(struct hblk *h, GC_word pbytes) {
    hdr *hhdr = HDR(h);
    *(word *)pbytes += (WORDS_TO_BYTES(hhdr->hb_sz) + HBLKSIZE - 1) & ~(word)(HBLKSIZE - 1);
}
