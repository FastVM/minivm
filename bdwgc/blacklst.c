...
GC_INNER void GC_default_print_heap_obj_proc(ptr_t p) {
    ...
    GC_err_printf("object at %p of appr. %lu bytes (%s)\n",
                  ...
                  kind == PTRFREE ? "atomic" : IS_UNCOLLECTABLE(kind) ? "uncollectable"
                                                                      : "composite");
}
...
STATIC void GC_print_blacklisted_ptr(word p, ptr_t source,
    ...
    if (0 == base) {
        ...
        GC_err_printf(
            "Black listing (%s) %p referenced from %p in"
...
GC_API struct GC_hblk_s *GC_CALL GC_is_black_listed(struct GC_hblk_s *h,
    ...
    if (!GC_all_interior_pointers && (get_pht_entry_from_index(GC_old_normal_bl, index) || get_pht_entry_from_index(GC_incomplete_normal_bl, index))) {
        return h + 1;
    }

    nblocks = divHBLKSZ(len);
    for (i = 0;;) {
        if (GC_old_stack_bl[divWORDSZ(index)] == 0 && GC_incomplete_stack_bl[divWORDSZ(index)] == 0) {
            /* An easy case */
            i += WORDSZ - modWORDSZ(index);
        } else {
            if (get_pht_entry_from_index(GC_old_stack_bl, index) || get_pht_entry_from_index(GC_incomplete_stack_bl, index)) {
                return h + (i + 1);
            }
            i++;
        }
