...
STATIC void GC_CALLBACK GC_reclaim_block(struct hblk *hbp,
    ...
    if (sz > MAXOBJBYTES) { /* 1 big object */
        ...
        /* should be looked at.                                       */
        GC_ASSERT(sz != 0 && (GC_markers_m1 > 1 ? 3 : GC_markers_m1 + 1) * (HBLKSIZE / sz + 1) + 16 >= hhdr->hb_n_marks);
#else
...
STATIC void GC_CALLBACK GC_do_enumerate_reachable_objects(struct hblk *hbp,
    ...
    for (bit_no = 0; p <= plim; bit_no += MARK_BIT_OFFSET(sz), p += sz) {
        if (mark_bit_from_hdr(hhdr, bit_no)) {
            ((struct enumerate_reachable_s *)ped)->proc(p, sz, ((struct enumerate_reachable_s *)ped)->client_data);
        }
    }
}
