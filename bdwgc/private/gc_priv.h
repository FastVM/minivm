...
#if (defined(__linux__) || defined(__GLIBC__) || defined(__GNU__) || defined(__CYGWIN__) || defined(HAVE_PTHREAD_SETNAME_NP_WITHOUT_TID) || defined(HAVE_PTHREAD_SETNAME_NP_WITH_TID_AND_ARG) || defined(HAVE_PTHREAD_SETNAME_NP_WITH_TID)) && !defined(_GNU_SOURCE)
...
#if defined(__MINGW32__) && !defined(__MINGW_EXCPT_DEFINE_PSDK) && defined(__i386__) && defined(GC_EXTERN) /* defined in gc.c */
...
#elif defined(_MSC_VER) || defined(__INTEL_COMPILER) || defined(__DMC__) || (GC_GNUC_PREREQ(3, 0) && defined(__STRICT_ANSI__)) || defined(__BORLANDC__) || defined(__WATCOMC__)
...
#define MS_TIME_DIFF(a, b)                                          \
    ...
    ((unsigned long)((a).tv_nsec + (1000000L * 1000 - (b).tv_nsec)) / 1000000UL + ((unsigned long)((a).tv_sec - (b).tv_sec) * 1000UL) - 1000UL)
...
#define MS_TIME_DIFF(a, b) (CLOCKS_PER_SEC % 1000 == 0 ? (unsigned long)((a) - (b)) / (unsigned long)(CLOCKS_PER_SEC / 1000) \
...
#if defined(CPPCHECK) && (defined(MSWIN32) || defined(MSWINCE) || defined(CYGWIN32))
#undef TEXT
#ifdef UNICODE
#define TEXT(s) L##s
#else
...
#define SMALL_OBJ(bytes) /* bytes argument should have no side-effect */ \
    (EXPECT((bytes) <= MAXOBJBYTES - MAX_EXTRA_BYTES, TRUE) || (bytes) <= MAXOBJBYTES - EXTRA_BYTES)
...
#if (!defined(MAX_HEAP_SECTS) || defined(CPPCHECK)) && (defined(CYGWIN32) || defined(MSWIN32) || defined(MSWINCE) || defined(USE_PROC_FOR_LIBRARIES))
#ifdef LARGE_CONFIG
#if CPP_WORDSZ > 32
    ...
    do {                                                                                                       \
        ...
        for (*(pbuf) = NULL;; capacity = *(psz)) {                                                             \
            *(psz) = GC_get_procedure_stack(*(pbuf), capacity);                                                \
            ...
            if (*(psz) > ALLOCA_SAFE_LIMIT || EXPECT(capacity != 0, FALSE)) {                                  \
        ...
        if (capacity > ALLOCA_SAFE_LIMIT && EXPECT(((capacity - *(psz)) & ~(GC_page_size - 1)) != 0, FALSE)) { \
...
#define FREE_PROCEDURE_STACK_LOCAL(buf, sz)                 \
    (void)((sz) > ALLOCA_SAFE_LIMIT                         \
               ? (GC_unmap_procedure_stack_buf(buf, sz), 0) \
               : 0)
...
GC_INLINE int GC_rand_next(GC_RAND_STATE_T *pseed) {
    AO_t next = (AO_t)((AO_load(pseed) * 1103515245U + 12345) & (unsigned)GC_RAND_MAX);
    AO_store(pseed, next);
    return (int)next;
}
#else
...
#endif
#if (defined(HAVE_PTHREAD_ATTR_GET_NP) || defined(HAVE_PTHREAD_GETATTR_NP)) && defined(IA64)
...
#elif defined(static_assert) && !defined(CPPCHECK) && (__STDC_VERSION__ >= 201112L)
...
#elif (defined(GC_LINUX_THREADS) || defined(GC_DGUX386_THREADS)) && !defined(GC_USESIGRT_SIGNALS)
...
#endif
#elif defined(GC_FREEBSD_THREADS) && defined(__GLIBC__) && !defined(GC_USESIGRT_SIGNALS)
#define SIG_SUSPEND (32 + 6)
#elif (defined(GC_FREEBSD_THREADS) || defined(HURD) || defined(RTEMS)) && !defined(GC_USESIGRT_SIGNALS)
...
#if defined(GC_ASSERTIONS) && (defined(USE_COMPILER_TLS) || (defined(LINUX) && !defined(ARM32) && GC_GNUC_PREREQ(3, 3) || defined(HPUX) /* and probably others ... */))
