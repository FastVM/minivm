...
GC_INNER int GC_has_other_debug_info(ptr_t p) {
    ...
    if (((oh *)p)->oh_sf != (START_FLAG ^ (word)body) && ((word *)p)[BYTES_TO_WORDS(sz) - 1] != (END_FLAG ^ (word)body)) {
        return 0;
    }
    if (((oh *)p)->oh_sz == sz) {
...
GC_API GC_ref_kind GC_CALL GC_get_back_ptr_info(void *dest, void **base_p,
    ...
    /* insisting on even addresses.                                   */
    {
        ...
        if ((word)alternate_target >= (word)GC_least_plausible_heap_addr && (word)alternate_target <= (word)GC_greatest_plausible_heap_addr && ((word)target < (word)GC_least_plausible_heap_addr || (word)target > (word)GC_greatest_plausible_heap_addr)) {
            bp = alternate_ptr;
        }
    }
#endif
...
STATIC ptr_t GC_check_annotated_obj(oh *ohdr) {
    ...
    if (((word *)body)[SIMPLE_ROUNDED_UP_WORDS(ohdr->oh_sz)] != (END_FLAG ^ (word)body)) {
...
#endif
    if ((word)clobbered_addr <= (word)(&ohdr->oh_sz) || ohdr->oh_string == 0) {
        GC_err_printf(
        ...
        GC_err_printf("%s %p in or near object at %p (%s:%d, sz= %lu)\n",
                      msg, (void *)clobbered_addr, p,
                      (word)(ohdr->oh_string) < HBLKSIZE ? "(smashed string)" : ohdr->oh_string[0] == '\0' ? "EMPTY(smashed?)"
                                                                                                           : ohdr->oh_string,
...
GC_API void GC_CALL GC_debug_register_finalizer_no_order(void *obj, GC_finalization_proc fn,
                                                         void *cd, GC_finalization_proc *ofn,
                                                         void **ocd) {
    GC_finalization_proc my_old_fn = OFN_UNSET;
    void *my_old_cd = NULL;
...
GC_API void GC_CALL GC_debug_register_finalizer_unreachable(void *obj, GC_finalization_proc fn,
                                                            void *cd, GC_finalization_proc *ofn,
                                                            void **ocd) {
    GC_finalization_proc my_old_fn = OFN_UNSET;
    void *my_old_cd = NULL;
...
GC_API void GC_CALL GC_debug_register_finalizer_ignore_self(void *obj, GC_finalization_proc fn,
                                                            void *cd, GC_finalization_proc *ofn,
                                                            void **ocd) {
    GC_finalization_proc my_old_fn = OFN_UNSET;
    void *my_old_cd = NULL;
