...
GC_INNER GC_bool GC_is_static_root(void *p) {
    ...
    if (last_root_set < n_root_sets && (word)p >= (word)GC_static_roots[last_root_set].r_start && (word)p < (word)GC_static_roots[last_root_set].r_end)
        return TRUE;
    for (i = 0; i < n_root_sets; i++) {
        if ((word)p >= (word)GC_static_roots[i].r_start && (word)p < (word)GC_static_roots[i].r_end) {
            last_root_set = i;
            return TRUE;
        }
    }
...
STATIC void GC_remove_roots_inner(ptr_t b, ptr_t e) {
    ...
    GC_ASSERT(I_HOLD_LOCK());
    for (i = 0; i < n_root_sets;) {
        if ((word)GC_static_roots[i].r_start >= (word)b && (word)GC_static_roots[i].r_end <= (word)e) {
            GC_remove_root_at_pos(i);
        } else {
            i++;
        }
        ...
        if (!EXPECT((word)e <= (word)r_start || (word)r_end <= (word)b, TRUE)) {
            ...
            if ((word)r_start < (word)b) {
                ...
                if ((word)e < (word)r_end) {
                    ...
                    GC_remove_root_at_pos(i);
                    if (i < n_root_sets - 1 && GC_static_roots[i].r_tmp && !GC_static_roots[i + 1].r_tmp) {
                        int j;
...
GC_INNER ptr_t GC_approx_sp(void) {
    volatile word sp;
#if ((defined(E2K) && defined(__clang__)) || (defined(S390) && (__clang_major__ < 8))) && !defined(CPPCHECK)
...
STATIC void GC_push_all_stack_partially_eager(ptr_t bottom, ptr_t top,
    ...
    if (GC_all_interior_pointers) {
        ...
        GC_ASSERT((word)bottom <= (word)cold_gc_frame && (word)cold_gc_frame <= (word)top);
...
STATIC void GC_push_current_stack(ptr_t cold_gc_frame, void *context) {
    ...
    /* GC_push_all_stack_partially_eager.                       */
    {
        ...
        if (GC_all_interior_pointers && (word)cold_gc_bs_pointer > (word)GC_register_stackbottom) {
