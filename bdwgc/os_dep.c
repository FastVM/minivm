...
#if defined(ADD_HEAP_GUARD_PAGES) || defined(LINUX_STACKBOTTOM) || defined(MMAP_SUPPORTED) || defined(NEED_PROC_MAPS)
#include <errno.h>
#endif
...
#if ((defined(LINUX_STACKBOTTOM) || defined(NEED_PROC_MAPS) || defined(PROC_VDB) || defined(SOFT_VDB)) && !defined(PROC_READ)) || defined(CPPCHECK)
...
GC_INNER const char *GC_get_maps(void) {
    ...
    do {
        ...
        if (maps_size > old_maps_size) {
            ...
            WARN(
                "Unexpected asynchronous /proc/self/maps growth"
...
/* original buffer.                                                     */
#if (defined(DYNAMIC_LOADING) && defined(USE_PROC_FOR_LIBRARIES)) || defined(IA64) || defined(INCLUDE_LINUX_THREAD_DESCR) || (defined(REDIRECT_MALLOC) && defined(GC_LINUX_THREADS))
...
GC_INNER GC_bool GC_enclosing_mapping(ptr_t addr, ptr_t *startp,
    ...
    for (;;) {
        ...
        if (prot[1] == 'w' && maj_dev == 0 && (word)my_end > (word)addr && (word)my_start <= (word)addr) {
            *startp = my_start;
            *endp = my_end;
            return TRUE;
        }
...
#if defined(NEED_FIND_LIMIT) || defined(UNIX_LIKE) || (defined(WRAP_MARK_SOME) && defined(NO_SEH_AVAILABLE))
...
#if defined(NEED_FIND_LIMIT) || (defined(WRAP_MARK_SOME) && defined(NO_SEH_AVAILABLE)) || (defined(USE_PROC_FOR_LIBRARIES) && defined(THREADS))
...
GC_INNER ptr_t GC_get_register_stack_base(void) {
    ...
    return (ptr_t)(((word)GC_stackbottom - BACKING_STORE_DISPLACEMENT - 1) & ~(BACKING_STORE_ALIGNMENT - 1));
}
...
GC_INNER ptr_t GC_get_register_stack_base(void) {
    ...
    GC_ASSERT(I_HOLD_LOCK());
#ifdef USE_LIBC_PRIVATES
    if (0 != &__libc_ia64_register_backing_store_base && 0 != __libc_ia64_register_backing_store_base) {
...
ptr_t GC_get_main_stack_base(void) {
...
#endif
    {
        if (pthread_attr_getstack(&attr, &stackaddr, &size) == 0 && stackaddr != NULL) {
    ...
    WARN(
        "pthread_getattr_np or pthread_attr_getstack failed"
        " for main thread\n",
        0);
#endif
#ifdef STACKBOTTOM
...
#define STACKBOTTOM_ALIGNMENT_M1 ((word)STACK_GRAN - 1)
#ifdef STACK_GROWS_DOWN
    result = (ptr_t)(((word)GC_approx_sp() + STACKBOTTOM_ALIGNMENT_M1) & ~STACKBOTTOM_ALIGNMENT_M1);
#else
...
#elif defined(HEURISTIC2)
    {
        ...
        if ((word)result > (word)HEURISTIC2_LIMIT && (word)sp < (word)HEURISTIC2_LIMIT) {
...
GC_INNER void GC_add_current_malloc_heap(void) {
    ...
    if (GC_is_malloc_heap_base(candidate)) {
        ...
        do {
            ...
            free(p);
            req_size *= 2;
        } while (GC_is_malloc_heap_base(candidate) && req_size < GC_max_root_size / 10 && req_size < 500000);
        if (GC_is_malloc_heap_base(candidate)) {
            free(new_l);
            return;
        }
...
STATIC void GC_register_root_section(ptr_t static_root) {
    ...
    while ((word)p < (word)GC_sysinfo.lpMaximumApplicationAddress) {
        ...
        if (result != sizeof(buf) || buf.AllocationBase == 0 || GC_is_heap_base(buf.AllocationBase)) break;
...
ptr_t GC_SysVGetDataStart(size_t max_page_size, ptr_t etext_addr) {
    word text_end = ((word)(etext_addr) + sizeof(word) - 1) & ~(word)(sizeof(word) - 1);
    /* etext rounded to word boundary       */
    word next_page = ((text_end + (word)max_page_size - 1) & ~((word)max_page_size - 1));
...
GC_INNER ptr_t GC_FreeBSDGetDataStart(size_t max_page_size,
    ...
    word text_end = ((word)(etext_addr) + sizeof(word) - 1) & ~(word)(sizeof(word) - 1);
    /* etext rounded to word boundary       */
    volatile word next_page = (text_end + (word)max_page_size - 1) & ~((word)max_page_size - 1);
...
STATIC ptr_t GC_unix_mmap_get_mem(size_t bytes) {
    ...
    result = mmap(last_addr, bytes, (PROT_READ | PROT_WRITE) | (GC_pages_executable ? PROT_EXEC : 0),
                  GC_MMAP_FLAGS | OPT_MAP_ANON, zero_fd, 0 /* offset */);
#undef IGNORE_PAGES_EXECUTABLE

    if (EXPECT(MAP_FAILED == result, FALSE)) {
        if (HEAP_START == last_addr && GC_pages_executable && (EACCES == errno || EPERM == errno))
            ABORT("Cannot allocate executable pages");
        return NULL;
    }
    last_addr = (ptr_t)(((word)result + bytes + GC_page_size - 1) & ~(GC_page_size - 1));
#if !defined(LINUX)
    if (last_addr == 0) {
...
void *os2_alloc(size_t bytes) {
    ...
    if (DosAllocMem(&result, bytes, (PAG_READ | PAG_WRITE | PAG_COMMIT) | (GC_pages_executable ? PAG_EXECUTE : 0)) != NO_ERROR) {
        return NULL;
    }
...
ptr_t GC_wince_get_mem(size_t bytes) {
    ...
    for (i = 0; i < GC_n_heap_bases; i++) {
        if (((word)(-(signed_word)GC_heap_lengths[i]) & (GC_sysinfo.dwAllocationGranularity - 1)) >= bytes) {
    ...
    if (i == GC_n_heap_bases) {
        /* Reserve more pages */
        size_t res_bytes =
            SIZET_SAT_ADD(bytes, (size_t)GC_sysinfo.dwAllocationGranularity - 1) & ~((size_t)GC_sysinfo.dwAllocationGranularity - 1);
...
ptr_t GC_win32_get_mem(size_t bytes) {
    ...
    if (GLOBAL_ALLOC_TEST) {
        ...
        result = (ptr_t)(((word)result + HBLKSIZE - 1) & ~(word)(HBLKSIZE - 1));
    } else
#endif
    /* else */ {
...
#define VIRTUAL_ALLOC_PAD 0
        ...
        result = (ptr_t)VirtualAlloc(NULL,
                                     ...
                                     GetWriteWatch_alloc_flag | (MEM_COMMIT | MEM_RESERVE) | GC_mem_top_down,
                                     GC_pages_executable ? PAGE_EXECUTE_READWRITE : PAGE_READWRITE);
#undef IGNORE_PAGES_EXECUTABLE
    }
#endif /* USE_WINALLOC */
...
STATIC ptr_t GC_unmap_start(ptr_t start, size_t bytes) {
    ptr_t result = (ptr_t)(((word)start + GC_page_size - 1) & ~(GC_page_size - 1));
...
static void block_unmap_inner(ptr_t start_addr, size_t len) {
    ...
    while (len != 0) {
        ...
        if (VirtualQuery(start_addr, &mem_info, sizeof(mem_info)) != sizeof(mem_info))
            ABORT("Weird VirtualQuery result");
...
#elif defined(AIX) || defined(CYGWIN32) || defined(HAIKU) || (defined(LINUX) && !defined(PREFER_MMAP_PROT_NONE)) || defined(HPUX)
...
#undef IGNORE_PAGES_EXECUTABLE
    ...
    {
#if !defined(SN_TARGET_PS3) && !defined(FORCE_MPROTECT_BEFORE_MADVISE) && defined(LINUX) && !defined(PREFER_MMAP_PROT_NONE)
        ...
        void *result = mmap(start_addr, len, (PROT_READ | PROT_WRITE) | (GC_pages_executable ? PROT_EXEC : 0),
                            MAP_PRIVATE | MAP_FIXED | OPT_MAP_ANON,
                            zero_fd, 0 /* offset */);
        if (EXPECT(MAP_FAILED == result, FALSE))
...
#undef IGNORE_PAGES_EXECUTABLE
#else
        if (mprotect(start_addr, len, (PROT_READ | PROT_WRITE) | (GC_pages_executable ? PROT_EXEC : 0)))
...
STATIC void GC_CALLBACK GC_default_push_other_roots(void) {
    ...
    if ((*(GC_old_allocator->mmp_enumerate))(PCR_Bool_false,
                                             GC_push_old_obj, 0) != PCR_ERes_okay) {
...
#if defined(SOFT_VDB) && !defined(NO_SOFT_VDB_LINUX_VER_RUNTIME_CHECK) || (defined(GLIBC_2_19_TSX_BUG) && defined(THREADS))
...
#if (defined(CHECKSUMS) && (defined(GWW_VDB) || defined(SOFT_VDB))) || defined(PROC_VDB)
...
#define PROTECT_INNER(addr, len, allow_write, C_msg_prefix)                                                                                    \
    if (vm_protect(GC_task_self, (vm_address_t)(addr), (vm_size_t)(len),                                                                       \
                   FALSE, VM_PROT_READ | ((allow_write) ? VM_PROT_WRITE : 0) | (GC_pages_executable ? VM_PROT_EXECUTE : 0)) == KERN_SUCCESS) { \
    } else                                                                                                                                     \
        ABORT(C_msg_prefix                                                                                                                     \
              "vm_protect() failed")
...
#define PROTECT_INNER(addr, len, allow_write, C_msg_prefix)                                                    \
    if (mprotect((caddr_t)(addr), (size_t)(len),                                                               \
                 PROT_READ | ((allow_write) ? PROT_WRITE : 0) | (GC_pages_executable ? PROT_EXEC : 0)) >= 0) { \
...
#define PROTECT_INNER(addr, len, allow_write, C_msg_prefix)                                                                                \
    if (VirtualProtect(addr, len,                                                                                                          \
                       GC_pages_executable ? ((allow_write) ? PAGE_EXECUTE_READWRITE : PAGE_EXECUTE_READ) : (allow_write) ? PAGE_READWRITE \
                                                                                                                          : PAGE_READONLY, \
                       &protect_junk)) {                                                                                                   \
    } else                                                                                                                                 \
        ABORT_ARG1(C_msg_prefix "VirtualProtect failed",                                                                                   \
...
#endif
#if defined(AARCH64) || defined(ARM32) || defined(MIPS) || __FreeBSD__ >= 7
...
#define CODE_OK (exc_info->ExceptionRecord->ExceptionInformation[0] == 1) /* Write fault */
...
GC_INLINE void GC_proc_read_dirty(GC_bool output_unneeded) {
    ...
    if (getpid() != saved_proc_pid && (-1 == GC_proc_fd /* no need to retry */
...
static const pagemap_elem_t *pagemap_buffered_read(size_t *pres,
    ...
    GC_ASSERT(len > 0);
    if (pagemap_buf_fpos <= fpos && fpos < pagemap_buf_fpos + (off_t)pagemap_buf_len) {
        ...
        for (;;) {
            ...
            if ((0 == pagemap_buf_len || pagemap_buf_fpos + (off_t)pagemap_buf_len != aligned_pos) && lseek(pagemap_fd, aligned_pos, SEEK_SET) == (off_t)(-1))
            ...
            if (next_fpos_hint > aligned_pos && next_fpos_hint - aligned_pos < VDB_BUF_SZ) {
                count = VDB_BUF_SZ;
            } else {
                count = len + ofs;
                if (count > VDB_BUF_SZ)
...
static void soft_set_grungy_pages(ptr_t vaddr /* start */, ptr_t limit,
    ...
    while ((word)vaddr < (word)limit) {
        ...
        const pagemap_elem_t *bufp = pagemap_buffered_read(&res,
                                                           ...
                                                           (size_t)((((word)limit - (word)vaddr + GC_page_size - 1) / GC_page_size) * sizeof(pagemap_elem_t)),
                                                           (off_t)((word)next_start_hint / GC_page_size * sizeof(pagemap_elem_t)));
        ...
        limit_buf = ((word)vaddr & ~(GC_page_size - 1)) + (res / sizeof(pagemap_elem_t)) * GC_page_size;
...
GC_INLINE void GC_soft_read_dirty(GC_bool output_unneeded) {
    ...
    if (getpid() != saved_proc_pid && (-1 == clear_refs_fd /* no need to retry */
    ...
    if (!output_unneeded) {
        ...
        for (i = 0; i != GC_n_heap_sects; ++i) {
            ...
            soft_set_grungy_pages(vaddr, vaddr + GC_heap_sects[i].hs_bytes,
                                  i < GC_n_heap_sects - 1 ? GC_heap_sects[i + 1].hs_start : NULL);
        }
#ifdef CHECKSUMS
        GC_or_pages(GC_written_pages, GC_grungy_pages);
#endif
        ...
        for (i = 0; (int)i < n_root_sets; ++i) {
            soft_set_grungy_pages(GC_static_roots[i].r_start,
                                  ...
                                  (int)i < n_root_sets - 1 ? GC_static_roots[i + 1].r_start : NULL);
        }
#endif
    }
...
GC_INNER void GC_read_dirty(GC_bool output_unneeded) {
    ...
    if (PCR_VD_Clear(GC_vd_base, NPAGES * HBLKSIZE, GC_grungy_bits) != PCR_ERes_okay) {
...
#ifdef PCR_VDB
    if (!GC_manual_vdb) {
        if ((word)h < (word)GC_vd_base || (word)h >= (word)(GC_vd_base + NPAGES * HBLKSIZE)) {
            return TRUE;
        }
...
GC_INNER void GC_remove_protection(struct hblk *h, word nblocks,
    ...
    for (current = h_trunc; (word)current < (word)h_end; ++current) {
        ...
        if (!is_ptrfree || (word)current < (word)h || (word)current >= (word)(h + nblocks)) {
            async_set_pht_entry_from_index(GC_dirty_pages, index);
        }
    }
...
STATIC void *GC_mprotect_thread(void *arg) {
    ...
    for (;;) {
        r = mach_msg(&msg.head, MACH_RCV_MSG | MACH_RCV_LARGE | (GC_mprotect_state == GC_MP_DISCARDING ? MACH_RCV_TIMEOUT : 0),
...
GC_INNER GC_bool GC_dirty_init(void) {
    ...
    if (GC_handle_fork) {
        ...
        WARN(
            "Can't turn on GC incremental mode as fork()"
            " handling requested\n",
            0);
        return FALSE;
    }
    ...
    GC_VERBOSE_LOG_PRINTF(
        "Initializing mach/darwin mprotect"
...
#define PRELOAD_SZ 200
                ...
                if (result_buf[0] == '?' || (result_buf[result_len - 2] == ':' && result_buf[result_len - 1] == '0'))
                    break;
                /* Get rid of embedded newline, if any.  Test for "main" */
                {
                    char *nl = strchr(result_buf, '\n');
                    if (nl != NULL && (word)nl < (word)(result_buf + result_len)) {
                        *nl = ':';
                    }
